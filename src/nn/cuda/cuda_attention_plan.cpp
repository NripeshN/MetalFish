...
RequireTensorSuffix(const NetworkResolvedExecutionStep &step,
  ...
  throw std::runtime_error("CUDA attention plan is missing tensor " +
...
void RequireMatrix(const NetworkResolvedTensorRef &tensor,
  ...
  if (tensor.dims.size() != 2 || tensor.dims[0] == 0 || tensor.dims[1] == 0) {
    throw std::runtime_error("CUDA attention " + std::string(role) +
...
void ResolveSmolgenPlan(const NetworkResolvedExecutionPlan &execution_plan,
  ...
  if (global.dims[0] != kCudaAttentionSquares * kCudaAttentionSquares ||
      global.dims[1] !=
          static_cast<std::uint32_t>(smolgen.dense2_width_per_head)) {
    throw std::runtime_error(
        "CUDA attention global smolgen tensor dimensions mismatch");
  }
...
CudaAttentionStagePlan ResolveCudaAttentionStagePlan(
  ...
  if (attention_step_index >= execution_plan.steps.size())
    throw std::runtime_error("CUDA attention plan step index is out of range");
  if (head_count <= 0)
    throw std::runtime_error("CUDA attention plan head count is invalid");

  const auto &attention = execution_plan.steps[attention_step_index];
  if (attention.kind != NetworkExecutionOpKind::Attention) {
    throw std::runtime_error("CUDA attention plan step is not attention: " +
                             attention.name);
  }
  ...
  if (qkv_width % head_count != 0) {
    throw std::runtime_error(
        "CUDA attention Q/K/V width is not divisible by heads");
  }
  if (dense_w.dims[1] != static_cast<std::uint32_t>(qkv_width)) {
    throw std::runtime_error(
        "CUDA attention output projection input width mismatch");
  }
  const int output_width = static_cast<int>(dense_w.dims[0]);
  RequireBiasWidth(dense_b, output_width, "output projection bias");
  if (output_width != input_width) {
    throw std::runtime_error(
        "CUDA attention output projection must return model width");
  }

  CudaAttentionStagePlan plan;
