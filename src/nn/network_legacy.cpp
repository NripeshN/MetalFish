...
// Lightweight replacement for Lc0's LayerAdapter that relies on
...
MultiHeadWeights::MultiHeadWeights(const MetalFishNN::Weights& weights)
  ...
  if (weights.has_policy_heads()) {
    ...
    if (weights.has_policy() || weights.has_policy1() ||
      ...
      throw std::runtime_error("Could not find valid policy head weights.");
    }
  }
  ...
  if (weights.has_value_heads()) {
    ...
    if (weights.has_value() || weights.has_ip_val_w()) {
      ...
      throw std::runtime_error("Could not find valid value head weights.");
    }
  }
}
