/*
  ...
  - Indices 0-1791: Regular moves including queen promotions (encoded as
  queen-direction moves)
  - Indices 1792-1857: Underpromotions only (N/B/R) in 3 directions × 22
  positions = 66 entries
*/
  ...
  if (transform != 0) {
    ...
    if (transform & kTransposeTransform) {
      ...
      if (transform & kFlipTransform)
        inv_transform |= kMirrorTransform;
      if (transform & kMirrorTransform)
        inv_transform |= kFlipTransform;
    } else {
