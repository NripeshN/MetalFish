...
public:
   ...
   */
  bool compute(const Position *positions[], const Move moves[],
...
public:
   ...
   * @param count Number of positions
   */
  bool score_moves(const Position *positions[], GPUMove *moves[],
                   const int move_counts[], const int16_t *history, int count);
...
public:
  ...
  bool scorer_available() const {
    return move_scorer_ && move_scorer_->available();
  }
