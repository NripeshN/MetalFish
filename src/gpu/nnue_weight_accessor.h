...
struct NNUEWeightAccessor;

// Specialization for Big Network
template<>
struct NNUEWeightAccessor<Eval::NNUE::NetworkBig> {
...
template<>
struct NNUEWeightAccessor<Eval::NNUE::NetworkSmall> {
    ...
    static constexpr int HiddenDim = Eval::NNUE::TransformedFeatureDimensionsSmall;
...
public:
    ...
    static ExtractedNetworkWeights extract(const Network& network) {
        ...
        result.ft.biases_size = network.featureTransformer.biases.size() * sizeof(int16_t);
        
        result.ft.weights = network.featureTransformer.weights.data();
        result.ft.weights_size = network.featureTransformer.weights.size() * sizeof(int16_t);
        
        result.ft.psqt_weights = network.featureTransformer.psqtWeights.data();
        result.ft.psqt_size = network.featureTransformer.psqtWeights.size() * sizeof(int32_t);
        
        // Extract threat weights if applicable
        if constexpr (Accessor::HasThreats) {
            if (network.featureTransformer.threatWeights.size() > 0) {
                result.ft.threat_weights = network.featureTransformer.threatWeights.data();
                result.ft.threat_weights_size = network.featureTransformer.threatWeights.size() * sizeof(int8_t);
            }
            if (network.featureTransformer.threatPsqtWeights.size() > 0) {
                result.ft.threat_psqt = network.featureTransformer.threatPsqtWeights.data();
                result.ft.threat_psqt_size = network.featureTransformer.threatPsqtWeights.size() * sizeof(int32_t);
            }
        }
...
template<typename Network>
ExtractedNetworkWeights get_network_info() {
    using Accessor = NNUEWeightAccessor<Network>;
    ...
    result.ft.weights_size = Accessor::FT_InputDims * Accessor::HiddenDim * sizeof(int16_t);
    result.ft.psqt_size = Accessor::FT_InputDims * Accessor::PSQTBuckets * sizeof(int32_t);
    
    if (Accessor::HasThreats) {
        result.ft.threat_weights_size = Accessor::FT_ThreatDims * Accessor::HiddenDim * sizeof(int8_t);
        result.ft.threat_psqt_size = Accessor::FT_ThreatDims * Accessor::PSQTBuckets * sizeof(int32_t);
    }
    ...
    for (int i = 0; i < Accessor::LayerStacks; i++) {
        ...
        layer.fc0_weights_size = Accessor::HiddenDim * (Accessor::FC0Out + 1) * sizeof(int8_t);
        layer.fc0_biases_size = (Accessor::FC0Out + 1) * sizeof(int32_t);
        
        // FC1: FC0Out*2 inputs, FC1Out outputs
        layer.fc1_weights_size = Accessor::FC0Out * 2 * Accessor::FC1Out * sizeof(int8_t);
...
template<typename Network>
size_t get_network_memory_requirement() {
...
inline void print_network_info(const ExtractedNetworkWeights& info, const char* name) {
    ...
    if (info.has_threats) {
        std::cout << "  Threat weights: " << info.ft.threat_weights_size / 1024 << " KB\n";
        std::cout << "  Threat PSQT: " << info.ft.threat_psqt_size / 1024 << " KB\n";
    }
