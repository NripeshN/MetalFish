...
public:
    ...
    void set_buffer(Buffer* buffer, int index, size_t offset = 0) override {
        ...
        if (mtl_buffer && encoder_) {
            [encoder_ setBuffer:mtl_buffer->mtl_buffer() 
                        offset:offset 
                       atIndex:index];
        }
    }
    ...
    void dispatch_threads(size_t width, size_t height, size_t depth) override {
        if (!encoder_ || !current_kernel_) return;
        ...
        NSUInteger thread_depth = std::min(depth, max_threads / (thread_width * thread_height));
        
        MTLSize threadgroup_size = MTLSizeMake(thread_width, thread_height, thread_depth);
    ...
    void dispatch_threadgroups(size_t groups_x, size_t groups_y, size_t groups_z,
                               size_t threads_x, size_t threads_y, size_t threads_z) override {
        if (!encoder_) return;
        ...
        [encoder_ dispatchThreadgroups:grid_size threadsPerThreadgroup:threadgroup_size];
    }
...
public:
    ...
    std::unique_ptr<Buffer> create_buffer(size_t size, 
        ...
        if (!device_ || size == 0) return nullptr;
    ...
    std::unique_ptr<Buffer> create_buffer(const void* data, size_t size,
        ...
        if (!device_ || !data || size == 0) return nullptr;
    ...
    std::unique_ptr<ComputeKernel> create_kernel(const std::string& name,
        ...
        @autoreleasepool {
            ...
            if (error || !pipeline) {
                std::cerr << "[MetalBackend] Failed to create pipeline: " << name << std::endl;
                return nullptr;
            }
    ...
    void synchronize() override {
        // Create and immediately complete a command buffer to ensure all work is done
        if (queue_) {
            @autoreleasepool {
    ...
    MetalBackend() {
        @autoreleasepool {
            ...
            if (device_) {
                ...
                std::cout << "[MetalBackend] Initialized: " << device_name() << std::endl;
