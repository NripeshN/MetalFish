...
inline int32_t make_halfka_index(uint ksq, uint piece_sq, uint piece_type, uint piece_color, uint perspective) {
    ...
    for (uint color = 0; color < 2; color++) {
        for (uint pt = 1; pt <= 6; pt++) {  // PAWN to KING
            ...
            while (bb && white_count < max_features && black_count < max_features) {
                ...
                int32_t black_feat = make_halfka_index(black_ksq ^ 56, sq ^ 56, pt, color ^ 1, 1);
...
kernel void feature_transform_incremental(
    ...
    device const accumulator_t* src_accumulators [[buffer(3)]],
    ...
    accumulator_t acc = src_accumulators[pos_idx * 2 * hidden_dim + perspective * hidden_dim + hidden_idx];
    ...
    accumulators[pos_idx * 2 * hidden_dim + perspective * hidden_dim + hidden_idx] = acc;
}
...
kernel void swap_accumulator_perspectives(
...
kernel void batch_nnue_evaluate(
    device const GPUPosition* positions [[buffer(0)]],
