...
inline int32_t make_halfka_index(uint ksq, uint piece_sq, uint piece_type,
    ...
    if (piece_color != perspective) piece_idx += 6;
...
kernel void extract_halfka_features(
    ...
    for (uint color = 0; color < 2; color++) {
        for (uint pt = 1; pt <= 5; pt++) {
            ...
            while (bb && white_count < max_features && black_count < max_features) {
                ...
                int32_t bf = make_halfka_index(black_ksq ^ 56, sq ^ 56, pt, color ^ 1, 1);
...
kernel void incremental_update(
    device const weight_t* weights [[buffer(0)]],
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    accumulator_t acc = src_acc[pos_idx * 2 * hidden_dim + perspective * hidden_dim + hidden_idx];
    ...
    dst_acc[pos_idx * 2 * hidden_dim + perspective * hidden_dim + hidden_idx] = acc;
}

kernel void incremental_update_simd(
    device const weight_t* weights [[buffer(0)]],
    device const FeatureUpdate* updates [[buffer(1)]],
    device const accumulator_t* src_acc [[buffer(2)]],
    device accumulator_t* dst_acc [[buffer(3)]],
    ...
    uint simd_group [[simdgroup_index_in_threadgroup]])
{
    ...
    for (uint i = 0; i < update.num_removed; i++) {
        int32_t feat_idx = update.removed[i];
        if (feat_idx >= 0) acc -= weights[feat_idx * hidden_dim + hidden_idx];
    }
    for (uint i = 0; i < update.num_added; i++) {
        int32_t feat_idx = update.added[i];
        if (feat_idx >= 0) acc += weights[feat_idx * hidden_dim + hidden_idx];
    ...
    uint tg_size [[threads_per_threadgroup]])
{
    ...
    for (uint perspective = 0; perspective < 2; perspective++) {
        device const accumulator_t* acc = accumulators + pos_idx * 2 * hidden_dim + perspective * hidden_dim;
        ...
        for (uint out = lid; out <= FC0_OUT; out += tg_size) {
            ...
            for (uint i = 0; i < hidden_dim; i++) {
                ...
                if (clipped != 0) sum += clipped * weights[i * (FC0_OUT + 1) + out];
            }
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    output[pos_idx * FC1_OUT + out_idx] = clipped_relu(int16_t(sum >> WEIGHT_SCALE_BITS));
}

kernel void fc2_output(
    device const int8_t* fc1_out [[buffer(0)]],
    device const layer_weight_t* weights [[buffer(1)]],
    device const int32_t* biases [[buffer(2)]],
    ...
    uint gid [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < FC1_OUT; i++) sum += in_ptr[i] * weights[i];
    ...
    int32_t skip_val = ((skip_white + skip_black) * 600 * OUTPUT_SCALE) / (2 * 127 * (1 << WEIGHT_SCALE_BITS));
    ...
    uint tg_size [[threads_per_threadgroup]])
{
    ...
    for (uint out = lid; out <= FC0_OUT; out += tg_size) {
        for (uint p = 0; p < 2; p++) {
            ...
            int16_t result = int16_t(sum >> WEIGHT_SCALE_BITS);
            if (out < FC0_OUT) fc0_sqr[p * FC0_OUT + out] = sqr_clipped_relu(result);
            else fc0_skip[p] = clipped_relu(result);
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (uint out = lid; out < FC1_OUT; out += tg_size) {
        ...
        for (uint i = 0; i < 2 * FC0_OUT; i++) sum += fc0_sqr[i] * fc1_weights[i * FC1_OUT + out];
    ...
    if (lid == 0) {
        ...
        for (uint i = 0; i < FC1_OUT; i++) sum += fc1_out[i] * fc2_weights[i];
        int32_t skip_val = ((fc0_skip[0] + fc0_skip[1]) * 600 * OUTPUT_SCALE) / (2 * 127 * (1 << WEIGHT_SCALE_BITS));
        output[pos_idx] = sum + skip_val;
    }
}
...
kernel void init_accumulators(
    device const weight_t* biases [[buffer(0)]],
...
kernel void zero_buffer(
    device int32_t* buffer [[buffer(0)]],
    constant uint& count [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    if (gid < count) buffer[gid] = 0;
}

kernel void copy_accumulator(
    ...
    uint gid [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < num_removed1; i++) {
        ...
        if (feat >= 0) acc -= weights[feat * hidden_dim + gid];
    }
...
kernel void fc0_sparse_bitmask(
    device const accumulator_t* accumulators [[buffer(0)]],
    device const layer_weight_t* weights [[buffer(1)]],
    device const int32_t* biases [[buffer(2)]],
    device const uint64_t* nnz_masks [[buffer(3)]],  // Bitmask of non-zero elements
    ...
    for (uint perspective = 0; perspective < 2; perspective++) {
        ...
        for (uint out = lid; out <= FC0_OUT; out += tg_size) {
            ...
            while (remaining) {
                ...
                if (remaining >> 32) idx = 32 + ctz(uint(remaining >> 32));
                else idx = ctz(uint(remaining));
...
kernel void feature_transform_permuted(
    device const weight_t* permuted_weights [[buffer(0)]],  // Permuted for tile access
    device const weight_t* biases [[buffer(1)]],
    device const int32_t* features [[buffer(2)]],
    device const uint32_t* feature_counts [[buffer(3)]],
    device accumulator_t* accumulators [[buffer(4)]],
    ...
    constant uint& batch_size [[buffer(6)]],
    constant uint& max_features [[buffer(7)]],
    constant uint& num_features [[buffer(8)]],  // Total feature dimension
    uint2 tg_pos [[threadgroup_position_in_grid]],
    uint simd_lane [[thread_index_in_simdgroup]],
    uint simd_group [[simdgroup_index_in_threadgroup]])
{
    ...
    // Permuted weight layout: weights[tile * num_features * 32 + feature * 32 + lane]
    for (uint i = 0; i < count; i++) {
        int32_t feat_idx = pos_features[i];
        if (feat_idx >= 0 && feat_idx < int32_t(num_features)) {
            // Access permuted weights - consecutive lanes access consecutive memory
            acc += permuted_weights[tile * num_features * 32 + feat_idx * 32 + simd_lane];
        }
    }
...
kernel void forward_simd_optimized(
    device const accumulator_t* accumulators [[buffer(0)]],
    device const layer_weight_t* fc0_weights [[buffer(1)]],
    device const int32_t* fc0_biases [[buffer(2)]],
    device const layer_weight_t* fc1_weights [[buffer(3)]],
...
kernel void feature_transform_vec4(
    device const weight_t* weights [[buffer(0)]],
    device const weight_t* biases [[buffer(1)]],
    device const int32_t* features [[buffer(2)]],
    device const uint32_t* feature_counts [[buffer(3)]],
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    if (pos_idx >= batch_size || hidden_base >= hidden_dim) return;
...
kernel void update_finny_entry(
    device const accumulator_t* src_acc [[buffer(0)]],
...
kernel void load_from_finny(
    device const accumulator_t* finny_acc [[buffer(0)]],
