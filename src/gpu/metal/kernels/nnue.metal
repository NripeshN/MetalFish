...
kernel void feature_transform_full(
    ...
    device const int32_t* feature_offsets [[buffer(3)]], // Start offset per position
...
kernel void feature_transform_incremental(
    ...
    device const int32_t* update_counts [[buffer(3)]], // [num_added, num_removed] per pos
...
kernel void output_layer(
    ...
    device const weight_t* weights [[buffer(1)]],
    device const weight_t* biases [[buffer(2)]],
    device const int8_t* skip [[buffer(3)]],          // Skip connection from FC0
    ...
    uint gid [[thread_position_in_grid]])
{
    ...
    int32_t skip_val = (skip[skip_idx] * 600 * OUTPUT_SCALE) / (127 * WEIGHT_SCALE);
...
kernel void psqt_eval(
    device const int32_t* features [[buffer(0)]],
    device const int32_t* feature_offsets [[buffer(1)]],
    device const weight_t* psqt_weights [[buffer(2)]],  // [num_features x buckets]
 ...
 */
kernel void copy_accumulator_swapped(
