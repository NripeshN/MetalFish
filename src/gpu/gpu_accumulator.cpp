...
bool GPUAccumulatorEntry::allocate(int dim) {
    ...
    psqt_accumulation = backend.create_buffer(2 * GPU_PSQT_BUCKETS * sizeof(int32_t));
...
bool GPUAccumulatorStack::initialize(int hidden_dim, bool has_threats) {
    ...
    initialized_ = true;
    std::cout << "[GPU AccStack] Initialized with " << MAX_PLY << " entries" << std::endl;
    return true;
}
...
kernel void acc_copy(
    ...
    full_transform_kernel_ = backend.create_kernel("acc_full_transform", "gpu_accumulator");
    incremental_kernel_ = backend.create_kernel("acc_incremental_update", "gpu_accumulator");
    copy_kernel_ = backend.create_kernel("acc_copy", "gpu_accumulator");
    
    return full_transform_kernel_ && full_transform_kernel_->valid() &&
           incremental_kernel_ && incremental_kernel_->valid() &&
           copy_kernel_ && copy_kernel_->valid();
}

bool GPUAccumulatorStack::allocate_buffers() {
    auto& backend = gpu();
    
    features_buffer_ = backend.create_buffer(GPU_MAX_FEATURES * sizeof(int32_t));
    feature_counts_buffer_ = backend.create_buffer(sizeof(uint32_t));
    update_buffer_ = backend.create_buffer(2 * GPU_MAX_FEATURES * sizeof(int32_t) + 2 * sizeof(uint32_t));
...
GPUAccumulatorEntry& GPUAccumulatorStack::at(int ply) {
    return entries_[ply];
}
...
bool GPUAccumulatorStack::compute_full(const Position& pos, const GPUNetworkData& network) {
    if (!initialized_ || !network.valid) return false;
    ...
    for (Square s = SQ_A1; s <= SQ_H8; ++s) {
        ...
        if (p != NO_PIECE && type_of(p) != KING) {
            ...
            int white_feat = int(wksq) * 640 + int(pc) * 320 + int(pt - 1) * 64 + int(s);
            int black_feat = int(flip_rank(bksq)) * 640 + int(~pc) * 320 + int(pt - 1) * 64 + int(flip_rank(s));
...
bool GPUAccumulatorStack::compute_incremental(const Position& pos, const GPUNetworkData& network,
                                              const GPUFeatureUpdate& update) {
    if (!initialized_ || !network.valid) return false;
    if (size_ < 2) return false;  // Need previous entry
    ...
    // Pack update data: [added_features...][removed_features...][num_added][num_removed]
    ...
    encoder->set_buffer(update_buffer_.get(), 2, update.num_added * sizeof(int32_t));  // removed features
...
bool GPUFeatureExtractor::extract_batch(const std::vector<const Position*>& positions,
...
void GPUAccumulatorStack::push() {}
void GPUAccumulatorStack::pop() {}
GPUAccumulatorEntry& GPUAccumulatorStack::current() { static GPUAccumulatorEntry e; return e; }
const GPUAccumulatorEntry& GPUAccumulatorStack::current() const { static GPUAccumulatorEntry e; return e; }
GPUAccumulatorEntry& GPUAccumulatorStack::at(int) { static GPUAccumulatorEntry e; return e; }
const GPUAccumulatorEntry& GPUAccumulatorStack::at(int) const { static GPUAccumulatorEntry e; return e; }
bool GPUAccumulatorStack::compute_full(const Position&, const GPUNetworkData&) { return false; }
bool GPUAccumulatorStack::compute_incremental(const Position&, const GPUNetworkData&, const GPUFeatureUpdate&) { return false; }
...
bool GPUFeatureExtractor::extract(const Position&, std::vector<int32_t>&, std::vector<int32_t>&) { return false; }
bool GPUFeatureExtractor::extract_batch(const std::vector<const Position*>&, std::vector<int32_t>&, std::vector<uint32_t>&, std::vector<uint32_t>&) { return false; }
bool GPUFeatureExtractor::compute_delta(const Position&, Move, GPUFeatureUpdate&, GPUFeatureUpdate&) { return false; }

static std::unique_ptr<GPUFeatureExtractor> g_feature_extractor;
GPUFeatureExtractor& gpu_feature_extractor() {
    if (!g_feature_extractor) g_feature_extractor = std::make_unique<GPUFeatureExtractor>();
    return *g_feature_extractor;
}
