...
void GPUMCTSBackend::score_to_wdl(int score, float& win, float& draw, float& loss) {
...
std::vector<std::pair<MCTS::MCTSMove, float>> GPUMCTSBackend::generate_policy(
    const MCTS::MCTSPosition& pos) {
  ...
  for (size_t i = 0; i < moves.size(); ++i) {
    float prob = (total_score > 0.0f) ? scores[i] / total_score : 1.0f / moves.size();
...
MCTS::MCTSEvaluation GPUMCTSBackend::evaluate(const MCTS::MCTSPosition& pos) {
  ...
  auto [psqt, positional] = gpu_manager_->evaluate_single(
      pos.stockfish_position(), use_big_network_);
...
double GPUMCTSBackend::avg_batch_size() const {
  if (batch_evals_ == 0) return 0.0;
  return static_cast<double>(total_positions_) / batch_evals_;
}

std::unique_ptr<GPUMCTSBackend> create_gpu_mcts_backend(GPUNNUEManager* manager) {
