...
__global__ void extract_threat_features(
  ...
  for (int attacker_color = 0; attacker_color < 2 && count < max_features; attacker_color++) {
    for (int pt = 1; pt <= 6 && count < max_features; pt++) {
      uint64_t attackers = piece_bitboards[pos_idx * 14 + attacker_color * 7 + pt];
      while (attackers && count < max_features) {
        int from = lsb64(attackers);
        attackers &= attackers - 1;

        for (int target_color = 0; target_color < 2 && count < max_features; target_color++) {
          for (int target_pt = 1; target_pt <= 6 && count < max_features; target_pt++) {
            uint64_t targets = piece_bitboards[pos_idx * 14 + target_color * 7 + target_pt];
            while (targets && count < max_features) {
              int to = lsb64(targets);
              targets &= targets - 1;

              // Simplified threat index calculation
              int32_t threat_idx = attacker_color * 768 + pt * 128 + target_pt * 16 +
                                   (from % 8) + (to % 8);
...
__global__ void double_incremental_update(
    const weight_t* __restrict__ weights,
    const int32_t* __restrict__ added1,
    const int32_t* __restrict__ removed1,
    const int32_t* __restrict__ added2,
    ...
    accumulator_t* __restrict__ dst_acc,
    int hidden_dim,
    int perspective) {
 ...
 */
__global__ void fc0_sparse_input(
    const accumulator_t* __restrict__ accumulators,
    const layer_weight_t* __restrict__ weights,
    const int32_t* __restrict__ biases,
    int8_t* __restrict__ output_sqr,
  ...
  for (int perspective = 0; perspective < 2; perspective++) {
    const accumulator_t* acc = accumulators + pos_idx * 2 * hidden_dim + perspective * hidden_dim;
...
__global__ void fc0_layer_batched(
    ...
    const layer_weight_t* __restrict__ weights,  // [LAYER_STACKS][hidden_dim*2][FC0_OUT+1]
    const int32_t* __restrict__ biases,          // [LAYER_STACKS][FC0_OUT+1]
    const int32_t* __restrict__ buckets,
    int8_t* __restrict__ output_sqr,
    int8_t* __restrict__ output_linear,
    int hidden_dim,
    int batch_size) {
  ...
  const layer_weight_t* bucket_weights = weights + bucket * hidden_dim * 2 * (FC0_OUT + 1);
 ...
 * Matches Metal's transform_accumulator_output kernel
 */
__global__ void transform_accumulator_output(
  ...
  const accumulator_t* acc = accumulators + pos_idx * hidden_dim * 2 + perspective * hidden_dim;

  int16_t sum0, sum1;

  if (use_threats && threat_accumulators) {
    const accumulator_t* threat_acc = threat_accumulators + pos_idx * hidden_dim * 2 + perspective * hidden_dim;
    sum0 = max(0, min(255, static_cast<int>(acc[out_idx] + threat_acc[out_idx])));
    sum1 = max(0, min(255, static_cast<int>(acc[out_idx + half_dim] + threat_acc[out_idx + half_dim])));
  } else {
    sum0 = max(0, min(254, static_cast<int>(acc[out_idx]) >> WEIGHT_SCALE_BITS));
    sum1 = max(0, min(254, static_cast<int>(acc[out_idx + half_dim]) >> WEIGHT_SCALE_BITS));
  }
 ...
 */
__global__ void copy_accumulator_fast(
    const accumulator_t* __restrict__ src,
    accumulator_t* __restrict__ dst,
    int count) {
  ...
  if (base + 3 < count) {
    ...
    reinterpret_cast<int4*>(dst)[base / 4] = reinterpret_cast<const int4*>(src)[base / 4];
  } else {
 ...
 */
__global__ void psqt_reduce(
    const int32_t* __restrict__ partial_sums,
    int32_t* __restrict__ output,
    int num_partials,
    int batch_size) {
...
void cuda_extract_halfka_features(
    const uint64_t* piece_bitboards,
...
void cuda_extract_threat_features(
    const uint64_t* piece_bitboards,
  ...
  extract_threat_features<<<grid, block, 0, stream>>>(
      piece_bitboards, threat_features, feature_counts, batch_size, max_features);
}
...
void cuda_feature_transform_incremental(
    const weight_t* weights,
    const int32_t* added_features,
    const int32_t* removed_features,
    const uint32_t* add_counts,
    const uint32_t* remove_counts,
    const accumulator_t* src_accumulators,
    accumulator_t* dst_accumulators,
    int hidden_dim,
    int batch_size,
    cudaStream_t stream) {
...
void cuda_double_incremental_update(
    const weight_t* weights,
    const int32_t* added1,
    const int32_t* removed1,
    const int32_t* added2,
    const int32_t* removed2,
    const uint32_t* counts,
    const accumulator_t* src_acc,
    accumulator_t* dst_acc,
    int hidden_dim,
    int perspective,
    cudaStream_t stream) {
  ...
  double_incremental_update<<<grid, block, 0, stream>>>(
      weights, added1, removed1, added2, removed2, counts,
      src_acc, dst_acc, hidden_dim, perspective);
}
...
void cuda_fc0_layer(
  ...
  fc0_layer<<<grid, block, 0, stream>>>(
      accumulators, weights, biases, output_sqr, output_linear,
      hidden_dim, batch_size);
}

void cuda_fc0_sparse_input(
    const accumulator_t* accumulators,
    const layer_weight_t* weights,
    const int32_t* biases,
    int8_t* output_sqr,
  ...
  fc0_sparse_input<<<grid, block, 0, stream>>>(
      accumulators, weights, biases, output_sqr, output_linear,
      hidden_dim, batch_size, bucket);
}

void cuda_fc0_layer_batched(
    const uint8_t* input,
    const layer_weight_t* weights,
    const int32_t* biases,
  ...
  fc0_layer_batched<<<grid, block, 0, stream>>>(
      input, weights, biases, buckets, output_sqr, output_linear,
      hidden_dim, batch_size);
}
...
void cuda_fc1_layer(
  ...
  fc1_layer<<<grid, block, 0, stream>>>(
      input, weights, biases, output, batch_size);
}

void cuda_fc2_layer(
    const int8_t* fc1_out,
    const layer_weight_t* weights,
    const int32_t* biases,
    const int8_t* skip_connection,
    int32_t* output,
    int batch_size,
    cudaStream_t stream) {
  ...
  fc2_layer<<<grid, block, 0, stream>>>(
      fc1_out, weights, biases, skip_connection, output, batch_size);
}
...
void cuda_psqt_reduce(
  ...
  psqt_reduce<<<grid, block, 0, stream>>>(
      partial_sums, output, num_partials, batch_size);
}
...
void cuda_swap_accumulator_perspectives(
    const accumulator_t* src,
    accumulator_t* dst,
    int hidden_dim,
    int batch_size,
    cudaStream_t stream) {
...
void cuda_transform_accumulator_output(
    const accumulator_t* accumulators,
  ...
  transform_accumulator_output<<<grid, block, 0, stream>>>(
      accumulators, threat_accumulators, output, hidden_dim,
      batch_size, use_threats, perspective);
}

void cuda_copy_accumulator_fast(
    const accumulator_t* src,
    accumulator_t* dst,
    int count,
    cudaStream_t stream) {
