...
GPUMove GPUMove::from_move(Move m) {
    ...
    gm.data = (from & 0x3F) | ((to & 0x3F) << 6) | ((type & 0x3) << 12) | ((promo & 0x3) << 14);
    gm.score = 0;
    return gm;
}
...
Move GPUMove::to_move() const {
    ...
    if (type == 0) {
        ...
        return Move::make<PROMOTION>(Square(from), Square(to), PieceType(promo + KNIGHT));
    } else if (type == 2) {
...
bool BatchSEE::allocate_buffers() {
    ...
    positions_buffer_ = backend.create_buffer(MAX_SEE_BATCH * sizeof(GPUPosition));
...
void BatchSEE::upload_attack_tables() {
    if (!attack_tables_buffer_) return;
...
bool BatchSEE::compute(const Position* positions[], 
                       const Move moves[],
...
bool BatchMoveScorer::score_moves(const Position* positions[],
                                  GPUMove* moves[],
