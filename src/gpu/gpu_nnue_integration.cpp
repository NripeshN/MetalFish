...
size_t GPUNetworkData::memory_usage() const {
    size_t total = 0;
    if (ft_weights) total += ft_weights->size();
    if (ft_biases) total += ft_biases->size();
    if (ft_psqt) total += ft_psqt->size();
    if (threat_weights) total += threat_weights->size();
    if (threat_psqt) total += threat_psqt->size();
    
    for (const auto& layer : layers) {
        if (layer.fc0_weights) total += layer.fc0_weights->size();
        if (layer.fc0_biases) total += layer.fc0_biases->size();
        if (layer.fc1_weights) total += layer.fc1_weights->size();
        if (layer.fc1_biases) total += layer.fc1_biases->size();
        if (layer.fc2_weights) total += layer.fc2_weights->size();
        if (layer.fc2_biases) total += layer.fc2_biases->size();
    }
...
bool GPUNNUEManager::compile_shaders() {
    ...
    if (!backend.compile_library("gpu_nnue_integration", GPU_NNUE_SHADER_SOURCE)) {
        std::cerr << "[GPU NNUE] Shader compilation failed" << std::endl;
        return false;
    }
    
    feature_transform_kernel_ = backend.create_kernel("gpu_feature_transform", "gpu_nnue_integration");
    forward_fused_kernel_ = backend.create_kernel("gpu_nnue_forward", "gpu_nnue_integration");
    psqt_kernel_ = backend.create_kernel("gpu_psqt_accumulate", "gpu_nnue_integration");
...
bool GPUNNUEManager::allocate_working_buffers() {
    ...
    positions_buffer_ = backend.create_buffer(max_positions * sizeof(GPUPositionData));
    white_features_buffer_ = backend.create_buffer(max_features * sizeof(int32_t));
    black_features_buffer_ = backend.create_buffer(max_features * sizeof(int32_t));
    feature_counts_buffer_ = backend.create_buffer(max_positions * 2 * sizeof(uint32_t));
    feature_offsets_buffer_ = backend.create_buffer(max_positions * sizeof(uint32_t));
    accumulators_buffer_ = backend.create_buffer(max_positions * 2 * max_hidden * sizeof(int32_t));
    psqt_buffer_ = backend.create_buffer(max_positions * GPU_PSQT_BUCKETS * sizeof(int32_t));
    output_buffer_ = backend.create_buffer(max_positions * sizeof(int32_t));
    
    if (!positions_buffer_ || !white_features_buffer_ || !black_features_buffer_ ||
        !feature_counts_buffer_ || !feature_offsets_buffer_ || !accumulators_buffer_ ||
        !psqt_buffer_ || !output_buffer_) {
        return false;
    }
...
bool GPUNNUEManager::allocate_network_buffers(GPUNetworkData& net, int hidden_dim, bool has_threats) {
    auto& backend = gpu();
    ...
    // Feature transformer
    net.ft_weights = backend.create_buffer(GPU_HALFKA_DIMS * hidden_dim * sizeof(int16_t));
    net.ft_biases = backend.create_buffer(hidden_dim * sizeof(int16_t));
    net.ft_psqt = backend.create_buffer(GPU_HALFKA_DIMS * GPU_PSQT_BUCKETS * sizeof(int32_t));
    ...
    if (has_threats) {
        net.threat_weights = backend.create_buffer(GPU_THREAT_DIMS * hidden_dim * sizeof(int8_t));
        net.threat_psqt = backend.create_buffer(GPU_THREAT_DIMS * GPU_PSQT_BUCKETS * sizeof(int32_t));
    ...
    for (int b = 0; b < GPU_LAYER_STACKS; b++) {
        ...
        layer.fc0_weights = backend.create_buffer(hidden_dim * 2 * (GPU_FC0_OUT + 1) * sizeof(int8_t));
        layer.fc0_biases = backend.create_buffer((GPU_FC0_OUT + 1) * sizeof(int32_t));
        layer.fc1_weights = backend.create_buffer(GPU_FC0_OUT * 2 * GPU_FC1_OUT * sizeof(int8_t));
...
bool GPUNNUEManager::load_networks(const Eval::NNUE::Networks& networks) {
    ...
    std::cout << "[GPU NNUE] Total memory required: " 
              << (get_network_memory_requirement<Eval::NNUE::NetworkBig>() + 
                  get_network_memory_requirement<Eval::NNUE::NetworkSmall>()) / 1024 
              << " KB" << std::endl;
    ...
    auto big_weights = GPUNNUEWeightExtractor<Eval::NNUE::NetworkBig>::extract(networks.big);
    if (big_weights.valid) {
        ...
        if (big_weights.ft.biases && big_network_.ft_biases) {
            std::memcpy(big_network_.ft_biases->data(), big_weights.ft.biases, 
        ...
        if (big_weights.ft.weights && big_network_.ft_weights) {
            std::memcpy(big_network_.ft_weights->data(), big_weights.ft.weights,
                       std::min(big_weights.ft.weights_size, big_network_.ft_weights->size()));
        }
        if (big_weights.ft.psqt_weights && big_network_.ft_psqt) {
            std::memcpy(big_network_.ft_psqt->data(), big_weights.ft.psqt_weights,
    ...
    auto small_weights = GPUNNUEWeightExtractor<Eval::NNUE::NetworkSmall>::extract(networks.small);
    if (small_weights.valid) {
        ...
        if (small_weights.ft.biases && small_network_.ft_biases) {
            std::memcpy(small_network_.ft_biases->data(), small_weights.ft.biases,
                       std::min(small_weights.ft.biases_size, small_network_.ft_biases->size()));
        }
        if (small_weights.ft.weights && small_network_.ft_weights) {
            std::memcpy(small_network_.ft_weights->data(), small_weights.ft.weights,
                       std::min(small_weights.ft.weights_size, small_network_.ft_weights->size()));
        }
        if (small_weights.ft.psqt_weights && small_network_.ft_psqt) {
            std::memcpy(small_network_.ft_psqt->data(), small_weights.ft.psqt_weights,
...
bool GPUNNUEManager::evaluate_batch(GPUEvalBatch& batch, bool use_big_network) {
    ...
    // Forward pass (use bucket 0 for simplicity - full impl uses per-position buckets)
...
std::pair<int32_t, int32_t> GPUNNUEManager::evaluate_single(const Position& pos, bool use_big) {
...
bool GPUNNUEManager::allocate_network_buffers(GPUNetworkData&, int, bool) { return false; }
bool GPUNNUEManager::load_networks(const Eval::NNUE::Networks&) { return false; }
bool GPUNNUEManager::evaluate_batch(GPUEvalBatch&, bool) { return false; }
std::pair<int32_t, int32_t> GPUNNUEManager::evaluate_single(const Position&, bool) { return {0, 0}; }
double GPUNNUEManager::avg_batch_time_ms() const { return 0; }
void GPUNNUEManager::reset_stats() {}
size_t GPUNNUEManager::gpu_memory_used() const { return 0; }
std::string GPUNNUEManager::status_string() const { return "GPU NNUE: Not available\n"; }
