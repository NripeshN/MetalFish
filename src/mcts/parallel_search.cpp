...
ParallelGPUEvaluator::~ParallelGPUEvaluator() { stop(); }
...
bool ParallelGPUEvaluator::initialize(GPU::GPUNNUEManager *gpu_manager,
  ...
  running_ = true;
  for (int i = 0; i < config.num_eval_threads; ++i) {
    eval_threads_.emplace_back(&ParallelGPUEvaluator::eval_thread_main, this,
                               i);
  }
...
SearchWorker::SearchWorker(int id, HybridTree *tree,
                           ParallelGPUEvaluator *evaluator,
...
void SearchWorker::stop() { stop_flag_ = true; }
...
void SearchWorker::backpropagate(HybridNode *node, float value, float draw_prob,
                                 float moves_left) {
  while (node) {
...
float SearchWorker::evaluate_position(const Position &pos) {
  ...
  if (evaluator_ && evaluator_->is_running()) {
    ...
    // Store in TT
    if (tt_) {
      tt_->update_mcts(pos.key(), result.value, result.draw_prob,
                       result.moves_left);
    }
...
ParallelSearchManager::~ParallelSearchManager() { stop(); }
...
void ParallelSearchManager::start_search(HybridTree *tree,
  ...
  for (int i = 0; i < num_threads_; ++i) {
    workers_.push_back(std::make_unique<SearchWorker>(i, tree, evaluator_.get(),
                                                      &mcts_tt(), config));
