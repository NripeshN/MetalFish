...
public:
  ...
  static void compute_puct_batch(const std::vector<float> &parent_q,
...
public:
  ...
  static void compute_softmax_gpu(const std::vector<float> &scores,
                                  float temperature,
                                  std::vector<float> &probs_out);

  // Compute softmax using SIMD (for smaller batches)
  static void compute_softmax_simd(const float *scores, int count,
                                   float temperature, float *probs_out);
};
...
private:
#ifdef __APPLE__
  // Custom deleter for posix_memalign-allocated memory (must use free(), not
  // delete[])
