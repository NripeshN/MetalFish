...
void UnifiedBatchBuffer::add_position(HybridNode* node, const MCTSPosition& pos) {
...
MCTSBatchEvaluator::~MCTSBatchEvaluator() {
  stop();
}
...
void MCTSBatchEvaluator::wait_all() {
  ...
  if (config_.use_async) {
    ...
    queue_cv_.wait(lock, [this] { return pending_queue_.empty() || stop_flag_; });
  }
}
...
float MCTSBatchEvaluator::evaluate_single(const MCTSPosition& pos) {
  if (!gpu_manager_) return 0.0f;
  
  auto [psqt, score] = gpu_manager_->evaluate_single(pos.stockfish_position(), true);
...
void MCTSBatchEvaluator::eval_thread_main() {
  while (!stop_flag_) {
    ...
    // Wait for batch
    {
      ...
      // Wait with timeout
      bool got_batch = queue_cv_.wait_for(lock, 
...
void MCTSBatchEvaluator::evaluate_batch(UnifiedBatchBuffer& batch) {
  if (!gpu_manager_ || batch.count == 0) return;
  ...
  // Update statistics
  double gpu_time_us = std::chrono::duration<double, std::micro>(end_time - start_time).count();
  ...
  if (total > 0) {
    stats_.avg_batch_size = static_cast<double>(stats_.total_positions.load()) / total;
