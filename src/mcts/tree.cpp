...
void ApplyNNPolicy(ThreadSafeNode *node, const EvaluationResult &result) {
  ...
  // Policy softmax temperature (MetalFish defaults)
  constexpr float kPolicySoftmaxTemp = 1.359f;
  const float inv_temp = 1.0f / kPolicySoftmaxTemp;

  // Stack-allocated scratch buffers (max legal moves in chess is ~218).
...
#ifdef __APPLE__
  // Gather logits from the policy result.
  for (int i = 0; i < n; ++i) {
    logits_buf[i] = result.get_policy(node->edges()[i].move);
  }

  // vDSP-accelerated softmax with temperature.
  float max_logit;
  vDSP_maxv(logits_buf, 1, &max_logit, n);
  float neg_max = -max_logit;
  vDSP_vsadd(logits_buf, 1, &neg_max, logits_buf, 1, n);
  vDSP_vsmul(logits_buf, 1, &inv_temp, logits_buf, 1, n);
  int vn = n;
  vvexpf(priors_buf, logits_buf, &vn);
  float sum;
  vDSP_sve(priors_buf, 1, &sum, n);
#else
  ...
  for (int i = 0; i < n; ++i) {
    node->edges()[i].SetPolicy(priors_buf[i]);
  }
}
...
void BatchedGPUEvaluator::eval_thread_main() {
  ...
  auto collect_batch = [&](std::vector<EvalRequest *> &target, int timeout_us) {
    ...
    // Use swap-and-clear pattern instead of O(n) erase from front.
    size_t count = std::min(pending_requests_.size(),
                            static_cast<size_t>(max_batch_size_));
    if (count > 0) {
      if (count == pending_requests_.size()) {
        // Take everything -- swap avoids copy.
        target.swap(pending_requests_);
        pending_requests_.clear();
        pending_requests_.reserve(max_batch_size_);
      } else {
        target.insert(target.end(), pending_requests_.begin(),
                      pending_requests_.begin() + count);
        // Shift remaining to front efficiently.
  ...
  // Persistent prefetch thread to avoid per-iteration thread creation overhead.
  ...
  while (running_.load(std::memory_order_acquire)) {
    if (!batch.empty()) {
      // Signal persistent prefetch thread to start collecting next batch.
      {
        std::lock_guard<std::mutex> lk(prefetch_mutex);
        prefetch_done = false;
      ...
      {
        std::unique_lock<std::mutex> lk(prefetch_mutex);
        prefetch_done_cv.wait(lk, [&] { return prefetch_done; });
      }
  ...
  // Shut down the persistent prefetch thread.
  {
    std::lock_guard<std::mutex> lk(prefetch_mutex);
    prefetch_shutdown = true;
    prefetch_requested = true; // Wake it up to exit.
  }
  prefetch_cv.notify_one();
