...
// Edges are packed contiguously in arrays for cache-friendly sequential access
// during PUCT selection. Only the parent ThreadSafeNode is cache-line aligned;
// individual edges do NOT need per-element alignment since they are scanned
// sequentially by a single thread during selection.
struct TSEdge {
  Move move = Move::none();
