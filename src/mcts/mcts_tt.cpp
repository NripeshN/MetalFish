...
bool MCTSTranspositionTable::probe(uint64_t key, MCTSTTEntry& entry) const {
  ...
  if (found) {
    entry = *found;
    stats_.hits++;
    if (found->has_mcts()) stats_.mcts_hits++;
    if (found->has_ab()) stats_.ab_hits++;
    return true;
  }
...
void MCTSTranspositionTable::store_mcts(uint64_t key, const MCTSStats& stats) {
  ...
  stats_.stores++;
  if (overwrite) stats_.overwrites++;
}
...
void MCTSTranspositionTable::update_mcts(uint64_t key, float value, float draw, float moves_left) {
  MCTSTTEntry* entry = find_or_create(key);
  if (!entry) return;
...
TTMoveOrderer::TTMoveOrderer(const MCTSTranspositionTable* tt)
    : tt_(tt) {}

std::vector<TTScoredMove> TTMoveOrderer::order_moves(const MCTSPosition& pos,
...
float TTMoveOrderer::capture_score(const MCTSPosition& pos, MCTSMove move) {
  ...
  if (!sf_pos.capture(m)) return 0.0f;
...
float TTMoveOrderer::promotion_score(MCTSMove move) {
  if (!move.is_promotion()) return 0.0f;
  
  Move m = move.to_stockfish();
  PieceType promo = m.promotion_type();
  
  if (promo == QUEEN) return 8.0f;
  if (promo == ROOK) return 4.0f;
  if (promo == BISHOP || promo == KNIGHT) return 2.0f;
