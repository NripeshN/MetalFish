...
PositionType PositionFeatures::classify() const {
  ...
  // Classify
  if (tactical > 0.8f) return PositionType::HIGHLY_TACTICAL;
  if (tactical > 0.6f) return PositionType::TACTICAL;
  if (strategic > 0.8f) return PositionType::HIGHLY_STRATEGIC;
  if (strategic > 0.6f) return PositionType::STRATEGIC;
...
PositionFeatures PositionClassifier::analyze(const Position& pos) const {
  ...
  for (const auto& m : moves) {
    if (pos.capture(m)) f.num_captures++;
    if (m.type_of() == PROMOTION) f.num_promotions++;
    if (pos.gives_check(m)) f.num_checks_available++;
  }
  ...
  float tactical = 0.0f;
  if (f.in_check) tactical += 0.3f;
  if (f.num_checks_available > 0) tactical += 0.1f * std::min(f.num_checks_available, 3);
  if (f.num_captures > 0) tactical += 0.05f * std::min(f.num_captures, 5);
  if (f.num_promotions > 0) tactical += 0.2f;
  if (f.hanging_pieces[WHITE] + f.hanging_pieces[BLACK] > 0) tactical += 0.2f;
  if (f.king_attackers[WHITE] > 2 || f.king_attackers[BLACK] > 2) tactical += 0.2f;
  if (f.material_imbalance > 200) tactical += 0.1f;
  f.tactical_score = std::min(1.0f, tactical);
  
  // Compute strategic score
  float strategic = 0.0f;
  if (f.is_closed) strategic += 0.3f;
  if (f.pawn_islands[WHITE] > 2 || f.pawn_islands[BLACK] > 2) strategic += 0.1f;
  if (f.passed_pawns[WHITE] > 0 || f.passed_pawns[BLACK] > 0) strategic += 0.15f;
  if (f.is_endgame) strategic += 0.2f;
  if (!f.in_check && f.num_captures < 3) strategic += 0.1f;
  int mobility_diff = std::abs(f.mobility[WHITE] - f.mobility[BLACK]);
  if (mobility_diff > 10) strategic += 0.1f;
...
float PositionClassifier::strategic_score(const Position& pos) const {
  ...
  if (is_position_closed(pos)) score += 0.3f;
  if (is_endgame_position(pos)) score += 0.2f;
  if (!pos.checkers()) score += 0.1f;
  
  // Pawn structure complexity
  int islands = count_pawn_islands(pos, WHITE) + count_pawn_islands(pos, BLACK);
  if (islands > 4) score += 0.2f;
...
bool PositionClassifier::has_forcing_moves(const Position& pos) const {
  if (pos.checkers()) return true;
  
  // Check for checks available
  MoveList<LEGAL> moves(pos);
  for (const auto& m : moves) {
    if (pos.gives_check(m)) return true;
  }
...
int PositionClassifier::count_hanging_pieces(const Position& pos, Color c) const {
  int count = 0;
  Color them = ~c;
  
  Bitboard dominated = pos.pieces(c) & ~pos.pieces(c, PAWN) & ~pos.pieces(c, KING);
...
int PositionClassifier::count_attacked_pieces(const Position& pos, Color c) const {
  int count = 0;
  Color them = ~c;
...
int PositionClassifier::count_king_attackers(const Position& pos, Color c) const {
  Color them = ~c;
  ...
  Bitboard their_pieces = pos.pieces(them) & ~pos.pieces(them, PAWN) & ~pos.pieces(them, KING);
  ...
  while (pawns) {
    ...
    if (c == WHITE) {
      ...
      for (Rank rr = Rank(r + 1); rr <= RANK_8; ++rr) {
        if (f > FILE_A) front_span |= make_square(File(f - 1), rr);
        front_span |= make_square(f, rr);
        if (f < FILE_H) front_span |= make_square(File(f + 1), rr);
      }
    } else {
      front_span = 0ULL;
...
Bitboard PositionClassifier::get_outpost_squares(const Position& pos, Color c) const {
  ...
  Bitboard central = (file_bb(FILE_C) | file_bb(FILE_D) | file_bb(FILE_E) | file_bb(FILE_F));
  if (c == WHITE) {
...
void StrategySelector::adjust_for_time(SearchStrategy& s, int time_left_ms, int increment_ms) const {
...
void StrategySelector::adjust_for_score(SearchStrategy& s, int current_score) const {
