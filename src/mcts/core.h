...
// Fast natural logarithm approximation using IEEE 754 bit-cast trick.
// ~5x faster than std::log with <0.1% relative error, sufficient for PUCT.
inline float FastLog(float x) {
  // log(x) ~ (as_int(x) - bias) * scale
  // where bias = 127 << 23 and scale = 1 / (1 << 23) * ln(2)
  static constexpr float kScale = 8.2629582881927490e-8f; // ln(2) / (1<<23)
...
// Fast exponential approximation using Schraudolph's bit-hack.
// ~4x faster than std::exp, suitable for softmax computation.
inline float FastExp(float x) {
  // Clamp to avoid overflow/underflow in integer arithmetic.
  x = std::max(-88.0f, std::min(88.0f, x));
  // 2^23 / ln(2) = 12102203.16; bias = 127 << 23 = 1065353216
...
void ApplyDirichletNoise(EdgeArray &edges, int num_edges, float epsilon,
  ...
  for (int i = 0; i < num_edges; ++i) {
    ...
    float noisy_p = (1.0f - epsilon) * current_p + epsilon * (noise[i] / total);
...
inline float ComputePuctScore(float q, float policy, float cpuct_sqrt_n,
  ...
  float u =
      cpuct_sqrt_n * policy / (1.0f + static_cast<float>(child_n_started));
...
PuctSelectionResult SelectBestChildPuct(
  ...
  for (int i = 0; i < num_edges; ++i) {
    ...
    float score =
        ComputePuctScore(q, policy, cpuct_sqrt_n, n_started, m_utility);
  ...
  if (result.second_best_idx >= 0 && result.best_idx >= 0) {
    ...
    if (result.second_best_score > best_q) {
      ...
      float visits_f =
          p1 * cpuct_sqrt_n / (result.second_best_score - best_q) - n1 + 1;
...
template <typename Edge> EdgeRank GetEdgeRank(const Edge &edge) {
...
inline void
FinalizeScoreUpdateAtomic(AtomicFloat &wl, AtomicFloat &d, AtomicFloat &m,
                          AtomicUint &n, AtomicFloat &w_sum, float new_v,
                          float new_d, float new_m, int multivisit = 1) {
  ...
  do {
    ...
    new_w = old_w + new_v * multivisit;
      ...
      !w_sum.compare_exchange_weak(old_w, new_w, std::memory_order_acq_rel));
}
...
inline int64_t CalculateTimeForMove(const TimeManagerParams &params,
  ...
  int64_t available =
      time_remaining_ms - static_cast<int64_t>(params.move_overhead_ms);
  if (available <= 0)
