...
float HybridNode::get_puct(float cpuct, float parent_n_sqrt, float fpu) const {
  ...
  if (parent_ && edge_index_ >= 0 && edge_index_ < static_cast<int>(parent_->edges_.size())) {
...
MCTSMove HybridSearch::get_best_move() const {
  ...
  while (!should_stop()) {
    ...
    if (search_pos.is_terminal()) {
      ...
      if (result == GameResult::WHITE_WON) {
        ...
        term_type = search_pos.is_black_to_move() ? HybridNode::Terminal::Loss : HybridNode::Terminal::Win;
      } else if (result == GameResult::BLACK_WON) {
        value = search_pos.is_black_to_move() ? 1.0f : -1.0f;
        term_type = search_pos.is_black_to_move() ? HybridNode::Terminal::Win : HybridNode::Terminal::Loss;
      }
      
      node->set_terminal(term_type, value);
      backpropagate(node, value, (result == GameResult::DRAW) ? 1.0f : 0.0f, 0.0f);
      continue;
    }
...
void HybridSearch::eval_thread_main() {
  while (!stop_flag_) {
    ...
    // Wait for batch or timeout
    {
      ...
      batch_cv_.wait_for(lock, std::chrono::microseconds(config_.batch_timeout_us),
                         [this] { return !current_batch_.empty() || stop_flag_; });
      
      if (stop_flag_ && current_batch_.empty()) break;
    ...
    if (gpu_nnue_ && !batch.positions.empty()) {
      ...
      if (success) {
        ...
        for (size_t i = 0; i < batch.nodes.size(); ++i) {
          ...
          float value = std::tanh(static_cast<float>(score) / 400.0f);  // Normalize
          ...
          // Backpropagate
          backpropagate(node, value, 0.3f, 30.0f);  // Default draw prob and moves left
        }
      }
    }
...
void HybridSearch::backpropagate(HybridNode* node, float value, float draw_prob, float moves_left) {
  while (node) {
...
bool HybridSearch::should_use_alphabeta(HybridNode* node, const MCTSPosition& pos) {
  if (!config_.use_ab_for_tactics) return false;
...
int HybridSearch::alphabeta_verify(const MCTSPosition& pos, int depth, int alpha, int beta) {
  // Use Stockfish's search for alpha-beta verification
  // This is a simplified version - full integration would use Stockfish::Search
