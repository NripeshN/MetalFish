...
int get_optimal_thread_count() {
  ...
  if (sysctlbyname("hw.perflevel0.physicalcpu", &perf_cores, &size, nullptr, 0) == 0) {
...
bool GPUResidentEvalBatch::initialize(int batch_capacity) {
  ...
  positions_buffer_ = backend.create_buffer(
      positions_size,
...
const int32_t* GPUResidentEvalBatch::psqt_scores() const {
  if (!results_buffer_) return nullptr;
...
GPUPositionData* GPUResidentEvalBatch::positions_data() {
  if (!positions_buffer_) return nullptr;
...
bool AppleSiliconMCTSEvaluator::initialize(GPU::GPUNNUEManager* gpu_manager,
...
void AppleSiliconMCTSEvaluator::evaluate_batch_sync(GPUResidentEvalBatch& batch) {
  evaluate_batch_async(batch, nullptr);
}

float AppleSiliconMCTSEvaluator::get_batch_result(
    const GPUResidentEvalBatch& batch,
    int index,
...
void AppleSiliconPolicySoftmax::compute_softmax_simd(
    const float* scores,
    int count,
    float temperature,
    float* probs_out) {
