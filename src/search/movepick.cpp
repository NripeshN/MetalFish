    ...
    , ply(pl)
{
    ...
    if (pos.checkers())
        ...
        stage = Stages((depth > 0 ? MAIN_TT : QSEARCH_TT) + !(ttm.is_ok() && pos.pseudo_legal(ttm)));
}
...
{
    cur = endMoves = endBadCaptures = moves;
    stage = Stages(PROBCUT_TT + !(ttm.is_ok() && pos.capture(ttm) && pos.pseudo_legal(ttm)));
}
    ...
    case GOOD_QUIET:
        if (!skipQuiets) {
            while (cur < endMoves) {
                Move m = *(cur++);
                if (m != ttMove && 
                    (!killers || !killers->is_killer(ply, m)))
                    return m;
            }
        }
        stage = Stages(stage + 1);
