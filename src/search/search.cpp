...
void TimeManager::init(const LimitsType& limits, Color us, int ply) {
    ...
    TimePoint timeLeft = std::max(TimePoint(1), time + inc * (movestogo - 1) - moveOverhead * (2 + movestogo));
    
    double mtg = movestogo;
    double optScale = std::min(0.0120 + std::pow(ply + 2.00, 0.400) * 0.0039,
                               0.2 * time / double(timeLeft)) * 1.15;
    double maxScale = std::min(6.5, 1.5 + 0.11 * mtg);

    optimumTime = TimePoint(optScale * timeLeft);
    maximumTime = TimePoint(std::min(0.825 * time - moveOverhead, maxScale * optimumTime));
}
...
// Start searching
void Worker::start_searching(Position& pos, const LimitsType& lim, StateListPtr& st) {
    rootPos = &pos;
    limits = lim;
    states = &st;
    ...
    for (const auto& m : MoveList<LEGAL>(pos)) {
        if (limits.searchmoves.empty() || 
            std::find(limits.searchmoves.begin(), limits.searchmoves.end(), m) != limits.searchmoves.end())
...
void Worker::iterative_deepening() {
    ...
    // Iterative deepening
    for (rootDepth = 1; rootDepth <= (limits.depth ? limits.depth : MAX_PLY); ++rootDepth) {
...
Value Worker::search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    for (Move* m = moves; m != end; ++m) {
        ...
        if (depth >= 2 && moveCount > 1 + (PvNode ? 1 : 0)) {
            int reduction = reductions[std::min(moveCount, MAX_MOVES - 1)] * depth / 16;
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha, newDepth - reduction, true);
    ...
    if (rootNode) {
        RootMove& rm = *std::find(rootMoves.begin(), rootMoves.end(), bestMove ? bestMove : moves[0]);
        rm.score = bestValue;
...
Value Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    tte->save(posKey, bestValue, false,
              ...
              DEPTH_QS, bestMove, ss->staticEval, TT->generation());
...
template Value Worker::search<Root>(Position&, Stack*, Value, Value, Depth, bool);
template Value Worker::search<PV>(Position&, Stack*, Value, Value, Depth, bool);
template Value Worker::search<NonPV>(Position&, Stack*, Value, Value, Depth, bool);
