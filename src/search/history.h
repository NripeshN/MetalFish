...
template <typename T, int D, bool Atomic = false> struct StatsEntry {
...
// DynStats is a dynamically sized array of Stats, used for thread-shared
// histories which should scale with the total number of threads. The
// SizeMultiplier gives the per-thread allocation count of T.
template <typename T, int SizeMultiplier> struct DynStats {
  ...
  void clear_range(int value, size_t threadIdx, size_t numaTotal) {
    ...
    size_t end = threadIdx + 1 == numaTotal
                     ? size
                     : uint64_t(threadIdx + 1) * size / numaTotal;
...
// ordering decisions. It uses 2 tables (one for each color) indexed by the
// move's from and to squares, see
// https://www.chessprogramming.org/Butterfly_Boards
using ButterflyHistory =
    Stats<std::int16_t, 7183, COLOR_NB, UINT_16_HISTORY_SIZE>;
...
using CapturePieceToHistory =
    Stats<std::int16_t, 10692, PIECE_NB, SQUARE_NB, PIECE_TYPE_NB>;
...
using PawnHistory =
    DynStats<AtomicStats<std::int16_t, 8192, PIECE_NB, SQUARE_NB>,
...
template <typename T, int D> struct CorrectionBundle {
...
template <CorrHistType> struct CorrHistTypedef {
  using type = DynStats<Stats<std::int16_t, CORRECTION_HISTORY_LIMIT, COLOR_NB>,
                        CORRHIST_BASE_SIZE>;
};

template <> struct CorrHistTypedef<PieceTo> {
  using type =
      Stats<std::int16_t, CORRECTION_HISTORY_LIMIT, PIECE_NB, SQUARE_NB>;
};

template <> struct CorrHistTypedef<Continuation> {
  using type = MultiArray<CorrHistTypedef<PieceTo>::type, PIECE_NB, SQUARE_NB>;
};

template <> struct CorrHistTypedef<NonPawn> {
  using type = DynStats<
      Stats<std::int16_t, CORRECTION_HISTORY_LIMIT, COLOR_NB, COLOR_NB>,
      CORRHIST_BASE_SIZE>;
};

} // namespace Detail

using UnifiedCorrectionHistory = DynStats<
    MultiArray<CorrectionBundle<std::int16_t, CORRECTION_HISTORY_LIMIT>,
               COLOR_NB>,
    CORRHIST_BASE_SIZE>;
...
struct SharedHistories {
  SharedHistories(size_t threadCount)
      : correctionHistory(threadCount), pawnHistory(threadCount) {
  ...
  template <Color c> auto &nonpawn_correction_entry(const Position &pos) {
