...
void benchmark_shader_execution() {
    ...
    for (int size : sizes) {
        ...
        std::cout << size << "\t\t" << avg_ms << "\t\t" << bandwidth_gbps << std::endl;
    }
}
...
void benchmark_unified_memory() {
    ...
    auto end = std::chrono::high_resolution_clock::now();
    double write_ms = std::chrono::duration<double, std::milli>(end - start).count();
    ...
    double read_ms = std::chrono::duration<double, std::milli>(end - start).count();
    
    double bytes = size * sizeof(float);
    std::cout << "CPU Write: " << write_ms << " ms (" 
              << (bytes / (write_ms / 1000.0)) / (1024.0 * 1024.0 * 1024.0) << " GB/s)" << std::endl;
    std::cout << "CPU Read:  " << read_ms << " ms (" 
              << (bytes / (read_ms / 1000.0)) / (1024.0 * 1024.0 * 1024.0) << " GB/s)" << std::endl;
