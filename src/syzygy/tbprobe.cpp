...
using namespace MetalFish::Tablebases;

int MetalFish::Tablebases::MaxCardinality;
...
constexpr int MAX_DTZ = 1 << 18; // Max DTZ supported times 2, large enough to
                                 // deal with the syzygy TB limit.
...
// There are 8, 4, or 2 PairsData records for each TBTable, according to the
// type of table and if positions have pawns or not. It is populated at first
// access.
struct PairsData {
...
public:
  ...
  void info() const {
    sync_cout << "info string Found " << foundWDLFiles << " WDL and "
              << foundDTZFiles << " DTZ tablebase files (up to "
              << MaxCardinality << "-man)." << sync_endl;
  }
...
// codes are the same for all blocks in the table. A non-symmetric pawnless TB
...
int decompress_pairs(PairsData *d, uint64_t idx) {
  ...
  // Now compute the difference idx - I(k). From the definition of k, we know
  // that
  //
  ...
  // Move to the previous/next block, until we reach the correct block that
...
// encode k pieces of the same type and color, first sort the pieces by square
// in ascending order s1 <= s2 <= ... <= sk then compute the unique index as:
//
//      idx = Binomial[1][s1] + Binomial[2][s2] + ... + Binomial[k][sk]
//
template <typename T, typename Ret = typename T::Ret>
CLANG_AVX512_BUG_FIX Ret do_probe_table(const Position &pos, T *entry,
                                        WDLScore wdl, ProbeState *result) {
  ...
  // only stores the 'white to move' case, so if the position to lookup has
  // black to move, we need to switch the color and flip the squares before to
  // lookup.
...
encode_remaining:
  ...
  // Encode remaining pawns and then pieces according to square, in ascending
  // order
...
// a group contains pieces of the same type and color. The exception is the
// leading group that, in case of positions without pawns, can be formed by 3
// different pieces (default) or by the king pair when there is not a unique
// piece apart from the kings. When there are pawns, pawns are always first in
// pieces[].
//
...
uint8_t *set_sizes(PairsData *d, uint8_t *data) {
  ...
  // See
  // https://web.archive.org/web/20201106232444/http://www.larsson.dogma.net/dcc99.pdf
...
// Called at every probe, memory map, and init only at first access. Function is
...
// "don't care" and tries to assign to it a value that improves the compression
...
bool Tablebases::root_probe(Position &pos, Search::RootMoves &rootMoves,
  ...
  for (auto &m : rootMoves) {
    ...
    m.tbScore =
        r >= bound ? VALUE_MATE - MAX_PLY - 1
        : r > 0
            ? Value((std::max(3, r - (MAX_DTZ / 2 - 200)) * int(PawnValue)) /
                    200)
        : r == 0 ? VALUE_DRAW
        : r > -bound
            ? Value((std::min(-3, r + (MAX_DTZ / 2 - 200)) * int(PawnValue)) /
                    200)
            : -VALUE_MATE + MAX_PLY + 1;
  }
...
bool Tablebases::root_probe_wdl(Position &pos, Search::RootMoves &rootMoves,
  ...
  static const int WDL_to_rank[] = {-MAX_DTZ, -MAX_DTZ + 101, 0, MAX_DTZ - 101,
                                    MAX_DTZ};
...
Config Tablebases::rank_root_moves(const OptionsMap &options, Position &pos,
                                   Search::RootMoves &rootMoves, bool rankDTZ,
                                   const std::function<bool()> &time_abort) {
  Config config;
  ...
  if (config.cardinality >= popcount(pos.pieces()) &&
    ...
    if (!config.rootInTB && !time_abort()) {
      ...
      dtz_available = false;
      config.rootInTB =
          root_probe_wdl(pos, rootMoves, options["Syzygy50MoveRule"]);
    }
  }
