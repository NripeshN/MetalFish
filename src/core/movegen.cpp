...
Move* generate_pawn_moves(const Position& pos, Move* moveList, Bitboard target) {
    ...
    Bitboard enemies = (Type == EVASIONS ? pos.checkers() : pos.pieces(Them)) & target;
...
Move* generate_piece_moves(const Position& pos, Move* moveList, Color us, Bitboard target) {
    static_assert(Pt != KING && Pt != PAWN, "Use specialized generators for king and pawn");
...
Move* generate_all(const Position& pos, Move* moveList) {
    ...
    if constexpr (Type == CAPTURES) {
        ...
        if (!checkers) return moveList;  // No evasions needed if not in check
    ...
    if constexpr (Type != EVASIONS) {
        ...
        // Castling
        if constexpr (Type != CAPTURES) {
            if (pos.can_castle(Us & KING_SIDE) && !pos.castling_impeded(CastlingRights(Us & KING_SIDE))) {
                *moveList++ = Move::make<CASTLING>(ksq, pos.castling_rook_square(CastlingRights(Us & KING_SIDE)));
            }
            if (pos.can_castle(Us & QUEEN_SIDE) && !pos.castling_impeded(CastlingRights(Us & QUEEN_SIDE))) {
                *moveList++ = Move::make<CASTLING>(ksq, pos.castling_rook_square(CastlingRights(Us & QUEEN_SIDE)));
            }
        }
    }
    else {
