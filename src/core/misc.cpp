...
/**
 * @file misc.cpp
 * @brief Utility helpers shared across core modules.
 *
 * Provides logging helpers, version reporting, and string utilities
...
struct Tie
    : public std::streambuf { // MSVC requires split streambuf for cin and cout
...
// Returns the full name of the current MetalFish version.
//
// For local dev compiles we try to append the commit SHA and
// commit date from git. If that fails only the local compilation
// date is set and "nogit" is specified:
//      MetalFish dev-YYYYMMDD-SHA
//      or
//      MetalFish dev-YYYYMMDD-nogit
//
// For releases (non-dev builds) we only include the version number:
//      MetalFish version
std::string engine_version_info() {
  std::stringstream ss;
  ss << "MetalFish " << version << std::setfill('0');
  ...
  if constexpr (version == "dev") {
    ...
    ss << year << std::setw(2) << std::setfill('0')
       << (1 + months.find(month) / 4) << std::setw(2) << std::setfill('0')
       << day;
#endif
...
void dbg_extremes_of(int64_t value, int slot) {
  ...
  while (current_max < value &&
         !extremes.at(slot)[1].compare_exchange_weak(current_max, value)) {
  }

  int64_t current_min = extremes.at(slot)[2].load();
  while (current_min > value &&
         !extremes.at(slot)[2].compare_exchange_weak(current_min, value)) {
  }
}
...
void dbg_print() {
  ...
  for (int i = 0; i < MaxDebugSlots; ++i)
    if ((n = stdev[i][0])) {
      ...
      std::cerr << "Stdev #" << i << ": Total " << n << " Stdev " << r
                << std::endl;
    }

  for (int i = 0; i < MaxDebugSlots; ++i)
    if ((n = extremes[i][0])) {
      std::cerr << "Extremity #" << i << ": Total " << n << " Min "
                << extremes[i][2] << " Max " << extremes[i][1] << std::endl;
    }
