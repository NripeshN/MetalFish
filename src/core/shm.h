...
// argv[0] contains the command used to invoke it, which does not involve the
// full path. Just using a path is not fully resilient either, as the executable
// could have changed if it wasn't locked by the OS. Ideally we would hash the
// executable but it's not really that important at this point. If the path is
// longer than 4095 bytes the hash will be computed from an unspecified amount
// of bytes of the path; in particular it can a hash of an empty string.
...
inline std::string getExecutablePathHash() {
  ...
  ssize_t len = readlink("/proc/curproc/exe", executable_path,
...
// Utilizes shared memory to store the value. It is deduplicated system-wide
// (for the single user).
template <typename T> class SharedMemoryBackend {
public:
  enum class Status {
    Success,
  ...
  SharedMemoryBackend() : status(Status::NotInitialized) {};
  ...
  SharedMemoryBackend(SharedMemoryBackend &&other) noexcept
      : pMap(other.pMap), hMapFile(other.hMapFile), status(other.status),
...
private:
  void initialize(const std::string &shm_name, const T &value) {
    ...
    hMapFile = windows_try_with_large_page_priviliges(
        [&](size_t largePageSize) {
          ...
          return CreateFileMappingA(
              ...
              PAGE_READWRITE | SEC_COMMIT | SEC_LARGE_PAGES, total_size_high,
              total_size_low, shm_name.c_str());
        },
        []() { return (void *)nullptr; });
    ...
    volatile DWORD *is_initialized = std::launder(
        reinterpret_cast<DWORD *>(reinterpret_cast<char *>(pMap) + sizeof(T)));
...
public:
  ...
  bool is_valid() const {
    return shm1 && shm1->is_open() && shm1->is_initialized();
  }
...
template <typename T> struct SharedMemoryBackendFallback {
  ...
  SharedMemoryBackendFallback &
  operator=(const SharedMemoryBackendFallback &) = delete;

  SharedMemoryBackendFallback(SharedMemoryBackendFallback &&other) noexcept
      : fallback_object(std::move(other.fallback_object)) {}

  SharedMemoryBackendFallback &
  operator=(SharedMemoryBackendFallback &&other) noexcept {
    fallback_object = std::move(other.fallback_object);
    return *this;
  }

  SystemWideSharedConstantAllocationStatus get_status() const {
    return fallback_object == nullptr
               ? SystemWideSharedConstantAllocationStatus::NoAllocation
...
template <typename T> struct SystemWideSharedConstant {
private:
...
public:
  // We can't run the destructor because it may be in a completely different
  // process. The object stored must also be obviously in-line but we can't
  // check for that, other than some basic checks that cover most cases.
  ...
  // Content is addressed by its hash. An additional discriminator can be added
  // to account for differences that are not present in the content, for example
  ...
  SystemWideSharedConstant(const T &value, std::size_t discriminator = 0) {
    ...
    std::size_t executable_hash =
        std::hash<std::string>{}(getExecutablePathHash());

    std::string shm_name =
        std::string("Local\\sf_") + std::to_string(content_hash) + "$" +
  ...
  operator=(const SystemWideSharedConstant &) = delete;

  SystemWideSharedConstant(SystemWideSharedConstant &&other) noexcept
      : backend(std::move(other.backend)) {}

  SystemWideSharedConstant &
  operator=(SystemWideSharedConstant &&other) noexcept {
    backend = std::move(other.backend);
    return *this;
  }
  ...
  SystemWideSharedConstantAllocationStatus get_status() const {
    return std::visit(
        [](const auto &end) -> SystemWideSharedConstantAllocationStatus {
          if constexpr (std::is_same_v<std::decay_t<decltype(end)>,
                                       std::monostate>) {
            return SystemWideSharedConstantAllocationStatus::NoAllocation;
          } else {
...
private:
  ...
  std::variant<std::monostate, SharedMemoryBackend<T>,
               SharedMemoryBackendFallback<T>>
      backend;
};
