...
/**
 * @file misc.h
 * @brief Declarations for utility helpers shared across core modules.
 */
...
constexpr bool is_strictly_assignable_v =
    ...
    (std::is_same_v<To, From> || !std::is_convertible_v<From, To>);
...
template <> inline void hash_combine(std::size_t &seed, const std::size_t &v) {
  seed ^= v + 0x9e3779b9 + (seed << 6) + (seed >> 2);
}

template <typename T> inline std::size_t get_raw_data_hash(const T &value) {
...
public:
  FixedString() : length_(0) { data_[0] = '\0'; }
...
template <std::size_t N> struct std::hash<MetalFish::FixedString<N>> {
  std::size_t operator()(const MetalFish::FixedString<N> &fstr) const noexcept {
