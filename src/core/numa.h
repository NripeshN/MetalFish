...
using SetThreadSelectedCpuSetMasks_t = BOOL (*)(HANDLE, PGROUP_AFFINITY,
                                                USHORT);

// https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-getthreadselectedcpusetmasks
using GetThreadSelectedCpuSetMasks_t = BOOL (*)(HANDLE, PGROUP_AFFINITY, USHORT,
                                                PUSHORT);
...
inline const CpuIndex SYSTEM_THREADS_NB =
    std::max<CpuIndex>(1, get_hardware_concurrency());
...
struct WindowsAffinity {
  ...
  std::optional<std::set<CpuIndex>> get_combined() const {
    ...
    std::set_intersection(oldApi->begin(), oldApi->end(), newApi->begin(),
                          newApi->end(),
  ...
  // affinity set by the old API already and we want to override that. Due to
  // the limitations of the old API we cannot detect its use reliably. There
  // will be cases where we detect not use but it has actually been used and
  // vice versa.

  bool likely_used_old_api() const {
    return oldApi.has_value() || !isOldDeterminate;
  }
};
...
inline std::pair<BOOL, std::vector<USHORT>> get_process_group_affinity() {
  ...
  for (int i = 0; i < MAX_TRIES; ++i) {
    ...
    USHORT *GroupArrayAligned =
        align_ptr_up<GroupArrayMinimumAlignment>(GroupArray.get());

    const BOOL status = GetProcessGroupAffinity(GetCurrentProcess(),
                                                &GroupCount, GroupArrayAligned);
...
// On Windows there are two ways to set affinity, and therefore 2 ways to get
// it. These are not consistent, so we have to check both. In some cases it is
// actually not possible to determine affinity. For example when two different
// threads have affinity on different processor groups, set using
// SetThreadAffinityMask, we cannot retrieve the actual affinities. From
// documentation on GetProcessAffinityMask:
...
inline WindowsAffinity get_process_affinity() {
  ...
  if (GetThreadSelectedCpuSetMasks_f != nullptr) {
    ...
    status = GetThreadSelectedCpuSetMasks_f(GetCurrentThread(), nullptr, 0,
                                            &RequiredMaskCount);
    ...
    if (status == 0 && GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
      ...
      // not consistent so GetProcessAffinityMask may still return some
      // affinity.
      auto groupAffinities =
          std::make_unique<GROUP_AFFINITY[]>(RequiredMaskCount);

      status = GetThreadSelectedCpuSetMasks_f(
          GetCurrentThread(), groupAffinities.get(), RequiredMaskCount,
          &RequiredMaskCount);
  ...
  // If proc == 0 then we cannot determine affinity because it spans processor
  // groups. On Windows 11 and Server 2022 it will instead
  //     > If, however, hHandle specifies a handle to the current process, the
  //     function > always uses the calling thread's primary group (which by
  //     default is the same > as the process' primary group) in order to set
  //     the > lpProcessAffinityMask and lpSystemAffinityMask.
  ...
  std::vector<USHORT>
      groupAffinity; // We need to capture this later and capturing
      ...
      std::thread th([&]() {
        ...
        for (auto procGroupIndex : groupAffinity) {
          ...
          for (int i = 0; i < std::min(numActiveProcessors, 2); ++i) {
            ...
            status = SetThreadGroupAffinity(GetCurrentThread(), &GroupAffinity,
                                            nullptr);
            if (status == 0) {
              affinity.isOldDeterminate = false;
              return;
...
public:
  NumaReplicatedAccessToken() : n(0) {}

  explicit NumaReplicatedAccessToken(NumaIndex idx) : n(idx) {}
...
public:
  ...
  // This function queries the system for the mapping of processors to NUMA
  // nodes. On Linux we read from standardized kernel sysfs, with a fallback to
  // single NUMA node. On Windows we utilize GetNumaProcessorNodeEx, which has
  // its quirks, see comment for Windows implementation of get_process_affinity.
  static NumaConfig
  from_system([[maybe_unused]] bool respectProcessAffinity = true) {
    ...
    // and any thread can be scheduled on all processors. We try to gather this
    // information from the sysfs first
    ...
    // /sys/devices/system/node/online contains information about active NUMA
    // nodes
    ...
    //
    // 2024-08-26: It appears that we need to actually always force this
    // behaviour. While Windows allows this to work now, such assignments have
    ...
    //     all of its threads' primary group is the same. Each thread's ideal
    //     processor is in the thread's primary group, so threads will
    //     preferentially be scheduled to processors on their primary group, but
    //     they are able to be scheduled to processors on any other group.
    //
    ...
    // If the user explicitly opts out from respecting the current process
    // affinity then it may be inconsistent with the current affinity
    // (obviously), so we consider it custom.
    if (!respectProcessAffinity)
  ...
  bool suggests_binding_threads(CpuIndex numThreads) const {
    ...
    auto is_node_small = [largestNodeSize](const std::set<CpuIndex> &node) {
      ...
      return static_cast<double>(node.size()) /
                 static_cast<double>(largestNodeSize) <=
             SmallNodeThreshold;
    };
    ...
    return (numThreads > largestNodeSize / 2 ||
            numThreads >= numNotSmallNodes * 4) &&
           nodes.size() > 1;
  }

  std::vector<NumaIndex>
  distribute_threads_among_numa_nodes(CpuIndex numThreads) const {
    ...
    if (nodes.size() == 1) {
      ...
      for (CpuIndex c = 0; c < numThreads; ++c) {
        ...
        for (NumaIndex n = 0; n < nodes.size(); ++n) {
          float fill = static_cast<float>(occupation[n] + 1) /
                       static_cast<float>(nodes[n].size());
  ...
  bind_current_thread_to_numa_node(NumaIndex n) const {
    ...
    if (SetThreadSelectedCpuSetMasks_f != nullptr) {
      ...
      const USHORT numProcGroups =
          USHORT(((highestCpuIndex + 1) + WIN_PROCESSOR_GROUP_SIZE - 1) /
                 WIN_PROCESSOR_GROUP_SIZE);
      auto groupAffinities = std::make_unique<GROUP_AFFINITY[]>(numProcGroups);
      std::memset(groupAffinities.get(), 0,
                  sizeof(GROUP_AFFINITY) * numProcGroups);
      ...
      for (CpuIndex c : nodes[n]) {
        ...
        groupAffinities[procGroupIndex].Mask |= KAFFINITY(1)
                                                << idxWithinProcGroup;
      }

      HANDLE hThread = GetCurrentThread();

      const BOOL status = SetThreadSelectedCpuSetMasks_f(
          hThread, groupAffinities.get(), numProcGroups);
      if (status == 0)
        std::exit(EXIT_FAILURE);

      // We yield this thread just to be sure it gets rescheduled.
      // This is defensive, allowed because this code is not performance
      // critical.
      SwitchToThread();
    }

    // Sometimes we need to force the old API, but do not use it unless
    // necessary.
    if (SetThreadSelectedCpuSetMasks_f == nullptr ||
        STARTUP_USE_OLD_AFFINITY_API) {
      // On earlier windows version (since windows 7) we cannot run a single
      // thread on multiple processor groups, so we need to restrict the group.
      // We assume the group of the first processor listed for this node.
      // Processors from outside this group will not be assigned for this
      // thread. Normally this won't be an issue because windows used to assign
      // NUMA nodes such that they cannot span processor groups. However, since
      // Windows 10 Build 20348 the behaviour changed, so there's a small window
      // of versions between this and Windows 11 that might exhibit problems
      ...
      // processor nodes. This is required because otherwise our thread
      // distribution code may produce suboptimal results.
      //
      // See
      // https://learn.microsoft.com/en-us/windows/win32/procthread/numa-support
      ...
      const size_t forcedProcGroupIndex =
          *(nodes[n].begin()) / WIN_PROCESSOR_GROUP_SIZE;
...
private:
  ...
  NumaConfig(EmptyNodeTag) : highestCpuIndex(0), customAffinity(false) {}
  ...
  indices_from_shortened_string(const std::string &s) {
...
// NumaReplicationContext informs all tracked instances when NUMA configuration
// changes.
...
// NUMA config changes the value stored at the index 0 is replicated to other
// nodes.
template <typename T> class NumaReplicated : public NumaReplicatedBase {
public:
  using ReplicatorFuncType = std::function<T(const T &)>;

  NumaReplicated(NumaReplicationContext &ctx) : NumaReplicatedBase(ctx) {
    replicate_from(T{});
  }
  ...
  template <typename FuncT> void modify_and_replicate(FuncT &&f) {
  ...
  void on_numa_config_changed() override {
    ...
    // because they all must be identical, but the first one is guaranteed to
    // exist.
    auto source = std::move(instances[0]);
    replicate_from(std::move(*source));
  }
  ...
  void replicate_from(T &&source) {
    ...
    if (cfg.requires_memory_replication()) {
      for (NumaIndex n = 0; n < cfg.num_numa_nodes(); ++n) {
        cfg.execute_on_numa_node(n, [this, &source]() {
...
template <typename T> class LazyNumaReplicated : public NumaReplicatedBase {
public:
  using ReplicatorFuncType = std::function<T(const T &)>;

  LazyNumaReplicated(NumaReplicationContext &ctx) : NumaReplicatedBase(ctx) {
...
private:
  ...
  void ensure_present(NumaIndex idx) const {
    ...
    cfg.execute_on_numa_node(idx, [this, idx]() {
      instances[idx] = std::make_unique<T>(*instances[0]);
    });
  }
...
public:
  ...
  operator=(const LazyNumaReplicatedSystemWide &) = delete;
  LazyNumaReplicatedSystemWide &
  operator=(LazyNumaReplicatedSystemWide &&other) noexcept {
    NumaReplicatedBase::operator=(*this, std::move(other));
    instances = std::exchange(other.instances, {});
  ...
  std::vector<std::pair<SystemWideSharedConstantAllocationStatus,
...
private:
  ...
  std::size_t get_discriminator(NumaIndex idx) const {
    ...
    // as a discriminator, locate the hardware/system numadomain this cpuindex
    // belongs to
    CpuIndex cpu = *cfg.nodes[idx].begin(); // get a CpuIndex from NumaIndex
    NumaIndex sys_idx =
        cfg_sys.is_cpu_assigned(cpu) ? cfg_sys.nodeByCpu.at(cpu) : 0;
  ...
  void ensure_present(NumaIndex idx) const {
    ...
    cfg.execute_on_numa_node(idx, [this, idx]() {
      instances[idx] =
          SystemWideSharedConstant<T>(*instances[0], get_discriminator(idx));
    });
  }
  ...
  void prepare_replicate_from(std::unique_ptr<T> &&source) {
    ...
    // Note that we cannot move here as we need to reallocate the data
    // on the correct NUMA node.
    // Even in the case of a single NUMA node we have to copy since it's shared
    // memory.
    if (cfg.requires_memory_replication()) {
      assert(cfg.num_numa_nodes() > 0);
      ...
      cfg.execute_on_numa_node(0, [this, &source]() {
        instances.emplace_back(
            SystemWideSharedConstant<T>(*source, get_discriminator(0)));
      });
...
class NumaReplicationContext {
public:
  NumaReplicationContext(NumaConfig &&cfg) : config(std::move(cfg)) {}
  ...
  void move_attached([[maybe_unused]] NumaReplicatedBase *oldObj,
                     NumaReplicatedBase *newObj) {
...
private:
  ...
  // std::set uses std::less by default, which is required for pointer
  // comparison
...
inline NumaReplicatedBase::NumaReplicatedBase(
    NumaReplicatedBase &&other) noexcept
