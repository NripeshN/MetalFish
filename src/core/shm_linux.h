...
inline std::unordered_set<SharedMemoryBase *>
    SharedMemoryRegistry::active_instances_;
...
private:
  ...
  static void register_signal_handlers() noexcept {
    ...
    constexpr int signals[] = {SIGHUP,  SIGINT, SIGQUIT, SIGILL,
...
template <typename T> class SharedMemory : public detail::SharedMemoryBase {
...
private:
  ...
  static std::string make_sentinel_base(const std::string &name) {
    ...
    char buf[32];
    std::snprintf(buf, sizeof(buf), "sfshm_%016" PRIx64,
...
public:
  explicit SharedMemory(const std::string &name) noexcept
      : name_(name), total_size_(calculate_total_size()),
  ...
  SharedMemory(SharedMemory &&other) noexcept
      : name_(std::move(other.name_)), fd_(other.fd_),
        mapped_ptr_(other.mapped_ptr_), data_ptr_(other.data_ptr_),
        header_ptr_(other.header_ptr_), total_size_(other.total_size_),
  ...
  [[nodiscard]] bool open(const T &initial_value) noexcept {
    ...
    while (true) {
      ...
      bool success = created_new ? setup_new_region(initial_value)
                                 : setup_existing_region(invalid_header);
  ...
  [[nodiscard]] uint32_t ref_count() const noexcept {
    return header_ptr_ ? header_ptr_->ref_count.load(std::memory_order_acquire)
                       : 0;
  }

  [[nodiscard]] bool is_initialized() const noexcept {
    return header_ptr_
               ? header_ptr_->initialized.load(std::memory_order_acquire)
               : false;
  }

  static void cleanup_all_instances() noexcept {
    detail::SharedMemoryRegistry::cleanup_all();
  }
...
private:
  ...
  bool create_sentinel_file_locked() noexcept {
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
      int fd = ::open(sentinel_path_.c_str(),
  ...
  [[nodiscard]] bool setup_new_region(const T &initial_value) noexcept {
    ...
    if (detail::portable_fallocate(fd_, 0, static_cast<off_t>(total_size_)) !=
        0)
      return false;

    mapped_ptr_ =
        mmap(nullptr, total_size_, PROT_READ | PROT_WRITE, MAP_SHARED, fd_, 0);
    ...
    header_ptr_ = reinterpret_cast<detail::ShmHeader *>(
        static_cast<char *>(mapped_ptr_) + sizeof(T));
  ...
  [[nodiscard]] bool setup_existing_region(bool &invalid_header) noexcept {
    ...
    data_ptr_ = static_cast<T *>(mapped_ptr_);
    header_ptr_ = std::launder(reinterpret_cast<detail::ShmHeader *>(
        static_cast<char *>(mapped_ptr_) + sizeof(T)));
...
template <typename T>
[[nodiscard]] std::optional<SharedMemory<T>>
create_shared(const std::string &name, const T &initial_value) noexcept {
