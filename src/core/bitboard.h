...
// given squares are not on a same file/rank/diagonal, it returns s2. For
// instance, between_bb(SQ_C4, SQ_F7) will return a bitboard with squares D5, E6
// and F7, but between_bb(SQ_E6, SQ_F8) will return a bitboard with the square
// F8. This trick allows to generate non-king evasion moves faster: the
// defending piece must either interpose itself to cover the check or capture
// the checking piece.
...
constexpr Bitboard safe_destination(Square s, int step) {
  ...
  return is_ok(to) && abs(file_of(s) - file_of(to)) <= 2 ? square_bb(to)
                                                         : Bitboard(0);
}
...
inline constexpr auto PseudoAttacks = []() constexpr {
  ...
  for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1) {
    ...
    attacks[QUEEN][s1] = attacks[BISHOP][s1] =
        Bitboards::pseudo_attacks(BISHOP, s1);
    attacks[QUEEN][s1] |= attacks[ROOK][s1] =
        Bitboards::pseudo_attacks(ROOK, s1);
  }
