...
using AdjustTokenPrivileges_t = bool (*)(HANDLE, BOOL, PTOKEN_PRIVILEGES, DWORD,
...
memory_allocator(ALLOC_FUNC alloc_func, size_t num) {
  ...
  char *raw_memory = reinterpret_cast<char *>(
      alloc_func(array_offset + num * sizeof(ElementType)));
  ASSERT_ALIGNED(raw_memory, alignof(T));
...
template <typename T> struct LargePageArrayDeleter {
  void operator()(T *ptr) const {
    return memory_deleter_array<T>(ptr, aligned_large_pages_free);
  }
};

template <typename T>
using LargePagePtr = std::conditional_t<
    std::is_array_v<T>,
...
make_unique_large_page(Args &&...args) {
  ...
  T *obj = memory_allocator<T>(aligned_large_pages_alloc,
...
template <typename T> struct AlignedDeleter {
  void operator()(T *ptr) const {
    return memory_deleter<T>(ptr, std_aligned_free);
  }
};

template <typename T> struct AlignedArrayDeleter {
  void operator()(T *ptr) const {
    return memory_deleter_array<T>(ptr, std_aligned_free);
  }
};

template <typename T>
using AlignedPtr = std::conditional_t<
    std::is_array_v<T>,
...
make_unique_aligned(Args &&...args) {
  const auto func = [](size_t size) {
    return std_aligned_alloc(alignof(T), size);
  };
...
auto windows_try_with_large_page_priviliges([[maybe_unused]] FuncYesT &&fyes,
                                            FuncNoT &&fno) {
  ...
  auto OpenProcessToken_f = OpenProcessToken_t(
      (void (*)())GetProcAddress(hAdvapi32, "OpenProcessToken"));
  if (!OpenProcessToken_f)
    return fno();
  auto LookupPrivilegeValueA_f = LookupPrivilegeValueA_t(
      (void (*)())GetProcAddress(hAdvapi32, "LookupPrivilegeValueA"));
  if (!LookupPrivilegeValueA_f)
    return fno();
  auto AdjustTokenPrivileges_f = AdjustTokenPrivileges_t(
      (void (*)())GetProcAddress(hAdvapi32, "AdjustTokenPrivileges"));
  ...
  // AdjustTokenPrivileges() succeeds, we still need to query GetLastError() to
  // ensure that the privileges were actually obtained.

  if (!AdjustTokenPrivileges_f(hProcessToken, FALSE, &tp,
