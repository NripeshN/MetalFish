...
Position &Position::set(const std::string &fenStr, bool isChess960,
                        StateInfo *si) {
  /*
  ...
  */
  ...
  while ((ss >> token) && !isspace(token)) {
    ...
    if (token == 'K') {
      for (rsq = make_square(FILE_H, rank); type_of(piece_on(rsq)) != ROOK;
           --rsq) {
      }
    } else if (token == 'Q') {
      for (rsq = make_square(FILE_A, rank); type_of(piece_on(rsq)) != ROOK;
           ++rsq) {
      }
...
std::string Position::fen() const {
  ...
  ss << (st->epSquare == SQ_NONE
             ? " -"
             : " " + std::string{char('a' + file_of(st->epSquare)),
                                 char('1' + rank_of(st->epSquare))});
...
void Position::update_slider_blockers(Color c) const {
  ...
  Bitboard snipers = ((attacks_bb(ROOK, ksq, 0) & pieces(QUEEN, ROOK)) |
                      (attacks_bb(BISHOP, ksq, 0) & pieces(QUEEN, BISHOP))) &
                     pieces(~c);
...
bool Position::legal(Move m) const {
  ...
  if (m.type_of() == EN_PASSANT) {
    ...
    Bitboard occupied =
        (pieces() ^ square_bb(from) ^ square_bb(capsq)) | square_bb(to);
  ...
  if (m.type_of() == CASTLING) {
    ...
    return !chess960 || !(attacks_bb(ROOK, to, pieces() ^ square_bb(to)) &
                          pieces(~us, ROOK));
  }
...
bool Position::gives_check(Move m) const {
  ...
  if (blockers_for_king(~sideToMove) & square_bb(from))
    if (!LineBB[square<KING>(~sideToMove)][from] ||
        !(LineBB[square<KING>(~sideToMove)][from] & square_bb(to)))
      return true;
  ...
  case PROMOTION:
    return attacks_bb(m.promotion_type(), to, pieces() ^ square_bb(from)) &
           square_bb(square<KING>(~sideToMove));
...
// Check if a move is pseudo-legal (could be legal in some position with same
// piece placement)
bool Position::pseudo_legal(Move m) const {
  ...
  if (pt == PAWN) {
    ...
    // Push
      ...
      if (us == WHITE) {
        if (dist != 8 && dist != 16)
          return false;
        if (dist == 16 && (relative_rank(us, from) != RANK_2 ||
                           (pieces() & square_bb(Square(int(from) + 8)))))
          return false;
      } else {
        if (dist != -8 && dist != -16)
          return false;
  ...
  case KNIGHT:
    ...
    break;
  case BISHOP:
    attacks = attacks_bb(BISHOP, from, pieces());
    break;
  case ROOK:
    attacks = attacks_bb(ROOK, from, pieces());
    break;
  case QUEEN:
    attacks = attacks_bb(QUEEN, from, pieces());
    break;
  case KING:
...
// Castling helper
void Position::do_castling_helper(bool doCastling, Color us, Square from,
                                  Square &to, Square &rfrom, Square &rto) {
