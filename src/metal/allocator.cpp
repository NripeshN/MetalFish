...
MetalAllocator::~MetalAllocator() { clear_cache(); }
...
Buffer MetalAllocator::malloc(size_t size) {
  ...
  // Try to find a cached buffer
  {
    ...
    if (it != bufferCache_.end() && !it->second.empty()) {
      ...
      while (current > peak &&
             !peakMemory_.compare_exchange_weak(peak, current)) {
      }
  ...
  if (!buffer) {
    throw std::runtime_error(
        "[MetalFish::MetalAllocator] Failed to allocate Metal buffer");
  }
...
size_t MetalAllocator::size(Buffer buffer) const { return buffer.size; }
