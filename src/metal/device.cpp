...
CommandEncoder::CommandEncoder(DeviceStream& stream) : stream_(stream), enc_(nullptr) {
    if (!stream_.buffer) {
...
Device::Device() {
    ...
    std::cout << "[MetalFish] Initialized Metal device: " << architecture_ << std::endl;
    std::cout << "[MetalFish] Unified memory: " << (device_->hasUnifiedMemory() ? "Yes" : "No") << std::endl;
    std::cout << "[MetalFish] Max threadgroup memory: " << device_->maxThreadgroupMemoryLength() << " bytes" << std::endl;
}
...
Device::~Device() {
    ...
    for (auto& [idx, stream] : stream_map_) {
        if (stream.queue) stream.queue->release();
    }
    if (device_) device_->release();
}

MTL::Device* Device::mtl_device() {
    return device_;
}
...
MTL::ComputePipelineState* Device::get_kernel(const std::string& name, MTL::Library* mtl_lib) {
    // Check cache
    ...
    if (!mtl_lib) {
        ...
        if (!mtl_lib) {
            std::cerr << "[MetalFish] No library available for kernel: " << name << std::endl;
            return nullptr;
        }
    }
    ...
    NS::Error* error = nullptr;
    MTL::ComputePipelineState* kernel = device_->newComputePipelineState(function, &error);
    function->release();
...
const std::string& Device::get_architecture() const {
