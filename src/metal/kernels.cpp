...
public:
    ...
    bool initialize(const std::string& libPath) {
        try {
            ...
            if (library_) {
                // Load kernels
                nnueFeatureTransform_ = device.get_kernel("nnue_feature_transform", library_);
                nnueFullyConnected_ = device.get_kernel("nnue_fully_connected", library_);
    ...
    void dispatch_feature_transform(Buffer& input, Buffer& output, size_t count) {
        if (!nnueFeatureTransform_) return;
        ...
        NS::UInteger threadGroupSize = nnueFeatureTransform_->maxTotalThreadsPerThreadgroup();
    ...
    void dispatch_fully_connected(Buffer& input, Buffer& weights, Buffer& bias, 
                                  Buffer& output, uint32_t inputSize, uint32_t outputSize) {
        if (!nnueFullyConnected_) return;
        ...
        NS::UInteger threadGroupSize = nnueFullyConnected_->maxTotalThreadsPerThreadgroup();
        if (threadGroupSize > outputSize) threadGroupSize = outputSize;
        MTL::Size groupSize = MTL::Size::Make(threadGroupSize, 1, 1);
    ...
    void dispatch_position_eval(Buffer& boardStates, Buffer& scores, size_t count) {
        if (!evaluatePosition_) return;
        ...
        MTL::Size gridSize = MTL::Size::Make(count, 1, 1);
        NS::UInteger threadGroupSize = evaluatePosition_->maxTotalThreadsPerThreadgroup();
        if (threadGroupSize > count) threadGroupSize = count;
        MTL::Size groupSize = MTL::Size::Make(threadGroupSize, 1, 1);
    ...
    // Move generation
    void dispatch_move_gen(Buffer& boardStates, Buffer& moves, Buffer& moveCounts, size_t count) {
        if (!generateMoves_) return;
        ...
        MTL::Size gridSize = MTL::Size::Make(count, 1, 1);
        NS::UInteger threadGroupSize = generateMoves_->maxTotalThreadsPerThreadgroup();
        if (threadGroupSize > count) threadGroupSize = count;
        MTL::Size groupSize = MTL::Size::Make(threadGroupSize, 1, 1);
