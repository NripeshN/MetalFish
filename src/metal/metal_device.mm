...
struct MetalDevice::Impl {
  ...
  bool initialize() {
    @autoreleasepool {
      ...
      NSError *error = nil;
      NSString *shaderPath =
          [[NSBundle mainBundle] pathForResource:@"nnue" ofType:@"metallib"];
      ...
      if (!library) {
        ...
        NSString *sourcePath = [[NSBundle mainBundle] pathForResource:@"nnue"
                                                               ofType:@"metal"];
        if (sourcePath) {
          NSString *source =
...
void MetalDevice::dispatch_compute(const std::string &kernel_name,
  ...
  @autoreleasepool {
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
      ...
      id<MTLBuffer> mtlBuffer =
          [pImpl->device newBufferWithBytesNoCopy:buffers[i]
    ...
    MTLSize threadgroupSize =
        MTLSizeMake(std::min(grid_size_x, (size_t)threadGroupSize), 1, 1);
