...
void double_inc_update(
    ...
    const Square ksq, AccumulatorState<PSQFeatureSet> &middle_state,
...
void double_inc_update(
    Color perspective,
    const FeatureTransformer<TransformedFeatureDimensions> &featureTransformer,
    const Square ksq, AccumulatorState<ThreatFeatureSet> &middle_state,
    AccumulatorState<ThreatFeatureSet> &target_state,
    const AccumulatorState<ThreatFeatureSet> &computed, const DirtyPiece &dp2);
...
void update_accumulator_refresh_cache(
    Color perspective, const FeatureTransformer<Dimensions> &featureTransformer,
...
template const AccumulatorState<PSQFeatureSet> &
AccumulatorStack::latest() const noexcept;
template const AccumulatorState<ThreatFeatureSet> &
AccumulatorStack::latest() const noexcept;
...
AccumulatorStack::accumulators() const noexcept {
  static_assert(std::is_same_v<T, PSQFeatureSet> ||
                    std::is_same_v<T, ThreatFeatureSet>,
...
void AccumulatorStack::evaluate_side(
  ...
  if ((accumulators<FeatureSet>()[last_usable_accum].template acc<Dimensions>())
    ...
    backward_update_incremental<FeatureSet>(
        perspective, pos, featureTransformer, last_usable_accum);
  }
}

// Find the earliest usable accumulator, this can either be a computed
// accumulator or the accumulator state just before a change that requires full
// refresh.
template <typename FeatureSet, IndexType Dimensions>
std::size_t AccumulatorStack::find_last_usable_accumulator(
  ...
  for (std::size_t curr_idx = size - 1; curr_idx > 0; curr_idx--) {
    if ((accumulators<FeatureSet>()[curr_idx].template acc<Dimensions>())
            .computed[perspective])
      return curr_idx;

    if (FeatureSet::requires_refresh(accumulators<FeatureSet>()[curr_idx].diff,
                                     perspective))
      return curr_idx;
  }
...
void AccumulatorStack::forward_update_incremental(
  ...
  assert((accumulators<FeatureSet>()[begin].template acc<Dimensions>())
             .computed[perspective]);
    ...
    if (next + 1 < size) {
      ...
      if constexpr (std::is_same_v<FeatureSet, ThreatFeatureSet>) {
        if (dp2.remove_sq != SQ_NONE &&
            (accumulators[next].diff.threateningSqs &
...
void AccumulatorStack::backward_update_incremental(
  ...
  assert(end < size);
  assert(
      (latest<FeatureSet>().template acc<Dimensions>()).computed[perspective]);

  const Square ksq = pos.square<KING>(perspective);

  for (std::int64_t next = std::int64_t(size) - 2; next >= std::int64_t(end);
       next--)
    update_accumulator_incremental<false>(perspective, featureTransformer, ksq,
                                          mut_accumulators<FeatureSet>()[next],
                                          accumulators<FeatureSet>()[next + 1]);

  assert((accumulators<FeatureSet>()[end].template acc<Dimensions>())
             .computed[perspective]);
}
...
template void AccumulatorStack::evaluate<TransformedFeatureDimensionsBig>(
    const Position &pos,
    const FeatureTransformer<TransformedFeatureDimensionsBig>
        &featureTransformer,
...
template void AccumulatorStack::evaluate<TransformedFeatureDimensionsSmall>(
    const Position &pos,
    const FeatureTransformer<TransformedFeatureDimensionsSmall>
        &featureTransformer,
    AccumulatorCaches::Cache<TransformedFeatureDimensionsSmall>
        &cache) noexcept;

namespace {

template <typename VectorWrapper, IndexType Width, UpdateOperation... ops,
          typename ElementType, typename... Ts,
          std::enable_if_t<is_all_same_v<ElementType, Ts...>, bool> = true>
void fused_row_reduce(const ElementType *in, ElementType *out,
                      const Ts *const... rows) {
  constexpr IndexType size =
      Width * sizeof(ElementType) / sizeof(typename VectorWrapper::type);
...
struct AccumulatorUpdateContext {
  ...
  AccumulatorUpdateContext(Color persp,
      ...
      : perspective{persp}, featureTransformer{ft}, from{accF}, to{accT} {}
  ...
  void apply(const typename FeatureSet::IndexList &added,
    ...
    for (IndexType j = 0; j < PSQTBuckets / Tiling::PsqtTileHeight; ++j) {
      auto *fromTilePsqt = reinterpret_cast<const psqt_vec_t *>(
          &fromPsqtAcc[j * Tiling::PsqtTileHeight]);
      auto *toTilePsqt = reinterpret_cast<psqt_vec_t *>(
          &toPsqtAcc[j * Tiling::PsqtTileHeight]);
...
void double_inc_update(
  ...
  assert(
      !middle_state.acc<TransformedFeatureDimensions>().computed[perspective]);
  assert(
      !target_state.acc<TransformedFeatureDimensions>().computed[perspective]);

  PSQFeatureSet::IndexList removed, added;
  PSQFeatureSet::append_changed_indices(perspective, ksq, middle_state.diff,
                                        removed, added);
  // you can't capture a piece that was just involved in castling since the rook
  // ends up in a square that the king passed
  assert(added.size() < 2);
  PSQFeatureSet::append_changed_indices(perspective, ksq, target_state.diff,
                                        removed, added);
  ...
  auto updateContext = make_accumulator_update_context(
      perspective, featureTransformer, computed, target_state);

  if (removed.size() == 2) {
    updateContext.template apply<Add, Sub, Sub>(added[0], removed[0],
                                                removed[1]);
  } else {
    updateContext.template apply<Add, Sub, Sub, Sub>(added[0], removed[0],
                                                     removed[1], removed[2]);
  }
...
void double_inc_update(
  ...
  ThreatFeatureSet::append_changed_indices(perspective, ksq, middle_state.diff,
                                           removed, added, &fusedData, true);
  ThreatFeatureSet::append_changed_indices(perspective, ksq, target_state.diff,
...
void update_accumulator_incremental(
  ...
  assert((computed.template acc<TransformedFeatureDimensions>())
             .computed[perspective]);
  assert(!(target_state.template acc<TransformedFeatureDimensions>())
              .computed[perspective]);
  ...
  if constexpr (Forward)
    FeatureSet::append_changed_indices(perspective, ksq, target_state.diff,
                                       removed, added);
  else
  ...
  if constexpr (std::is_same_v<FeatureSet, ThreatFeatureSet>)
    ...
    if ((Forward && removed.size() == 1) || (!Forward && added.size() == 1)) {
      ...
      assert(added.size() == 2);
      updateContext.template apply<Add, Add, Sub>(added[0], added[1],
                                                  removed[0]);
    } else {
      assert(added.size() == 2 && removed.size() == 2);
      updateContext.template apply<Add, Add, Sub, Sub>(added[0], added[1],
                                                       removed[0], removed[1]);
    }
  }

  (target_state.template acc<TransformedFeatureDimensions>())
...
Bitboard get_changed_pieces(const std::array<Piece, SQUARE_NB> &oldPieces,
  ...
  for (int i = 0; i < 64; i += 32) {
    const __m256i old_v =
        _mm256_loadu_si256(reinterpret_cast<const __m256i *>(&oldPieces[i]));
    const __m256i new_v =
        _mm256_loadu_si256(reinterpret_cast<const __m256i *>(&newPieces[i]));
  ...
  uint8x16x4_t old_v =
      vld4q_u8(reinterpret_cast<const uint8_t *>(oldPieces.data()));
  uint8x16x4_t new_v =
      vld4q_u8(reinterpret_cast<const uint8_t *>(newPieces.data()));
...
void update_accumulator_refresh_cache(
  ...
  using Tiling [[maybe_unused]] =
      SIMDTiling<Dimensions, Dimensions, PSQTBuckets>;

  const Square ksq = pos.square<KING>(perspective);
  auto &entry = cache[ksq][perspective];
  PSQFeatureSet::IndexList removed, added;

  const Bitboard changedBB =
      get_changed_pieces(entry.pieces, pos.piece_array());
  ...
  while (removedBB) {
    ...
    removed.push_back(
        PSQFeatureSet::make_index(perspective, sq, entry.pieces[sq], ksq));
  }
  while (addedBB) {
    Square sq = pop_lsb(addedBB);
    added.push_back(
        PSQFeatureSet::make_index(perspective, sq, pos.piece_on(sq), ksq));
  }
  ...
  for (IndexType j = 0; j < PSQTBuckets / Tiling::PsqtTileHeight; ++j) {
    ...
    auto *entryTilePsqt = reinterpret_cast<psqt_vec_t *>(
        &entry.psqtAccumulation[j * Tiling::PsqtTileHeight]);
    ...
    for (int i = 0; i < removed.ssize(); ++i) {
      ...
      const size_t offset = PSQTBuckets * index + j * Tiling::PsqtTileHeight;
      auto *columnPsqt = reinterpret_cast<const psqt_vec_t *>(
          &featureTransformer.psqtWeights[offset]);
