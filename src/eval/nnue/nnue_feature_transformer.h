...
/**
 * @file nnue_feature_transformer.h
...
void permute(std::array<T, N> &data,
             const std::array<std::size_t, OrderSize> &order) {
...
public:
  ...
  static constexpr IndexType ThreatInputDimensions =
      ThreatFeatureSet::Dimensions;
  ...
  static constexpr std::uint32_t get_hash_value() {
    return (UseThreats ? ThreatFeatureSet::HashValue
  ...
  bool read_parameters(std::istream &stream) {
    ...
    if (UseThreats) {
      read_little_endian<ThreatWeightType>(
          stream, threatWeights.data(), ThreatInputDimensions * HalfDimensions);
  ...
  bool write_parameters(std::ostream &stream) const {
    std::unique_ptr<FeatureTransformer> copy =
        std::make_unique<FeatureTransformer>(*this);
    ...
    if (UseThreats) {
      ...
      std::copy(std::begin(copy->threatPsqtWeights),
                std::begin(copy->threatPsqtWeights) +
  ...
  std::int32_t transform(const Position &pos,
    ...
    const auto &threatAccumulatorState =
        accumulatorStack.latest<ThreatFeatureSet>();
    ...
    for (IndexType p = 0; p < 2; ++p) {
...
#endif
      if (UseThreats) {
        const vec_t *tin0 = reinterpret_cast<const vec_t *>(
            &(threatAccumulation[perspectives[p]][0]));
      ...
      for (IndexType j = 0; j < HalfDimensions / 2; ++j) {
        ...
        if (UseThreats) {
          BiasType sum0t =
              threatAccumulation[static_cast<int>(perspectives[p])][j + 0];
          BiasType sum1t = threatAccumulation[static_cast<int>(perspectives[p])]
                                             [j + HalfDimensions / 2];
  ...
  alignas(CacheLineSize)
      std::array<WeightType, HalfDimensions * InputDimensions> weights;
  alignas(CacheLineSize) std::array<
      ThreatWeightType,
      UseThreats ? HalfDimensions * ThreatInputDimensions : 0> threatWeights;
  alignas(CacheLineSize)
      std::array<PSQTWeightType, InputDimensions * PSQTBuckets> psqtWeights;
  alignas(CacheLineSize) std::array<
      PSQTWeightType,
...
template <MetalFish::Eval::NNUE::IndexType TransformedFeatureDimensions>
struct std::hash<
    MetalFish::Eval::NNUE::FeatureTransformer<TransformedFeatureDimensions>> {
  std::size_t operator()(const MetalFish::Eval::NNUE::FeatureTransformer<
                         TransformedFeatureDimensions> &ft) const noexcept {
