...
using namespace MetalFish::Eval::NNUE;
...
NetworkOutput Network<Arch, Transformer>::evaluate(
    const Position &pos, AccumulatorStack &accumulatorStack,
...
void Network<Arch, Transformer>::verify(
  ...
  if (f) {
    ...
    f("NNUE evaluation using " + evalfilePath + " (" +
...
NnueEvalTrace Network<Arch, Transformer>::trace_evaluate(
  ...
  for (IndexType bucket = 0; bucket < LayerStacks; ++bucket) {
    const auto materialist = featureTransformer.transform(
        pos, accumulatorStack, cache, transformedFeatures, bucket);
    const auto positional = network[bucket].propagate(transformedFeatures);
