...
// Information on a particular pair of pieces and whether they should be
// excluded
struct PiecePairData {
  // Layout: bits 8..31 are the index contribution of this piece pair, bits 0
  // and 1 are exclusion info
  uint32_t data;

  constexpr PiecePairData() : data(0) {}

  constexpr PiecePairData(bool excluded_pair, bool semi_excluded_pair,
                          IndexType feature_index_base)
      : data((uint32_t(excluded_pair) << 1) |
             (uint32_t(semi_excluded_pair && !excluded_pair)) |
             (uint32_t(feature_index_base) << 8)) {}

  // lsb: excluded if from < to; 2nd lsb: always excluded
  constexpr uint8_t excluded_pair_info() const {
    return static_cast<uint8_t>(data);
  }

  constexpr IndexType feature_index_base() const {
    return static_cast<IndexType>(data >> 8);
  }
};
  ...
  for (Piece piece : AllPieces) {
    ...
    for (Square from = SQ_A1; from <= SQ_H8; ++from) {
      ...
      if (type_of(piece) != PAWN) {
        ...
        Bitboard attacks = (pieceIdx < 8)
                               ? pawn_attacks_bb<WHITE>(square_bb(from))
...
constexpr auto init_index_luts() {
  ...
  for (Piece attacker : AllPieces) {
    for (Piece attacked : AllPieces) {
      ...
      bool excluded = map < 0;
      indices[attacker][attacked] =
          PiecePairData(excluded, semi_excluded, feature);
    }
  }
...
FullThreats::make_index(Color perspective, Piece attacker, Square from,
  ...
  const IndexType index =
      piecePairData.feature_index_base() +
