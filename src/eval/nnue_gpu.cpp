...
NNUEBuffers::~NNUEBuffers() { release(); }
...
NNUEEvaluator::NNUEEvaluator() : buffers_(std::make_unique<NNUEBuffers>()) {}
...
bool NNUEEvaluator::init() {
  try {
    ...
    std::cout << "[NNUE] GPU acceleration initialized successfully"
              << std::endl;
    return true;
...
bool NNUEEvaluator::load_network(const std::string &path) {
  ...
  if (buffers_->fc0_weights) {
    memset(buffers_->fc0_weights->contents(), 0,
           buffers_->fc0_weights->length());
  }
...
void NNUEEvaluator::get_active_features(const Position &pos, Color perspective,
  ...
  // Each feature is encoded as: king_square * 641 + piece_square * 10 +
  // piece_type
  ...
  for (Color c : {WHITE, BLACK}) {
    for (PieceType pt = PAWN; pt <= QUEEN; ++pt) {
      ...
      while (bb) {
        ...
        int feature =
            int(oriented_ksq) * 641 + int(oriented_sq) * 12 + piece_idx;
...
void NNUEEvaluator::evaluate_batch(
  ...
  for (const Position *pos : positions) {
    ...
    // Combine features
    for (int f : white_features)
      all_features.push_back(f);
    for (int f : black_features)
      all_features.push_back(f);
  ...
  memcpy(buffers_->active_features->contents(), all_features.data(),
         all_features.size() * sizeof(int));
  memcpy(buffers_->feature_counts->contents(), feature_counts.data(),
