...
int feature_index(Square ksq, Square psq, Piece pc, Color perspective) {
    ...
    if (pcIndex < 0 || pcIndex > 11) return -1;  // Skip kings
...
Network::~Network() {
    // Destructor - Metal resources will be freed automatically by Buffer destructor
    if (featureTransformKernel) featureTransformKernel->release();
    if (affineTransformKernel) affineTransformKernel->release();
    if (clippedReluKernel) clippedReluKernel->release();
}
