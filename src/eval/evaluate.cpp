...
int psq_value(Piece pc, Square s) {
    ...
    switch (pt) {
        case PAWN:   return PawnTable[rs];
        case KNIGHT: return KnightTable[rs];
        case BISHOP: return BishopTable[rs];
        case ROOK:   return RookTable[rs];
        case QUEEN:  return QueenTable[rs];
        case KING:   return KingMiddleGameTable[rs];
        default:     return 0;
    }
}
...
bool is_network_loaded() {
    return NNUE::network && NNUE::network->is_loaded();
}
