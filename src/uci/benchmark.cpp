...
// bench 64 1 15                    : search default positions up to depth 15
// (TT = 64MB) bench 64 1 100000 default nodes  : search default positions for
// 100K nodes each bench 64 4 5000 current movetime : search current position
// with 4 threads for 5 sec bench 16 1 5 blah perft          : run a perft 5 on
// positions in file "blah"
std::vector<std::string> setup_bench(const std::string &currentFen,
                                     std::istream &is) {
...
BenchmarkSetup setup_benchmark(std::istream &is) {
  ...
  for (const auto &game : BenchmarkPositions) {
    ...
    for (const std::string &fen : game) {
      ...
      const int correctedTime =
          static_cast<int>(getCorrectedTime(ply) * timeScaleFactor);
      setup.commands.emplace_back("go movetime " +
                                  std::to_string(correctedTime));
