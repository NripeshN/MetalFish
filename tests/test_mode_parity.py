...
NN_WEIGHTS = os.path.join(
    os.path.dirname(__file__), "..", "networks", "BT4-1024x15x32h-swa-6147500.pb.gz"
)
...
POSITIONS = [
    ...
    (
        "QGD setup",
        "r1bq1rk1/ppp1bppp/2n2n2/3pp3/2PP4/2N1PN2/PP2BPPP/R1BQ1RK1 w - - 0 8",
    ),
    (
        "Blunder trap (avoid Rb4)",
        "2kr4/1q1r1p2/4p3/p3P3/2Pn1PP1/2R5/1Q3B1P/5BK1 w - - 4 34",
    ),
    (
        "Middlegame",
        "r2qk2r/ppp2ppp/2n1bn2/2b1p3/4P3/2NP1N2/PPP2PPP/R1BQKB1R w KQkq - 0 6",
    ),
...
def main():
    ...
    for pos_idx, (pos_name, fen) in enumerate(POSITIONS):
        ...
        if ab[0] != mcts[0]:
            ...
            if gap is not None and gap > 100:
                msg = (
                    f"  ** DISAGREEMENT: AB={ab[0]} ({ab[1]}) vs "
                ...
                print(
                    f"  (minor disagreement: AB={ab[0]} vs MCTS={mcts[0]}, "
    ...
    print()
    print(
        f"{'Position':<30s} {'AB':<10s} {'MCTS':<10s} {'Hybrid':<10s} {'AB Score':<12s} {'MCTS Score':<12s} {'Hybrid Score':<12s}"
    )
    print("-" * 96)
    for pos_name, fen, pos_results in results:
        ...
        hybrid = pos_results.get("Hybrid", ("N/A", "N/A", None))
        print(
            f"{pos_name:<30s} {ab[0]:<10s} {mcts[0]:<10s} {hybrid[0]:<10s} {ab[1]:<12s} {mcts[1]:<12s} {hybrid[1]:<12s}"
        )

    print()
    if disagreements:
        print(
            f"FLAGGED DISAGREEMENTS (>100cp gap between AB and MCTS): {len(disagreements)}"
        )
