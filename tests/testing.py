...
def test_uci_protocol():
    ...
    def test_hybrid_init_failure_falls_back_to_ab():
        """Hybrid transformer init failure should still return bestmove."""
        import subprocess

        missing_weights = BUILD_PATH / "__missing_transformer.pb"
        result = subprocess.run(
            [str(METALFISH_BIN)],
            input=(
                "uci\n"
                ...
                f"setoption name NNWeights value {missing_weights}\n"
                "isready\n"
                "position startpos\n"
                "go depth 1\n"
                "quit\n"
        ...
        assert "Falling back to Alpha-Beta search" in output, output[-1000:]
