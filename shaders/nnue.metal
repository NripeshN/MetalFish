/*
  ...
  - Input: 45056 features (HalfKAv2_hm) -> 1024 hidden units (big) or 128 (small)
...
kernel void feature_transform(
    ...
    device const int32_t* active_features [[buffer(1)]],  // Active feature indices
    device const int32_t* feature_counts [[buffer(2)]],   // Number of features per position
    device const weight_t* biases [[buffer(3)]],          // [hidden_dim]
    device acc_t* output [[buffer(4)]],                    // [batch_size x hidden_dim]
...
kernel void incremental_update(
    ...
    device const int32_t* update_counts [[buffer(3)]],  // [num_added, num_removed] per position
    ...
    uint2 tid [[thread_position_in_grid]])
{
    ...
    for (int i = 0; i < num_removed; i++) {
        int feature_idx = removed_features[pos_idx * 32 + i]; // Max 32 features per update
        if (feature_idx >= 0) {
            acc -= weights[feature_idx * hidden_dim + hidden_idx];
        }
    }
...
kernel void fc0_sparse(
    device const acc_t* input [[buffer(0)]],           // Transformed features [batch x hidden_dim]
    ...
    uint tg_idx [[threadgroup_position_in_grid]])
{
    ...
    if (pos_idx >= batch_size || out_idx >= FC0_OUT + 1) return;
    ...
    uint2 tid [[thread_position_in_grid]])
{
    ...
    if (pos_idx >= batch_size || out_idx >= output_dim) return;
    ...
    if (apply_relu) {
        output[pos_idx * output_dim + out_idx] = clipped_relu(int16_t(acc >> WEIGHT_SCALE_BITS));
    } else {
 ...
 */
kernel void output_layer(
    device const int8_t* fc1_output [[buffer(0)]],
    device const weight_t* weights [[buffer(1)]],
    device const weight_t* biases [[buffer(2)]],
    device const int16_t* fc0_skip [[buffer(3)]],  // Skip connection from FC0
    ...
    uint tid [[thread_position_in_grid]])
{
    ...
    int32_t fwd_out = (fc0_skip[pos_idx] * (600 * OUTPUT_SCALE)) / (127 * (1 << WEIGHT_SCALE_BITS));
...
kernel void nnue_forward_batch(
    device const acc_t* transformed [[buffer(0)]],         // Pre-computed accumulators [batch x 2 x hidden_dim]
...
kernel void psqt_eval(
    device const int32_t* active_features [[buffer(0)]],
    device const int32_t* feature_counts [[buffer(1)]],
    device const int16_t* psqt_weights [[buffer(2)]],  // [num_features x buckets]
    ...
    uint2 tid [[thread_position_in_grid]])
{
    ...
    if (pos_idx >= batch_size || bucket_idx >= num_buckets) return;
