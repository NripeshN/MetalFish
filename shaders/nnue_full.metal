...
// ============================================================================
// Constants matching MetalFish's NNUE exactly
// ============================================================================
...
// ============================================================================
// This is the critical kernel: transforms active piece features to hidden
// layer. Uses incremental updates when possible for efficiency.

kernel void feature_transformer_full(
    device const ft_weight_t *weights
    [[buffer(0)]],                                // [FT_IN_DIMS x FT_OUT_DIMS]
    device const ft_bias_t *biases [[buffer(1)]], // [FT_OUT_DIMS]
    device const int *active_features [[buffer(2)]], // Active feature indices
    device const int *feature_counts
    [[buffer(3)]], // Cumulative count per position
  ...
  if (pos_idx >= batch_size || hidden_idx >= ft_out_dims)
    return;
...
kernel void
feature_transformer_update(device const ft_weight_t *weights [[buffer(0)]],
...
kernel void
fc0_layer(device const ft_acc_t *accumulator
          [[buffer(0)]], // [2 x FT_OUT_DIMS] (white/black)
          device const fc_weight_t *weights
          [[buffer(1)]], // [FT_OUT_DIMS x FC0_OUTPUTS]
          device const int32_t *biases [[buffer(2)]], // [FC0_OUTPUTS]
          device int8_t *output_sqr
          [[buffer(3)]], // SqrClippedReLU output [FC0_OUTPUTS-1]
          device int8_t *output_clip
          [[buffer(4)]], // ClippedReLU output [FC0_OUTPUTS-1]
  ...
  int out_idx = gid;
  if (out_idx >= FC0_OUTPUTS)
    return;

  // Get accumulators (perspective based on side to move)
  device const ft_acc_t *acc_us =
      stm == 0 ? accumulator : accumulator + ft_out_dims;
  device const ft_acc_t *acc_them =
      stm == 0 ? accumulator + ft_out_dims : accumulator;
...
kernel void fc0_layer_optimized(device const ft_acc_t *accumulator
                                [[buffer(0)]],
  ...
  for (int i = simd_lane; i < ft_out_dims; i += SIMD_SIZE) {
    ...
    partial_sum +=
        clipped_them * weights[(ft_out_dims + i) * FC0_OUTPUTS + out_idx];
  }
...
kernel void fc1_layer(device const int8_t *input [[buffer(0)]], // [FC1_INPUTS]
                      device const fc_weight_t *weights
                      [[buffer(1)]], // [FC1_INPUTS x FC1_OUTPUTS]
                      device const int32_t *biases
                      [[buffer(2)]],                       // [FC1_OUTPUTS]
                      device int8_t *output [[buffer(3)]], // [FC1_OUTPUTS]
                      uint gid [[thread_position_in_grid]],
                      uint simd_lane [[thread_index_in_simdgroup]]) {
  int out_idx = gid;
  if (out_idx >= FC1_OUTPUTS)
    return;
...
kernel void fc1_layer_optimized(device const int8_t *input [[buffer(0)]],
                                device const fc_weight_t *weights [[buffer(1)]],
                                device const int32_t *biases [[buffer(2)]],
...
kernel void fc2_layer(device const int8_t *input [[buffer(0)]], // [FC1_OUTPUTS]
                      device const fc_weight_t *weights
                      [[buffer(1)]],                            // [FC1_OUTPUTS]
                      device const int32_t *bias [[buffer(2)]], // [1]
                      device const int16_t *skip_input
                      [[buffer(3)]],                        // Skip from FC0
  ...
  if (simd_lane == 0) {
    ...
    int32_t skip =
        (*skip_input) * (600 * OUTPUT_SCALE) / (127 * (1 << WEIGHT_SCALE_BITS));
...
kernel void nnue_forward_fused(
    ...
    constant int &ft_out_dims [[buffer(8)]], constant int &stm [[buffer(9)]],
  ...
  if (lid < FC0_OUTPUTS) {
    ...
    for (int i = 0; i < ft_out_dims; i++) {
      ...
      sum +=
          clipped_them * fc0_weights[(ft_out_dims + i) * FC0_OUTPUTS + out_idx];
    }
  ...
  // Prepare FC1 input: [sqr_us, sqr_them, clip_us, clip_them] -> just [sqr_us,
  // sqr_them] In MetalFish, FC1 input is the concatenation of SqrClippedReLU
  // outputs
  ...
  if (lid == 0) {
    ...
    // Add skip connection
    int32_t skip =
        skip_value * (600 * OUTPUT_SCALE) / (127 * (1 << WEIGHT_SCALE_BITS));
...
kernel void nnue_batch_eval(device const ft_acc_t *accumulators
                            [[buffer(0)]], // [batch x 2 x FT_OUT_DIMS]
                            device const fc_weight_t *fc0_weights [[buffer(1)]],
                            device const int32_t *fc0_biases [[buffer(2)]],
                            device const fc_weight_t *fc1_weights [[buffer(3)]],
                            device const int32_t *fc1_biases [[buffer(4)]],
  ...
  if (lid < FC0_OUTPUTS) {
    ...
    for (int i = 0; i < ft_out_dims; i++) {
      ...
      sum += clipped_relu(acc_them[i]) *
             fc0_weights[(ft_out_dims + i) * FC0_OUTPUTS + out_idx];
    }
...
kernel void psqt_accumulate(device const int *active_features [[buffer(0)]],
                            device const int *feature_counts [[buffer(1)]],
                            device const int16_t *psqt_weights
                            [[buffer(2)]], // [FT_IN_DIMS x PSQT_BUCKETS]
                            device int32_t *psqt_output
                            [[buffer(3)]], // [batch x PSQT_BUCKETS]
