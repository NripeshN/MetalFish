...
EN_PASSANT_FEN="rnbqkbnr/ppp1pppp/8/3pP3/8/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2"
...
write_summary() {
  ...
  mkdir -p "$(dirname "${SUMMARY}")"
  {
    ...
    echo "- after-e4 CUDA search smoke: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-after-e4-smoke.log")"
    echo "- after-e4 CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-after-e4-search.json")"
    echo "- promotion CUDA search smoke: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-promotion-smoke.log")"
    echo "- promotion CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-promotion-search.json")"
    echo "- en-passant CUDA search smoke: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-en-passant-smoke.log")"
    echo "- en-passant CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-en-passant-search.json")"
    ...
    echo "- hybrid after-e4 CUDA UCI smoke: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-hybrid-after-e4-smoke.log")"
    echo "- hybrid after-e4 CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-hybrid-after-e4-search.json")"
    echo "- hybrid promotion CUDA UCI smoke: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-hybrid-promotion-smoke.log")"
    echo "- hybrid promotion CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-hybrid-promotion-search.json")"
    echo "- hybrid en-passant CUDA UCI smoke: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-hybrid-en-passant-smoke.log")"
    echo "- hybrid en-passant CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-uci-hybrid-en-passant-search.json")"
    ...
    echo "- packaged after-e4 CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-package-uci-after-e4-search.json")"
    echo "- packaged promotion CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-package-uci-promotion-search.json")"
    echo "- packaged en-passant CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-package-uci-en-passant-search.json")"
    ...
    echo "- packaged hybrid after-e4 CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-after-e4-search.json")"
    echo "- packaged hybrid promotion CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-promotion-search.json")"
    echo "- packaged hybrid en-passant CUDA search JSON: $(summary_log_status "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-en-passant-search.json")"
    ...
    summary_failure_lines "after-e4 CUDA search smoke" \
      "${BUILD_DIR}/cuda-gpu-uci-after-e4-smoke.log"
    summary_failure_lines "promotion CUDA search smoke" \
      "${BUILD_DIR}/cuda-gpu-uci-promotion-smoke.log"
    summary_failure_lines "en-passant CUDA search smoke" \
      "${BUILD_DIR}/cuda-gpu-uci-en-passant-smoke.log"
    summary_failure_lines "hybrid CUDA UCI smoke" \
      "${BUILD_DIR}/cuda-gpu-uci-hybrid-smoke.log"
    summary_failure_lines "hybrid after-e4 CUDA UCI smoke" \
      "${BUILD_DIR}/cuda-gpu-uci-hybrid-after-e4-smoke.log"
    summary_failure_lines "hybrid promotion CUDA UCI smoke" \
      "${BUILD_DIR}/cuda-gpu-uci-hybrid-promotion-smoke.log"
    summary_failure_lines "hybrid en-passant CUDA UCI smoke" \
      "${BUILD_DIR}/cuda-gpu-uci-hybrid-en-passant-smoke.log"
    ...
    echo "- cuda-after-e4: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-uci-after-e4-smoke.log" "not reached")"
    echo "- cuda-promotion: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-uci-promotion-smoke.log" "not reached")"
    echo "- cuda-en-passant: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-uci-en-passant-smoke.log" "not reached")"
    ...
    echo "- hybrid-after-e4: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-uci-hybrid-after-e4-smoke.log" "not reached")"
    echo "- hybrid-promotion: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-uci-hybrid-promotion-smoke.log" "not reached")"
    echo "- hybrid-en-passant: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-uci-hybrid-en-passant-smoke.log" "not reached")"
    ...
    echo "- packaged-cuda-after-e4: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-package-uci-after-e4-smoke.log" "not reached")"
    echo "- packaged-cuda-promotion: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-package-uci-promotion-smoke.log" "not reached")"
    echo "- packaged-cuda-en-passant: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-package-uci-en-passant-smoke.log" "not reached")"
    ...
    echo "- packaged-hybrid-after-e4: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-after-e4-smoke.log" "not reached")"
    echo "- packaged-hybrid-promotion: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-promotion-smoke.log" "not reached")"
    echo "- packaged-hybrid-en-passant: $(summary_line_or_missing '^bestmove ' "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-en-passant-smoke.log" "not reached")"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --go "nodes 1" \
  --json-out "${BUILD_DIR}/cuda-gpu-uci-after-e4-search.json" \
  --expect-output "CUDA transformer backend" \
  --expect-output "MCTS runtime: backend=cuda" \
  --expect-output "minibatch=1" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-uci-after-e4-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --go "nodes 1" \
  --json-out "${BUILD_DIR}/cuda-gpu-uci-promotion-search.json" \
  --expect-output "CUDA transformer backend" \
  --expect-output "MCTS runtime: backend=cuda" \
  --expect-output "minibatch=1" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-uci-promotion-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --setoption MCTSAddDirichletNoise=false \
  --setoption TransformerLowTimeFallbackMs=0 \
  --position "fen ${EN_PASSANT_FEN}" \
  --go "nodes 1" \
  --json-out "${BUILD_DIR}/cuda-gpu-uci-en-passant-search.json" \
  --expect-output "CUDA transformer backend" \
  --expect-output "MCTS runtime: backend=cuda" \
  --expect-output "minibatch=1" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-uci-en-passant-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --expect-output "CUDA transformer backend" \
  --expect-output "Final: MCTSPlayouts=" \
  --json-out "${BUILD_DIR}/cuda-gpu-uci-hybrid-after-e4-search.json" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-uci-hybrid-after-e4-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --expect-output "CUDA transformer backend" \
  --expect-output "Final: MCTSPlayouts=" \
  --json-out "${BUILD_DIR}/cuda-gpu-uci-hybrid-promotion-search.json" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-uci-hybrid-promotion-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --expect-output "CUDA transformer backend" \
  --expect-output "Final: MCTSPlayouts=" \
  --json-out "${BUILD_DIR}/cuda-gpu-uci-hybrid-en-passant-search.json" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-uci-hybrid-en-passant-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --position "fen ${AFTER_E4_FEN}" \
  --go "nodes 1" \
  --json-out "${BUILD_DIR}/cuda-gpu-package-uci-after-e4-search.json" \
  --expect-output "CUDA transformer backend" \
  --expect-output "MCTS runtime: backend=cuda" \
  --expect-output "minibatch=1" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-package-uci-after-e4-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --position "fen ${PROMOTION_FEN}" \
  --go "nodes 1" \
  --json-out "${BUILD_DIR}/cuda-gpu-package-uci-promotion-search.json" \
  --expect-output "CUDA transformer backend" \
  --expect-output "MCTS runtime: backend=cuda" \
  --expect-output "minibatch=1" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-package-uci-promotion-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --position "fen ${EN_PASSANT_FEN}" \
  --go "nodes 1" \
  --json-out "${BUILD_DIR}/cuda-gpu-package-uci-en-passant-search.json" \
  --expect-output "CUDA transformer backend" \
  --expect-output "MCTS runtime: backend=cuda" \
  --expect-output "minibatch=1" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-package-uci-en-passant-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --expect-output "CUDA transformer backend" \
  --expect-output "Final: MCTSPlayouts=" \
  --json-out "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-after-e4-search.json" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-after-e4-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --expect-output "CUDA transformer backend" \
  --expect-output "Final: MCTSPlayouts=" \
  --json-out "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-promotion-search.json" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-promotion-smoke.log"
...
METALFISH_CUDA_PROFILE=0 \
  ...
  --expect-output "CUDA transformer backend" \
  --expect-output "Final: MCTSPlayouts=" \
  --json-out "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-en-passant-search.json" \
  "${UCI_CUDA_RUNTIME_EXPECT_ARGS[@]}" \
  "${UCI_CUDA_MCTS_WARMUP_EXPECT_ARGS[@]}" \
  | tee "${BUILD_DIR}/cuda-gpu-package-uci-hybrid-en-passant-smoke.log"
