...
write_summary() {
  ...
  mkdir -p "$(dirname "${SUMMARY}")"
  {
    ...
    echo "- CUDA dynamic PE sparse dense: ${METALFISH_CUDA_DYNAMIC_PE_SPARSE_DENSE:-1}"
...
METALFISH_NN_WEIGHTS="${WEIGHTS}" \
  ...
  METALFISH_CUDA_DYNAMIC_PE_SPARSE_DENSE="${METALFISH_CUDA_DYNAMIC_PE_SPARSE_DENSE:-1}" \
  METALFISH_CUDA_PROFILE=0 \
