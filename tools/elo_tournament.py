...
class EngineConfig:
    ...
    def to_cutechess_args(self) -> List[str]:
        ...
        args = ["-engine", f"cmd={self.cmd}", f"name={self.name}", f"proto={self.proto}"]
...
class EloCalculator:
        ...
        for iteration in range(100):
            ...
            for engine in engines:
                ...
                if total_games > 0:
                    # Adjust rating
                    k = 32  # K-factor
                    adjustment = k * (total_actual - total_expected) / (total_games / 10)
    ...
    def get_confidence_interval(self, wins: int, draws: int, losses: int, 
...
class Tournament:
    ...
    def __init__(self, base_dir: Path):
        ...
        self.cutechess = base_dir / "reference" / "cutechess" / "build" / "cutechess-cli"
...
'''
    ...
    def run_match(self, engine1: EngineConfig, engine2: EngineConfig, 
                  games: int, time_control: str, concurrency: int = 1) -> List[GameResult]:
        ...
        cmd.extend([
            ...
            "-rounds", str(games // 2),  # Each round is 2 games (alternating colors)
            "-games", "2",  # 2 games per round (one as white, one as black)
            "-pgnout", pgn_file.name,
            "-concurrency", str(concurrency),
            "-recover",
            "-repeat"
        ])
        
        print(f"  Running: {engine1.name} vs {engine2.name} ({games} games)...", end=" ", flush=True)
        ...
        if score_match:
            ...
            print(f"{Colors.GREEN}Done{Colors.RESET} ({wins1}-{wins2}-{draws}, {score_pct:.1f}%)")
        else:
    ...
    def run_round_robin(self, games_per_pair: int, time_control: str, 
        ...
        print(f"  Total games: {len(self.engines) * (len(self.engines) - 1) * games_per_pair // 2}")
        ...
        for i, engine1 in enumerate(self.engines):
            for engine2 in self.engines[i+1:]:
                results = self.run_match(engine1, engine2, games_per_pair, 
                                         time_control, concurrency)
    ...
    def print_results(self, ratings: Dict[str, float]):
        ...
        for rank, (engine, elo) in enumerate(sorted_engines, 1):
            ...
            print(f"  {rank:<6} {color}{engine:<25}{Colors.RESET} {elo:>8.0f} {expected:>10}")
        ...
        for key, match in sorted(self.elo_calc.results.items()):
            ...
            print(f"    Score: {match.wins1}-{match.draws}-{match.wins2} "
...
def get_engine_configs(base_dir: Path, stockfish_levels: List[int] = None) -> Dict[str, EngineConfig]:
...
def run_ci_match(base_dir: Path, engine1_name: str, engine2_name: str,
                 games: int, time_control: str, output_file: Path) -> Dict[str, Any]:
...
def generate_pr_comment(summary: Dict[str, Any]) -> str:
    ...
    for match in summary["matches"]:
        total = match["total"]
        score_pct = (match["wins1"] + match["draws"] * 0.5) / total * 100 if total > 0 else 50
        lines.append(
...
def main():
    parser = argparse.ArgumentParser(
        description="Run a chess engine Elo tournament"
    )
    ...
    parser.add_argument(
        "--time", "-t", type=str, default="10+0.1",
    ...
    parser.add_argument(
        "--stockfish-levels", "-s", type=str, default="1,5,10,15,20",
    ...
    parser.add_argument(
        "--metalfish-only", "-m", action="store_true",
        help="Only test MetalFish variants (AB vs MCTS)"
    )
    
    # CI mode arguments
    parser.add_argument(
        "--ci-match", action="store_true",
        help="CI mode: run a single match between two engines"
    )
    parser.add_argument(
        "--engine1", type=str,
        help="First engine name for CI match"
    )
    parser.add_argument(
        "--engine2", type=str,
        help="Second engine name for CI match"
    )
    parser.add_argument(
        "--output", "-o", type=str,
        help="Output file for CI match results (JSON)"
    )
    parser.add_argument(
        "--ci-aggregate", action="store_true",
        help="CI mode: aggregate results from multiple match jobs"
    )
    parser.add_argument(
        "--results-dir", type=str,
        help="Directory containing match result JSON files"
    )
    parser.add_argument(
        "--ci-list-engines", action="store_true",
        help="CI mode: list available engines and pairs as JSON"
    )
    parser.add_argument(
        "--ci-comment", action="store_true",
    ...
    if args.ci_match:
        if not args.engine1 or not args.engine2:
            print("Error: --engine1 and --engine2 required for --ci-match", file=sys.stderr)
            sys.exit(1)
        
        output_file = Path(args.output) if args.output else Path(f"results/{args.engine1}_vs_{args.engine2}.json")
        ...
        try:
            run_ci_match(base_dir, args.engine1, args.engine2, args.games, args.time, output_file)
    ...
    if not tournament.cutechess.exists():
        print(f"{Colors.RED}Error: cutechess-cli not found at {tournament.cutechess}{Colors.RESET}")
        sys.exit(1)
    
    if args.metalfish_only:
        # Just test MetalFish AB vs MCTS
        metalfish_path = base_dir / "build" / "metalfish"
        tournament.add_engine(EngineConfig(
            name="MetalFish-AB",
            cmd=str(metalfish_path),
            options={"Threads": "1", "Hash": "128"}
        ))
