...
CUDA_GRAPH="${METALFISH_WINDOWS_CUDA_GRAPH:-${METALFISH_CUDA_GRAPH:-${METALFISH_CUDA_GRAPH_EXECUTION:-}}}"
if [[ "${METALFISH_WINDOWS_CUDA_GRAPH:-}" == "0" ||
      "${METALFISH_CUDA_GRAPH:-}" == "0" ||
...
write_runtime_manifest() {
  ...
  RUNTIME_STATUS_FOR_MANIFEST="$1" \
    ...
    python3 tools/cuda_runtime_manifest_writer.py \
      --runtime-kind windows-cuda \
      --manifest "${ARTIFACT_DIR}/windows-cuda-runtime-gate-manifest.json"
}
...
Invoke-UciSmoke -Name "cuda-after-e4-mcts" -Commands @(
  "uci",
  "isready",
  "setoption name EvalFile value \$NnueBig",
  "setoption name EvalFileSmall value \$NnueSmall",
...
Invoke-UciSmoke -Name "hybrid-cuda-after-e4" -Commands @(
  "uci",
  "isready",
  "setoption name EvalFile value \$NnueBig",
  "setoption name EvalFileSmall value \$NnueSmall",
...
\$Manifest = [ordered]@{
  ...
  uci_smokes = [ordered]@{
    ...
    cuda_after_e4_mcts = [ordered]@{
      go = "nodes 1"
      fen = \$AfterE4Fen
      bestmove = (Find-BestMove \$AfterE4MctsText)
      backend_selected = (Test-BackendSelected \$AfterE4MctsText)
    ...
    hybrid_cuda_after_e4 = [ordered]@{
      go = "nodes 50"
      fen = \$AfterE4Fen
      bestmove = (Find-BestMove \$HybridAfterE4Text)
      backend_selected = (Test-BackendSelected \$HybridAfterE4Text)
