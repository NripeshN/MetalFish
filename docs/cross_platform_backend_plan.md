...
| CUDA GPU runtime gate | `tools/run_gcp_cuda_gpu_gate.sh` | Direct runtime pass `7d9e8ebf`, `metalfish-cuda-direct-linux-7d9e8ebf`, L4 `g2-standard-8` in `us-east1-c`, 2026-05-27; validates CUDA graph-replay MCTS warmup, extracted Linux CUDA package smoke and JSON package manifest, packaged `test_nn_comparison` batch/reuse/graph checks, packaged BT4/legacy full-policy probe suites and isolation probes, runtime manifest, BK.07 `h5f6`, Hybrid clock-safety smokes, same-commit Metal BT4/legacy probe-suite comparison from Metal CI `26513673361`, and same-commit Metal-vs-CUDA batch timing from `metal-nn-comparison.log`. Metal compare deltas: BT4 `value=4.77e-07`, `wdl=4.77e-07`, `moves_left=4.6e-05`, `top_logit=1.9432e-05`, `policy_max=3.7163e-05`; legacy `value=1.013e-06`, `top_logit=7.6e-06`, `policy_max=1.43e-05`. Batch timing compare: best common batch `b16`, CUDA `2.9077 ms/eval`, Metal `11.4166 ms/eval`, `3.926x` speedup; CUDA best batch `b16` |
...
| GitHub Windows CUDA compile gate | `.github/workflows/windows-cuda-compile.yml` | Latest branch pass `26513673537`; installs CUDA through the repo-owned NVIDIA network-installer script, then produces a self-smoked `metalfish-windows-x86_64-msvc-cuda` package artifact with `metalfish.exe`, `metalfish_nn_probe.exe`, `test_nn_comparison.exe`, and structured compile/package manifests. Exact Windows runtime input pass: `26513673537` |
| GitHub Windows CUDA runtime gate | `.github/workflows/windows-cuda-runtime-gate.yml` | Requires `windows_cuda_run_id`; `tools/fetch_windows_cuda_runtime_inputs.py` validates the same-commit Windows CUDA compile run, downloads and validates the package JSON manifest, and, when `require_metal_compare=true`, fetches same-commit Metal artifacts plus `metal-nn-comparison.log` before creating the Windows L4 VM; `stable_batch_size` mirrors the Linux CUDA GPU gate and feeds every packaged probe/MCTS/Hybrid CUDA smoke; direct runtime pass `7d9e8ebf`, `metalfish-cuda-direct-win-7d9e8ebf`, Windows Server 2022 `g2-standard-8` L4 vWS in `us-east1-b`, 2026-05-27, compile run `26513673537`, Metal run `26513673361`, event-driven UCI `bestmove` wait, graph-replay assertions, packaged JSON manifest validation, packaged `test_nn_comparison` batch/reuse smoke, BK.07 `h5f6`, Hybrid clock-safety smokes, Metal comparison, and runtime manifest. Metal compare deltas: BT4 `value=4.77e-07`, `wdl=4.77e-07`, `moves_left=4.6e-05`, `top_logit=1.9432e-05`, `policy_max=3.7163e-05`; legacy `value=1.013e-06`, `top_logit=7.6e-06`, `policy_max=1.43e-05`. Batch timing compare: best common batch `b16`, CUDA `3.1111 ms/eval`, Metal `11.4166 ms/eval`, `3.670x` speedup; CUDA best batch `b32` at `3.0502 ms/eval` |
| GitHub macOS Metal | `.github/workflows/ci.yml` | `26513673361`, Metal NN parity artifact, benchmark artifact, and BK.07 smoke |
| GitHub portable Linux/Windows CPU | `.github/workflows/portable-ci.yml` | `26513673542` |
| GitHub hybrid regression | `.github/workflows/hybrid-regression.yml` | `26513673534` |
...
The 2026-05-27 branch-tip direct pass `fa0f3690` revalidated the same contract
on both CUDA runtime targets from one detached checkout. Linux ran
`metalfish-cuda-direct-linux-fa0f3690` on `g2-standard-8` L4 in `us-east1-b`;
Windows ran `metalfish-cuda-direct-win-fa0f3690` on Windows Server 2022
`g2-standard-8` L4 vWS in `us-east1-b` after central and west-zone stockouts.
The run used Metal CI `26502861162` and Windows CUDA compile gate
`26502861163`, wrote
...
legacy compare status `0`, and final compare status `0`. Both operating
systems matched the same macOS Metal probe-suite references with BT4 maximum
deltas of `4.77e-07` value, `4.77e-07` WDL, `4.6e-05` moves-left,
`1.9432e-05` top logit, and `3.7163e-05` policy; legacy maximum deltas were
`1.013e-06` value, `7.6e-06` top logit, and `1.43e-05` policy. Direct release
promotion from `results/cuda_runtime_direct/fa0f3690` accepted both packages
for `v0.1.0-alpha`; the Linux and Windows portable package manifests both
recorded `source_commit=fa0f36903a40b8d314b71f7d71a9990d8503fec2`.
