...
| CUDA GPU runtime gate | `tools/run_gcp_cuda_gpu_gate.sh` | Direct runtime pass `9d9ca3a`, `metalfish-cuda-direct-linux-9d9ca3a4`, L4 `g2-standard-8` in `us-central1-a`, 2026-05-29 UTC; validates CUDA graph-replay MCTS warmup, extracted Linux CUDA package smoke, JSON package manifest, Linux package-check JSON evidence, packaged `test_nn_comparison` batch/reuse/graph checks, packaged BT4/legacy full-policy probe suites and isolation probes, timed pure-MCTS CUDA smoke, source and packaged pure-MCTS CUDA ponder lifecycle smokes, packaged BK.07/kiwipete MCTS and Hybrid search JSON, runtime manifest, BK.07 `h5f6`, Hybrid clock-start and clock-safety smokes, same-commit Metal BT4/legacy probe-suite comparison from Metal CI `26623653985`, same-commit Metal-vs-CUDA batch timing from `metal-nn-comparison.log`, and same-SHA Metal-vs-CUDA UCI search JSON comparison using packaged search results. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Stable-batch timing compare: `b16`, CUDA `3.2180 ms/eval`, Metal `11.8450 ms/eval`, `3.681x` speedup; fastest CUDA batch was `b32` at `3.1688 ms/eval`. Search compare: CUDA MCTS BK.07 `h5f6` matches Metal MCTS `h5f6`, CUDA MCTS kiwipete `e2a6` matches Metal MCTS `e2a6`, CUDA Hybrid BK.07 `a3b4` matches Metal Hybrid `a3b4`, and CUDA Hybrid kiwipete `c5b6` matches Metal Hybrid `c5b6` |
...
| GitHub Windows CUDA compile gate | `.github/workflows/windows-cuda-compile.yml` | Latest branch pass `26623653948`; installs CUDA through the repo-owned NVIDIA network-installer script, then produces a self-smoked `metalfish-windows-x86_64-msvc-cuda` package artifact with `metalfish.exe`, `metalfish_nn_probe.exe`, `test_nn_comparison.exe`, and structured compile/package manifests. Last exact Windows runtime input pass: `26623653948` |
| GitHub Windows CUDA runtime gate | `.github/workflows/windows-cuda-runtime-gate.yml` | Requires `windows_cuda_run_id`; `tools/fetch_windows_cuda_runtime_inputs.py` validates the same-commit Windows CUDA compile run, downloads and validates the package JSON manifest, and, when `require_metal_compare=true`, fetches same-commit Metal artifacts plus `metal-nn-comparison.log` before creating the Windows L4 VM; `stable_batch_size` mirrors the Linux CUDA GPU gate and feeds every packaged probe/MCTS/Hybrid CUDA smoke; direct runtime pass `9d9ca3a`, `metalfish-cuda-direct-win-9d9ca3a4`, Windows Server 2022 `g2-standard-8` L4 vWS in `us-central1-a`, 2026-05-29 UTC, compile run `26623653948`, Metal run `26623653985`, event-driven UCI `bestmove` wait, graph-replay assertions, packaged JSON manifest validation, packaged `test_nn_comparison` batch/reuse smoke, timed pure-MCTS CUDA smoke, packaged pure-MCTS CUDA ponder lifecycle smoke, BK.07 `h5f6`, Hybrid clock-start and clock-safety smokes, Metal comparison, runtime manifest, and same-SHA Metal-vs-CUDA UCI search JSON comparison. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Stable-batch timing compare: `b16`, CUDA `3.1599 ms/eval`, Metal `11.8450 ms/eval`, `3.749x` speedup; fastest CUDA batch was `b32` at `3.0343 ms/eval`. Search compare: Windows CUDA MCTS BK.07 `h5f6` matches Metal MCTS `h5f6`, Windows CUDA MCTS kiwipete `e2a6` matches Metal MCTS `e2a6`, Windows CUDA Hybrid BK.07 `a3b4` matches Metal Hybrid `a3b4`, and Windows CUDA Hybrid kiwipete `c5b6` matches Metal Hybrid `c5b6` |
| GitHub macOS Metal | `.github/workflows/ci.yml` | `26623653985`, Metal NN parity artifact, benchmark artifact, BK.07 and kiwipete smoke/search JSON snapshots |
| GitHub portable Linux/Windows CPU | `.github/workflows/portable-ci.yml` | `26623653946` |
| GitHub hybrid regression | `.github/workflows/hybrid-regression.yml` | `26623653938` |
...
Last direct-root promotion validation used runtime root `9d9ca3a` and produced
`metalfish-linux-x86_64-cuda.tar.gz` at `154936818` bytes, SHA-256
`c4cb17f679b275453efd10141b1d6fcba1c8e06a445151a43247004299631289`, and
`metalfish-windows-x86_64-msvc-cuda.zip` at `561958387` bytes, SHA-256
`4cfe1e87e73dce3526477515e316ba60a6d9065b31a6bda5a6bf99132dd7af19`.
