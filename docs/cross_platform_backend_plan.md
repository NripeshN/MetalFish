...
| CUDA GPU runtime gate | `tools/run_gcp_cuda_gpu_gate.sh` | Direct runtime pass `1f001b87`, `metalfish-cuda-direct-1f001b8-linux-1f001b87`, L4 `g2-standard-8` in `us-central1-a`, 2026-05-29 UTC; validates CUDA graph-replay MCTS warmup, extracted Linux CUDA package smoke, package manifest and package-check JSON evidence, packaged `test_nn_comparison` batch/reuse/graph checks, packaged BT4/legacy full-policy probe suites and isolation probes, timed pure-MCTS CUDA smoke, source and packaged pure-MCTS CUDA ponder lifecycle smokes, packaged BK.07/kiwipete/after-e4/promotion/en-passant MCTS and Hybrid search JSON, runtime manifest, Hybrid clock-start and clock-safety smokes, same-commit Metal BT4/legacy probe-suite comparison from Metal CI `26646803860`, same-commit Metal-vs-CUDA batch timing from `metal-nn-comparison.log`, and same-SHA Metal-vs-CUDA UCI search JSON comparison. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Stable-batch timing compare: `b16`, CUDA `3.3032 ms/eval`, Metal `12.4127 ms/eval`, `3.758x` speedup. Search compare: CUDA MCTS BK.07 `h5f6`, kiwipete `e2a6`, after-e4 `e7e5`, promotion `a7a8q`, and en-passant `d2d4` match Metal MCTS; CUDA Hybrid BK.07 `a3b4`, kiwipete `c5b6`, after-e4 `e7e5`, promotion `a7a8q`, and en-passant `d2d4` match Metal Hybrid |
| GitHub CUDA GPU runtime gate | `.github/workflows/cuda-gpu-gate.yml` | Manual dispatch; `metal_ci_run_id` is required by default and `tools/fetch_cuda_gpu_gate_inputs.py` must fetch a successful same-commit `MetalFish CI` artifact before the L4 VM starts, so the CUDA suite hard-compares against current macOS Metal BT4 and legacy artifacts and records Metal-vs-CUDA batch benchmark data before spending GPU time; release comparison enforces the stable batch selected by `METALFISH_CUDA_STABLE_EXECUTION_BATCH_SIZE` with `METALFISH_MAX_CUDA_METAL_EVAL_MS_RATIO` defaulting to `1.0`, while every-batch ratios remain diagnostic; `CUBLAS_WORKSPACE_CONFIG` is unset by default and only forwarded through the diagnostic `cublas_workspace_config` input |
| GitHub Windows CUDA compile gate | `.github/workflows/windows-cuda-compile.yml` | Latest branch pass `26646804122`; installs CUDA through the repo-owned NVIDIA network-installer script, then produces a self-smoked `metalfish-windows-x86_64-msvc-cuda` package artifact with `metalfish.exe`, `metalfish_nn_probe.exe`, `test_nn_comparison.exe`, and structured compile/package manifests. Last exact Windows runtime input pass: `26646804122` |
| GitHub Windows CUDA runtime gate | `.github/workflows/windows-cuda-runtime-gate.yml` | Requires `windows_cuda_run_id`; `tools/fetch_windows_cuda_runtime_inputs.py` validates the same-commit Windows CUDA compile run, downloads and validates the package JSON manifest, and, when `require_metal_compare=true`, fetches same-commit Metal artifacts plus `metal-nn-comparison.log` before creating the Windows L4 VM; `stable_batch_size` mirrors the Linux CUDA GPU gate and feeds every packaged probe/MCTS/Hybrid CUDA smoke; direct runtime pass `1f001b87`, `metalfish-cuda-direct-1f001b8-win-1f001b87`, Windows Server 2022 `g2-standard-8` L4 vWS in `us-central1-a`, 2026-05-29 UTC, compile run `26646804122`, Metal run `26646803860`, event-driven UCI `bestmove` wait, graph-replay assertions, packaged JSON manifest validation, packaged `test_nn_comparison` batch/reuse smoke, timed pure-MCTS CUDA smoke, packaged pure-MCTS CUDA ponder lifecycle smoke, BK.07/kiwipete/after-e4/promotion/en-passant MCTS and Hybrid search JSON, Hybrid clock-start and clock-safety smokes, Metal comparison, runtime manifest, and same-SHA Metal-vs-CUDA UCI search JSON comparison. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Stable-batch timing compare: `b16`, CUDA `3.1992 ms/eval`, Metal `12.4127 ms/eval`, `3.880x` speedup. Search compare: Windows CUDA MCTS BK.07 `h5f6`, kiwipete `e2a6`, after-e4 `e7e5`, promotion `a7a8q`, and en-passant `d2d4` match Metal MCTS; Windows CUDA Hybrid BK.07 `a3b4`, kiwipete `c5b6`, after-e4 `e7e5`, promotion `a7a8q`, and en-passant `d2d4` match Metal Hybrid |
| GitHub macOS Metal | `.github/workflows/ci.yml` | `26646803860`, Metal NN parity artifact, benchmark artifact, BK.07/kiwipete/after-e4/promotion/en-passant smoke/search JSON snapshots |
| GitHub portable Linux/Windows CPU | `.github/workflows/portable-ci.yml` | `26646804079` |
| GitHub hybrid regression | `.github/workflows/hybrid-regression.yml` | `26646804124` |
...
- The Linux CUDA L4 runtime package now ships `test_nn_comparison` alongside
  ...
  accepted. It also runs packaged BT4 and legacy full-policy probe suites,
  packaged BT4/legacy isolation probes, and packaged BK.07/kiwipete MCTS and
  Hybrid search JSON probes. The GCP wrapper prefers package logs and package
  search JSON for Metal/CUDA comparison when they are available. The
  2026-05-26 L4 pass `metalfish-cuda-gate-592f939-g4` on
  `g2-standard-4` in `us-west4-c` accepted the extracted package with
  ...
  BK.07 `bestmove h5f6`. The same run wrote
  `cuda-gpu-runtime-manifest.json` with source archive, VM shape, Metal compare
  inputs, status fields, and hashes for 30 collected artifacts; the manifest
  recorded `remote_status=0`, `bt4_compare_status=0`, `legacy_compare_status=0`,
  and `final_compare_status=0`.
...
- CUDA graph execution now keeps workspace allocations across ordinary
...
- Portable Linux and Windows artifacts now include `PORTABLE_ARTIFACT.md` plus
  a structured `metalfish.portable_artifact` JSON package manifest generated by
  `tools/write_portable_manifest.py`, so downloaded CI packages state the
  platform, binary name, source branch/commit, backend scope, package kind,
  package-relative file list, SHA-256 hashes, sizes, and executable bits. The
  Linux, MinGW, and MSVC package smoke steps validate the JSON manifest from
  the extracted artifact before upload. The current portable packages are CPU
  AB plus diagnostic stub-MCTS builds; they do not imply CUDA or DirectML
  strength support.
...
- Dynamic and static positional-encoding tensor geometry now resolves through
  the shared execution-plan contract instead of CUDA-local constants. The
  2026-05-25 L4 gate `metalfish-cuda-gate-20260525-pe-geometry` accepted CUDA
  unit tests, fixed BT4 references, batch parity, single/batch reuse stress,
  BT4 and legacy NN probe suites, auto/CUDA/hybrid UCI smokes,
  ANE-disable hybrid smoke, and graph replay. That run reported
...
compile gate and, by default, also downloads the same-commit macOS Metal probe
suite artifacts. The workflow validates both upstream run ids before spending a
GCP VM: `windows_cuda_run_id` must come from `Windows CUDA Compile Gate`, and
...
`go nodes 1`; BK.07 MCTS returned `bestmove h5f6` at `go nodes 50`;
`NNBackend=auto` selected CUDA for MCTS; and Hybrid loaded the same CUDA
transformer backend while returning `bestmove e2e4`. The same pass ran the
packaged `test_nn_comparison.exe` BT4 smoke with fixed references, batch parity
through batch 33, single and batch reuse stress, graph reuse probing, and a CUDA
batch benchmark ending at `executor=resolved+graph-replay`. It also
hard-compared packaged Windows CUDA full-policy outputs against macOS Metal
suite logs: BT4 maximum deltas were `4.77e-07` value, `4.77e-07` WDL,
`4.6e-05` moves-left, `1.9432e-05` top logit, and `3.7163e-05` policy; legacy
maximum deltas were `1.013e-06` value, `7.6e-06` top logit, and `1.43e-05`
policy. That closes the earlier Windows gap where the runtime gate only proved
CUDA self-consistency; Windows now shares the Metal-vs-CUDA numeric comparison
...
`26543757816` and Windows CUDA compile gate `26543757817`, wrote
`direct-runtime-gates-manifest.json`, and produced passing Linux and Windows
runtime manifests with remote/runtime status `0`, BT4 compare status `0`,
legacy compare status `0`, search compare status `0`, benchmark compare status
`0`, and final compare status `0`. Both operating systems matched the same
macOS Metal probe-suite references with BT4 maximum deltas of `4.77e-07`
value, `4.77e-07` WDL, `4.6e-05` moves-left, `1.9432e-05` top logit, and
`3.7163e-05` policy; legacy maximum deltas were `1.013e-06` value, `7.6e-06`
top logit, and `1.43e-05` policy. Linux MCTS search matched Metal on BK.07
with `h5f6` and kiwipete with `e2a6`; Windows MCTS matched both sentinels.
Hybrid BK.07 is now a strict same-bestmove compare rather than a metadata-only
smoke: Linux CUDA and Windows CUDA both returned `a3b4`, exactly matching the
same-commit Metal Hybrid artifact. The Linux and Windows portable package
manifests both recorded
`source_commit=4d3569369308e065ba1a865bcce5cc84e958877d`.

The 2026-05-28 direct pass `67d2c1d7` revalidated the release-facing runtime
contract after stabilizing the Hybrid kiwipete parity sentinel. Standard branch
CI was green at the same tip: Metal CI `26550380889`, Windows CUDA compile gate
`26550380840`, portable CI `26550380826`, and hybrid regression
`26550380854`. Linux ran `metalfish-cuda-direct-linux-67d2c1d7` on a
`g2-standard-8` L4 VM in `us-central1-a`; Windows ran
`metalfish-cuda-direct-win-67d2c1d7` on Windows Server 2022 `g2-standard-8`
L4 vWS in `us-central1-a`. Both targets matched the same macOS Metal BT4 and
legacy probe-suite references with the existing small floating-point deltas,
matched Metal MCTS on BK.07 (`h5f6`) and kiwipete (`e2a6`), matched Metal
Hybrid on BK.07 (`a3b4`), and matched Metal Hybrid on kiwipete (`c5b6`). The
Hybrid kiwipete comparison now uses `go nodes 50`; the earlier `go nodes 8`
budget mostly measured backend startup timing and could finish before AB had a
root move on Windows, which made it unsuitable as a cross-platform Hybrid
parity sentinel.

The 2026-05-28 direct pass `95335c45` revalidated the same release-facing
contract after adding timed pure-MCTS CUDA runtime smokes. Standard branch CI
was green at the same tip: Metal CI `26591533795`, Windows CUDA compile gate
`26591533872`, portable CI `26591533792`, and hybrid regression
`26591533871`. Linux ran `metalfish-cuda-direct-linux-95335c45` on a
`g2-standard-8` L4 VM in `us-east1-d`; Windows ran
`metalfish-cuda-direct-win-95335c45` on Windows Server 2022 `g2-standard-8`
L4 vWS in `us-west1-a`. Both targets matched the same macOS Metal BT4 and
legacy probe-suite references with the existing small floating-point deltas,
matched Metal MCTS on BK.07 (`h5f6`) and kiwipete (`e2a6`), matched Metal
Hybrid on BK.07 (`a3b4`) and kiwipete (`c5b6`), and passed the new timed
pure-MCTS CUDA smoke with `go movetime 500` without AB fallback. Direct release
promotion then produced same-commit packages
`metalfish-v0.1.0-alpha-linux-x86_64-cuda.tar.gz` and
`metalfish-v0.1.0-alpha-windows-x86_64-msvc-cuda.zip` under
`results/cuda_release_direct/95335c45/`, with
`cuda-release-artifacts-manifest.json` validating both package manifests and
runtime manifests against
`95335c456f309c699f6ff0f9c786c506396c5d14`.

The 2026-05-28 direct pass `512dec39` revalidated the release-facing contract
after adding pure-MCTS CUDA ponder lifecycle smokes. Standard branch CI was
green at the same tip: Metal CI `26596458371`, Windows CUDA compile gate
`26596458283`, portable CI `26596458515`, and hybrid regression `26596458190`.
Linux ran `metalfish-cuda-direct-linux-512dec39` on a `g2-standard-8` L4 VM in
`us-central1-a`; Windows ran `metalfish-cuda-direct-win-512dec39` on Windows
Server 2022 `g2-standard-8` L4 vWS in `us-west1-b` after earlier zones were
stocked out. Both targets matched the same macOS Metal BT4 and legacy
probe-suite references with the existing small floating-point deltas, matched
Metal MCTS on BK.07 (`h5f6`) and kiwipete (`e2a6`), matched Metal Hybrid on
BK.07 (`a3b4`) and kiwipete (`c5b6`), and passed the new pure-MCTS CUDA ponder
lifecycle smoke. The ponder smoke starts `go ponder`, confirms the engine does
not emit `bestmove` before `ponderhit`/`stop`, then requires a normal follow-up
search to return `bestmove`; Linux validates both source-build and packaged
artifacts, and Windows validates the packaged artifact. Direct release
promotion then produced same-commit packages
`metalfish-v0.1.0-alpha-linux-x86_64-cuda.tar.gz` and
`metalfish-v0.1.0-alpha-windows-x86_64-msvc-cuda.zip` under
`results/cuda_release_direct/512dec39/`, with
`cuda-release-artifacts-manifest.json` validating both package manifests and
runtime manifests against
`512dec39f11427279b35a2145b8b5d52218d538e`.

The 2026-05-29 direct pass `1f001b87` revalidated the release-facing runtime
contract after adding promotion and en-passant CUDA-vs-Metal search sentinels
and constraining the Hybrid promotion sentinel to `searchmoves a7a8q nodes 50`.
Standard branch CI was green at the same tip: Metal CI `26646803860`, Windows
CUDA compile gate `26646804122`, portable CI `26646804079`, and hybrid
regression `26646804124`. Linux ran
`metalfish-cuda-direct-1f001b8-linux-1f001b87` on a `g2-standard-8` L4 VM in
`us-central1-a`; Windows ran `metalfish-cuda-direct-1f001b8-win-1f001b87` on
Windows Server 2022 `g2-standard-8` L4 vWS in `us-central1-a`. Both targets
matched the same macOS Metal BT4 and legacy probe-suite references with the
existing small floating-point deltas, matched Metal MCTS on BK.07 (`h5f6`),
kiwipete (`e2a6`), after-e4 (`e7e5`), promotion (`a7a8q`), and en-passant
(`d2d4`), and matched Metal Hybrid on BK.07 (`a3b4`), kiwipete (`c5b6`),
after-e4 (`e7e5`), promotion (`a7a8q`), and en-passant (`d2d4`). The Linux
runtime manifest recorded remote, BT4, legacy, benchmark, search, and final
comparison status `0`; the Windows runtime manifest recorded runtime, BT4,
legacy, benchmark, search, and final comparison status `0`. The stable-batch
benchmark comparison used `b16`: Linux CUDA measured `3.3032 ms/eval` versus
Metal `12.4127 ms/eval` (`3.758x`), and Windows CUDA measured `3.1992 ms/eval`
...
writing a compact JSON manifest. Probe suites can replay explicit UCI move
histories, including castling and promotions, so backend comparison covers the
same position-history encoder contract used by search. For CUDA runs, the
...
top moves, and full policy. `tools/run_gcp_cuda_gpu_gate.sh` and
