...
| CUDA GPU runtime gate | `tools/run_gcp_cuda_gpu_gate.sh` | Direct runtime pass `b9165dbf`, `metalfish-cuda-direct-b9165dbf-linux-b9165dbf`, L4 `g2-standard-8` in `us-east1-b`, 2026-05-29 UTC; validates CUDA graph-replay MCTS warmup, extracted Linux CUDA package smoke, package manifest and package-check JSON evidence, packaged `test_nn_comparison` batch/reuse/graph checks, packaged BT4/legacy full-policy probe suites and isolation probes, timed pure-MCTS CUDA smoke, source and packaged pure-MCTS CUDA ponder lifecycle smokes, packaged BK.07/kiwipete/after-e4 MCTS and Hybrid search JSON, runtime manifest, Hybrid clock-start and clock-safety smokes, same-commit Metal BT4/legacy probe-suite comparison from Metal CI `26635969138`, same-commit Metal-vs-CUDA batch timing from `metal-nn-comparison.log`, and same-SHA Metal-vs-CUDA UCI search JSON comparison. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Stable-batch timing compare: `b16`, CUDA `3.1053 ms/eval`, Metal `11.5241 ms/eval`, `3.711x` speedup. Search compare: CUDA MCTS BK.07 `h5f6`, kiwipete `e2a6`, and after-e4 `e7e5` match Metal MCTS; CUDA Hybrid BK.07 `a3b4`, kiwipete `c5b6`, and after-e4 `e7e5` match Metal Hybrid |
...
| GitHub Windows CUDA compile gate | `.github/workflows/windows-cuda-compile.yml` | Latest branch pass `26635969159`; installs CUDA through the repo-owned NVIDIA network-installer script, then produces a self-smoked `metalfish-windows-x86_64-msvc-cuda` package artifact with `metalfish.exe`, `metalfish_nn_probe.exe`, `test_nn_comparison.exe`, and structured compile/package manifests. Last exact Windows runtime input pass: `26635969159` |
| GitHub Windows CUDA runtime gate | `.github/workflows/windows-cuda-runtime-gate.yml` | Requires `windows_cuda_run_id`; `tools/fetch_windows_cuda_runtime_inputs.py` validates the same-commit Windows CUDA compile run, downloads and validates the package JSON manifest, and, when `require_metal_compare=true`, fetches same-commit Metal artifacts plus `metal-nn-comparison.log` before creating the Windows L4 VM; `stable_batch_size` mirrors the Linux CUDA GPU gate and feeds every packaged probe/MCTS/Hybrid CUDA smoke; direct runtime pass `b9165dbf`, `metalfish-cuda-direct-b9165dbf-win-b9165dbf`, Windows Server 2022 `g2-standard-8` L4 vWS in `us-central1-a`, 2026-05-29 UTC, compile run `26635969159`, Metal run `26635969138`, event-driven UCI `bestmove` wait, graph-replay assertions, packaged JSON manifest validation, packaged `test_nn_comparison` batch/reuse smoke, timed pure-MCTS CUDA smoke, packaged pure-MCTS CUDA ponder lifecycle smoke, BK.07/kiwipete/after-e4 MCTS and Hybrid search JSON, Hybrid clock-start and clock-safety smokes, Metal comparison, runtime manifest, and same-SHA Metal-vs-CUDA UCI search JSON comparison. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Stable-batch timing compare: `b16`, CUDA `3.1412 ms/eval`, Metal `11.5241 ms/eval`, `3.669x` speedup. Search compare: Windows CUDA MCTS BK.07 `h5f6`, kiwipete `e2a6`, and after-e4 `e7e5` match Metal MCTS; Windows CUDA Hybrid BK.07 `a3b4`, kiwipete `c5b6`, and after-e4 `e7e5` match Metal Hybrid |
| GitHub macOS Metal | `.github/workflows/ci.yml` | `26635969138`, Metal NN parity artifact, benchmark artifact, BK.07/kiwipete/after-e4 smoke/search JSON snapshots |
| GitHub portable Linux/Windows CPU | `.github/workflows/portable-ci.yml` | `26635969137` |
| GitHub hybrid regression | `.github/workflows/hybrid-regression.yml` | `26635969194` |
