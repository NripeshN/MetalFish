...
| CUDA GPU runtime gate | `tools/run_gcp_cuda_gpu_gate.sh` | Direct runtime pass `512dec39`, `metalfish-cuda-direct-linux-512dec39`, L4 `g2-standard-8` in `us-central1-a`, 2026-05-28 UTC; validates CUDA graph-replay MCTS warmup, extracted Linux CUDA package smoke, JSON package manifest, Linux package-check JSON evidence, packaged `test_nn_comparison` batch/reuse/graph checks, packaged BT4/legacy full-policy probe suites and isolation probes, timed pure-MCTS CUDA smoke, source and packaged pure-MCTS CUDA ponder lifecycle smokes, packaged BK.07/kiwipete MCTS and Hybrid search JSON, runtime manifest, BK.07 `h5f6`, Hybrid clock-safety smokes, same-commit Metal BT4/legacy probe-suite comparison from Metal CI `26596458371`, same-commit Metal-vs-CUDA batch timing from `metal-nn-comparison.log`, and same-SHA Metal-vs-CUDA UCI search JSON comparison using packaged search results. Metal compare deltas: BT4 `value=1.79e-07`, `wdl=5.37e-07`, `moves_left=5.3e-05`, `top_logit=2.0921e-05`, `policy_max=3.1538e-05`; legacy `value=7.75e-07`, `top_logit=7.6e-06`, `policy_max=1.24e-05`. Batch timing compare: best common batch `b16`, CUDA `3.1351 ms/eval`, Metal `11.6329 ms/eval`, `3.711x` speedup. Search compare: CUDA MCTS BK.07 `h5f6` matches Metal MCTS `h5f6`, CUDA MCTS kiwipete `e2a6` matches Metal MCTS `e2a6`, CUDA Hybrid BK.07 `a3b4` matches Metal Hybrid `a3b4`, and CUDA Hybrid kiwipete `c5b6` matches Metal Hybrid `c5b6` |
...
| GitHub Windows CUDA compile gate | `.github/workflows/windows-cuda-compile.yml` | Latest branch pass `26596458283`; installs CUDA through the repo-owned NVIDIA network-installer script, then produces a self-smoked `metalfish-windows-x86_64-msvc-cuda` package artifact with `metalfish.exe`, `metalfish_nn_probe.exe`, `test_nn_comparison.exe`, and structured compile/package manifests. Last exact Windows runtime input pass: `26596458283` |
| GitHub Windows CUDA runtime gate | `.github/workflows/windows-cuda-runtime-gate.yml` | Requires `windows_cuda_run_id`; `tools/fetch_windows_cuda_runtime_inputs.py` validates the same-commit Windows CUDA compile run, downloads and validates the package JSON manifest, and, when `require_metal_compare=true`, fetches same-commit Metal artifacts plus `metal-nn-comparison.log` before creating the Windows L4 VM; `stable_batch_size` mirrors the Linux CUDA GPU gate and feeds every packaged probe/MCTS/Hybrid CUDA smoke; direct runtime pass `512dec39`, `metalfish-cuda-direct-win-512dec39`, Windows Server 2022 `g2-standard-8` L4 vWS in `us-west1-b`, 2026-05-28 UTC, compile run `26596458283`, Metal run `26596458371`, event-driven UCI `bestmove` wait, graph-replay assertions, packaged JSON manifest validation, packaged `test_nn_comparison` batch/reuse smoke, timed pure-MCTS CUDA smoke, packaged pure-MCTS CUDA ponder lifecycle smoke, BK.07 `h5f6`, Hybrid clock-safety smokes, Metal comparison, runtime manifest, and same-SHA Metal-vs-CUDA UCI search JSON comparison. Metal compare deltas: BT4 `value=1.79e-07`, `wdl=5.37e-07`, `moves_left=5.3e-05`, `top_logit=2.0921e-05`, `policy_max=3.1538e-05`; legacy `value=7.75e-07`, `top_logit=7.6e-06`, `policy_max=1.24e-05`. Batch timing compare: best common batch `b16`, CUDA `3.0838 ms/eval`, Metal `11.6329 ms/eval`, `3.772x` speedup. Search compare: Windows CUDA MCTS BK.07 `h5f6` matches Metal MCTS `h5f6`, Windows CUDA MCTS kiwipete `e2a6` matches Metal MCTS `e2a6`, Windows CUDA Hybrid BK.07 `a3b4` matches Metal Hybrid `a3b4`, and Windows CUDA Hybrid kiwipete `c5b6` matches Metal Hybrid `c5b6` |
| GitHub macOS Metal | `.github/workflows/ci.yml` | `26596458371`, Metal NN parity artifact, benchmark artifact, BK.07 and kiwipete smoke/search JSON snapshots |
| GitHub portable Linux/Windows CPU | `.github/workflows/portable-ci.yml` | `26596458515` |
| GitHub hybrid regression | `.github/workflows/hybrid-regression.yml` | `26596458190` |
