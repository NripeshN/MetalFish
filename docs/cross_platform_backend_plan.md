...
| CUDA GPU runtime gate | `tools/run_gcp_cuda_gpu_gate.sh` | Direct runtime pass `bc30b058`, `metalfish-cuda-direct-linux-bc30b058`, L4 `g2-standard-8` in `us-central1-a`, 2026-05-29 UTC; validates CUDA graph-replay MCTS warmup, extracted Linux CUDA package smoke, JSON package manifest, Linux package-check JSON evidence, packaged `test_nn_comparison` batch/reuse/graph checks, packaged BT4/legacy full-policy probe suites and isolation probes, timed pure-MCTS CUDA smoke, source and packaged pure-MCTS CUDA ponder lifecycle smokes, packaged BK.07/kiwipete MCTS and Hybrid search JSON, runtime manifest, BK.07 `h5f6`, Hybrid clock-start and clock-safety smokes, same-commit Metal BT4/legacy probe-suite comparison from Metal CI `26614086504`, same-commit Metal-vs-CUDA batch timing from `metal-nn-comparison.log`, and same-SHA Metal-vs-CUDA UCI search JSON comparison using packaged search results. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Batch timing compare: best common batch `b16`, CUDA `3.2238 ms/eval`, Metal `11.9827 ms/eval`, `3.717x` speedup. Search compare: CUDA MCTS BK.07 `h5f6` matches Metal MCTS `h5f6`, CUDA MCTS kiwipete `e2a6` matches Metal MCTS `e2a6`, CUDA Hybrid BK.07 `a3b4` matches Metal Hybrid `a3b4`, and CUDA Hybrid kiwipete `c5b6` matches Metal Hybrid `c5b6` |
| GitHub CUDA GPU runtime gate | `.github/workflows/cuda-gpu-gate.yml` | Manual dispatch; `metal_ci_run_id` is required by default and `tools/fetch_cuda_gpu_gate_inputs.py` must fetch a successful same-commit `MetalFish CI` artifact before the L4 VM starts, so the CUDA suite hard-compares against current macOS Metal BT4 and legacy artifacts and records Metal-vs-CUDA batch benchmark data before spending GPU time; CUDA benchmark comparison also enforces `METALFISH_MAX_CUDA_METAL_EVAL_MS_RATIO` with a default of `1.0`, so CUDA cannot be slower than Metal on common batches unless a diagnostic override is set; `CUBLAS_WORKSPACE_CONFIG` is unset by default and only forwarded through the diagnostic `cublas_workspace_config` input |
| GitHub Windows CUDA compile gate | `.github/workflows/windows-cuda-compile.yml` | Latest branch pass `26614086547`; installs CUDA through the repo-owned NVIDIA network-installer script, then produces a self-smoked `metalfish-windows-x86_64-msvc-cuda` package artifact with `metalfish.exe`, `metalfish_nn_probe.exe`, `test_nn_comparison.exe`, and structured compile/package manifests. Last exact Windows runtime input pass: `26614086547` |
| GitHub Windows CUDA runtime gate | `.github/workflows/windows-cuda-runtime-gate.yml` | Requires `windows_cuda_run_id`; `tools/fetch_windows_cuda_runtime_inputs.py` validates the same-commit Windows CUDA compile run, downloads and validates the package JSON manifest, and, when `require_metal_compare=true`, fetches same-commit Metal artifacts plus `metal-nn-comparison.log` before creating the Windows L4 VM; `stable_batch_size` mirrors the Linux CUDA GPU gate and feeds every packaged probe/MCTS/Hybrid CUDA smoke; direct runtime pass `bc30b058`, `metalfish-cuda-direct-win-bc30b058`, Windows Server 2022 `g2-standard-8` L4 vWS in `us-central1-a`, 2026-05-29 UTC, compile run `26614086547`, Metal run `26614086504`, event-driven UCI `bestmove` wait, graph-replay assertions, packaged JSON manifest validation, packaged `test_nn_comparison` batch/reuse smoke, timed pure-MCTS CUDA smoke, packaged pure-MCTS CUDA ponder lifecycle smoke, BK.07 `h5f6`, Hybrid clock-start and clock-safety smokes, Metal comparison, runtime manifest, and same-SHA Metal-vs-CUDA UCI search JSON comparison. Metal compare deltas: BT4 `value=3.57e-07`, `wdl=4.77e-07`, `moves_left=5.34e-05`, `top_logit=2.116e-05`, `policy_max=3.4653e-05`; legacy `value=7.75e-07`, `top_logit=6.7e-06`, `policy_max=1.24e-05`. Batch timing compare: best common batch `b16`, CUDA `3.0543 ms/eval`, Metal `11.9827 ms/eval`, `3.923x` speedup; fastest CUDA batch was `b32` at `3.0284 ms/eval`. Search compare: Windows CUDA MCTS BK.07 `h5f6` matches Metal MCTS `h5f6`, Windows CUDA MCTS kiwipete `e2a6` matches Metal MCTS `e2a6`, Windows CUDA Hybrid BK.07 `a3b4` matches Metal Hybrid `a3b4`, and Windows CUDA Hybrid kiwipete `c5b6` matches Metal Hybrid `c5b6` |
| GitHub macOS Metal | `.github/workflows/ci.yml` | `26614086504`, Metal NN parity artifact, benchmark artifact, BK.07 and kiwipete smoke/search JSON snapshots |
| GitHub portable Linux/Windows CPU | `.github/workflows/portable-ci.yml` | `26614086484` |
| GitHub hybrid regression | `.github/workflows/hybrid-regression.yml` | `26614086535` |
...
Last direct-root promotion validation used runtime root `bc30b05` with
`--tag-name v0.1.0-alpha` and produced
`metalfish-v0.1.0-alpha-linux-x86_64-cuda.tar.gz` at `154936784` bytes,
SHA-256 `0b1dfc657f1ecec391d7e15f7245fbe65829b8a2ede9cfdac80f229b44304a2f`,
and `metalfish-v0.1.0-alpha-windows-x86_64-msvc-cuda.zip` at `561958376`
bytes, SHA-256
`1e9a5dce037afa794cf17d2a9e834b6da3999cdae30c7ed28f9355e03a2c2654`.
