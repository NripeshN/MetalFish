...
Additionally, real-world performance is influenced by hybrid CPU--GPU execution decisions, where certain search and evaluation workloads may execute on the CPU despite GPU availability.
...
These projections assume effective workload partitioning between CPU and GPU components, and actual gains depend on the frequency with which search and evaluation tasks are executed on each processing unit.
...
\textbf{Hybrid Execution Trade-offs:} The use of a hybrid CPU--GPU execution model introduces dynamic performance trade-offs, as certain workloads may execute more efficiently on the CPU despite GPU availability. This can lead to variability in realized speedups compared to idealized GPU-only performance models.
