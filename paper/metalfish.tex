...
\begin{itemize}
\item \textbf{Heuristic priors}: Policy based on chess heuristics, not uniform
...
\item \textbf{MCTS transposition table}: Caches evaluations across searches (97.7\% hit rate observed)
\end{itemize}
...
Table~\ref{tab:mcts_profile} shows where time is spent during MCTS search, explaining why throughput is 294K nodes/second rather than the millions suggested by raw GPU evaluation speed.
...
\midrule
Selection & 97.9\% & Tree traversal with PUCT \\
Expansion & 1.6\% & Move generation, node creation \\
Evaluation & 0.4\% & GPU NNUE (mostly TT hits) \\
Backpropagation & 0.1\% & Statistics update \\
\midrule
Total nodes & \multicolumn{2}{r}{881,677} \\
NPS & \multicolumn{2}{r}{293,889} \\
TT hit rate & \multicolumn{2}{r}{97.7\%} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ablation} shows the impact of disabling the position classifier and alpha-beta verifier.

\begin{table}[t]
\caption{Ablation Study (1 second per position, 4 positions)}
\label{tab:ablation}
\centering
\begin{tabular}{lrr}
\toprule
Configuration & Avg Nodes & Avg NPS \\
\midrule
Full Hybrid (classifier + verifier) & 41,255 & 269,334 \\
No Classifier (fixed weights) & 153,181 & 250,021 \\
No Verifier (MCTS only) & 40,804 & 266,504 \\
Pure MCTS & 153,870 & 251,170 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Key findings}:
\begin{enumerate}
\item \textbf{Classifier impact}: The position classifier significantly affects time allocation---with classifier enabled, MCTS receives only 41K nodes vs 153K without classifier, as more time is allocated to alpha-beta verification.

\item \textbf{Consistent NPS}: Throughput remains stable at 250--270K NPS regardless of configuration, confirming that the classifier and verifier do not introduce significant overhead.

\item \textbf{Verifier overhead}: Minimal impact on node count when verifier is disabled (41K vs 40K), suggesting the verifier's time budget is separate from MCTS allocation.
\end{enumerate}
...
Table~\ref{tab:correctness} verifies GPU evaluation reproducibility. We use the official Stockfish benchmark positions (16 positions from opening, middlegame, endgame, and mate scenarios) for all validation.
...
In our experiments, the verifier rarely overrides MCTS (0 overrides in benchmark positions), suggesting MCTS produces reasonable moves. The heuristic-based policy priors help guide MCTS toward capturing and checking moves.

\textbf{Tactical test results}: On the official Stockfish benchmark positions (16 positions including tactical middlegame and mate positions), the engine successfully finds strong moves including Scholar's mate threat (Bxf7+) and other tactical sequences. The heuristic policy priors significantly improve tactical move ordering compared to uniform priors.

\subsection{Limitations}

\begin{itemize}
\item \textbf{Single-threaded MCTS}: Our current implementation uses single-threaded MCTS due to GPU evaluation thread-safety constraints. Multi-threaded MCTS would significantly increase throughput.

\item \textbf{Heuristic vs trained policy}: We use heuristic-based policy priors. A trained policy network would further improve MCTS efficiency, especially in complex positions where heuristics are insufficient.
...
\item \textbf{Playing strength}: Elo rating has not been validated against established engines; this is future work.
\end{itemize}
...
\textbf{Limitations}: We use heuristic-based policy priors (captures, checks, promotions, center control), which improves over uniform priors but is still less effective than a trained policy network. Playing strength has not been validated against established engines; this is future work.

\textbf{Key insight}: The hybrid MCTS-alpha-beta architecture provides a framework for combining strategic exploration with tactical precision on GPU-accelerated unified memory systems. The primary bottleneck is tree traversal (selection at 97.9\%), not GPU evaluation, suggesting that algorithmic improvements to MCTS selection would have greater impact than further GPU optimization.
