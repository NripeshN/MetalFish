...
\begin{abstract}
We present MetalFish, a novel chess engine that combines Monte Carlo Tree Search (MCTS) with traditional alpha-beta pruning, using GPU-accelerated NNUE evaluation on Apple Silicon's unified memory architecture. Our hybrid approach dynamically classifies positions as tactical or strategic, allocating search resources accordingly: MCTS for exploration in strategic positions, alpha-beta for tactical verification. On Apple M2 Max, our GPU NNUE implementation achieves 572$\times$ speedup through batch evaluation (0.3~$\mu$s/position at N=4096 vs 258~$\mu$s for single positions), enabling MCTS throughput of 190K--280K nodes/second. The position classifier identifies five position types with distinct search strategies, while the alpha-beta verifier provides tactical oversight with configurable override thresholds. We demonstrate that GPU dispatch overhead (140~$\mu$s median) makes single-position GPU evaluation unsuitable for pure alpha-beta search, but batch-oriented MCTS effectively amortizes this cost. Our hybrid architecture achieves competitive play while providing a framework for combining the strategic depth of MCTS with the tactical precision of alpha-beta.
...
\item \textbf{MCTS transposition table}: Caches evaluations across searches
\end{itemize}
...
\item \textbf{MCTS throughput}: 190K--280K nodes/second with GPU NNUE evaluation, leveraging batch processing to amortize dispatch overhead.
...
\textbf{Key insight}: The hybrid MCTS-alpha-beta architecture combines the strategic exploration of MCTS with the tactical precision of alpha-beta, while GPU acceleration provides efficient batch evaluation for MCTS leaf nodes. This architecture demonstrates a practical approach to leveraging GPU compute for chess engines on unified memory systems.
