...
\begin{enumerate}
\item \textbf{Quantified CPU vs GPU NNUE comparison}: CPU NNUE achieves 1.34M positions/second (0.75~$\mu$s/pos); GPU single-position blocking latency is 265~$\mu$s (353$\times$ slower).
...
\item \textbf{Batch scaling analysis}: Per-position cost drops from 265~$\mu$s (N=1) to 0.3~$\mu$s (N=4096), with dispatch overhead amortized across positions.

\item \textbf{True batching verification}: Single command buffer with two dispatches achieves 552$\times$ speedup over N sequential command buffers at N=1024.
...
256 & 300 & 362 & 400 & 1.2 \\
512 & 353 & 413 & 458 & 0.7 \\
1024 & 511 & 620 & 655 & 0.5 \\
2048 & 794 & 896 & 996 & 0.4 \\
4096 & 1,389 & 1,483 & 1,660 & 0.3 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Key findings}:
\begin{enumerate}
\item Latency is approximately constant (265--300~$\mu$s) for batch sizes 1--256, confirming dispatch dominance.
\item Per-position cost drops from 265~$\mu$s (N=1) to 0.3~$\mu$s (N=4096).
...
\midrule
16 & 4,565~$\mu$s & 270~$\mu$s & 16.9$\times$ \\
64 & 18,063~$\mu$s & 289~$\mu$s & 62.4$\times$ \\
256 & 72,601~$\mu$s & 310~$\mu$s & 234.1$\times$ \\
1024 & 291,539~$\mu$s & 528~$\mu$s & 552.0$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item NNUE kernel dispatch: 265~$\mu$s
...
Single-position GPU blocking latency (265~$\mu$s) makes GPU evaluation unsuitable for alpha-beta search in synchronous blocking mode. Alpha-beta's sequential, data-dependent pruning prevents effective batching without significant architectural changes such as:
\begin{itemize}
...
\item \textbf{GPU single-position}: 265~$\mu$s median (353$\times$ slower than CPU)
...
\item \textbf{True batching}: 552$\times$ speedup at N=1024
