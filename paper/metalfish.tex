...
\subsection{Command Buffer Optimization}

We minimize dispatch overhead through several techniques:
\begin{itemize}
\item \textbf{Unretained references}: Use \texttt{commandBufferWithUnretainedReferences} to avoid Objective-C retain/release overhead
\item \textbf{Hazard tracking disabled}: Use \texttt{MTLResourceHazardTrackingModeUntracked} for unified memory buffers with explicit barriers
\item \textbf{Pre-allocated buffers}: All working buffers allocated at initialization to avoid per-dispatch allocation
\end{itemize}
