...
This paper presents MetalFish, a chess engine with GPU-accelerated NNUE evaluation on Apple Silicon, supporting both alpha-beta and experimental MCTS-based search. Our primary contribution is demonstrating that GPU NNUE evaluation can match CPU NNUE quality---validated by MetalFish-AB achieving competitive Elo with Stockfish. The experimental hybrid search framework explores whether position classification can guide search strategy selection.
...
\begin{enumerate}
\item \textbf{GPU NNUE with validated quality}: Efficient batch evaluation achieving 635$\times$ speedup, with evaluation quality validated by competitive Elo against Stockfish.
...
\item \textbf{Hardware}: All engines on same Apple M2 Max (12-core CPU, 38-core GPU, 64GB)
...
Table~\ref{tab:elo_ratings} shows the final Elo ratings computed using iterative Bayesian estimation with Patricia (known $\sim$3500 Elo) as anchor.

\begin{table}[htbp]
\caption{Tournament Elo Ratings (900 games, 45 matches, 10+0.1s)}
...
10 & Lc0$^\dagger$ & 903 \\
\bottomrule
\multicolumn{3}{l}{\footnotesize $^\dagger$Lc0 run without production network; result not representative.}
\end{tabular}
\end{table}

\textbf{Key findings}:
\begin{enumerate}
\item \textbf{MetalFish-AB competitive with Stockfish}: MetalFish-AB (3873 Elo) slightly exceeds Stockfish-Full (3853 Elo), achieving 2 wins and 18 draws in 20 direct games. This validates that GPU NNUE evaluation quality matches CPU NNUE.

\item \textbf{Hybrid search gap}: MetalFish-Hybrid (1512 Elo) and MetalFish-MCTS (1424 Elo) significantly underperform, demonstrating that heuristic policy priors are insufficient for competitive MCTS strength. The $\sim$2400 Elo gap represents the cost of not having trained priors.
...
MetalFish-Hybrid & Lc0$^\dagger$ & 20 & 0 & 0 \\
MetalFish-MCTS & Lc0$^\dagger$ & 20 & 0 & 0 \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^\dagger$Lc0 without production network.}
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    ylabel={Elo Rating},
    ymin=0, ymax=4500,
    symbolic x coords={MF-AB, SF, Patricia, SF-15, SF-10, SF-5, SF-1, MF-Hyb, MF-MCTS, Lc0},
    xtick=data,
...
\end{axis}
\end{tikzpicture}
\caption{Tournament Elo ratings. MF-AB = MetalFish-AB, SF = Stockfish-Full, SF-N = Stockfish Level N. Lc0 result not representative (missing production network).}
\label{fig:elo_chart}
\end{figure}
...
\begin{enumerate}
\item \textbf{GPU NNUE validates evaluation quality}: MetalFish-AB achieves 3873 Elo versus Stockfish-Full's 3853 Elo (2-0-18 head-to-head), demonstrating that GPU NNUE evaluation matches CPU NNUE quality under identical tournament conditions.

\item \textbf{Hybrid search is experimental}: The hybrid (1512 Elo) and pure MCTS (1424 Elo) variants significantly underperform, demonstrating that heuristic-based policy priors are insufficient. The framework is the contribution, not the current strength.
...
\textbf{Key insight}: The GPU NNUE implementation preserves evaluation quality, validated by MetalFish-AB's competitive Elo with Stockfish under identical tournament conditions. The experimental hybrid framework demonstrates that combining MCTS with alpha-beta is architecturally feasible, but competitive MCTS strength requires trained policy priors rather than heuristics.

\textbf{Future directions}: A trained policy network is expected to significantly reduce the $\sim$2400 Elo gap between MetalFish-AB and MetalFish-Hybrid, enabling the hybrid architecture to leverage MCTS's exploratory strengths.

\subsection*{Reproducibility}

\textbf{Hardware}: Apple M2 Max, 64GB unified memory. \textbf{Software}: macOS 14.0, Xcode 15.0, Metal 3.0. \textbf{Build}: CMake, -O3, LTO enabled. \textbf{Source}: \url{https://github.com/NripeshN/MetalFish}. \textbf{Branch}: \texttt{hybrid-mcts-alphabeta}. \textbf{Benchmarks}: \texttt{gpubench}, \texttt{mctsbench}, \texttt{hybridbench} UCI commands. \textbf{Tournament}: \texttt{tools/elo\_tournament.py} with cutechess-cli.
