...
\begin{enumerate}
\item \textbf{Optimized GPU NNUE implementation}: We achieve 278~$\mu$s median single-position blocking latency and 0.3~$\mu$s per position at batch size 4096.
...
The 147~$\mu$s median dispatch overhead represents the minimum cost for any GPU operation in synchronous blocking mode.
...
\midrule
1 & 0.2 $\mu$s & 275 $\mu$s & 99.9\% \\
8 & 0.3 $\mu$s & 266 $\mu$s & 99.9\% \\
512 & 5.9 $\mu$s & 373 $\mu$s & 98.5\% \\
\bottomrule
\end{tabular}
\end{table}
...
128 & 289 & 378 & 418 & 2.3 \\
512 & 377 & 483 & 603 & 0.7 \\
1024 & 549 & 648 & 687 & 0.5 \\
2048 & 833 & 953 & 980 & 0.4 \\
3072 & 1,122 & 1,240 & 1,370 & 0.4 \\
4096 & 1,433 & 1,579 & 1,602 & 0.3 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key findings}: (1) Latency is approximately constant (271--289~$\mu$s) for batch sizes 1--128, showing dispatch dominance. (2) Per-position cost drops from 278~$\mu$s (N=1) to 0.3~$\mu$s (N=4096). (3) Latency increases linearly beyond N=512, indicating kernel compute becoming significant.
...
\midrule
16 & 4,679 $\mu$s & 274 $\mu$s & 17.1$\times$ \\
64 & 18,647 $\mu$s & 291 $\mu$s & 64.1$\times$ \\
256 & 74,702 $\mu$s & 316 $\mu$s & 236.2$\times$ \\
1024 & 297,663 $\mu$s & 562 $\mu$s & 529.5$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
Single-position GPU blocking latency (278~$\mu$s) makes GPU evaluation unsuitable for alpha-beta search in synchronous blocking mode and without speculative evaluation. Alpha-beta search evaluates positions sequentially with data-dependent pruning, making batch accumulation impractical without significant architectural changes.
...
\begin{enumerate}
\item \textbf{278~$\mu$s} median single-position blocking latency
\item \textbf{0.3~$\mu$s} per-position cost at batch size 4096
