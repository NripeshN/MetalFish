...
\begin{abstract}
We present MetalFish, a hybrid CPU-GPU chess engine implementing Stockfish-style alpha-beta search with Metal GPU acceleration on Apple Silicon unified memory architecture. Our empirical evaluation reveals a fundamental challenge: GPU kernel dispatch overhead (703~$\mu$s median) exceeds CPU NNUE evaluation time (84~ns median) by approximately 8,400$\times$ for single positions, making GPU acceleration counterproductive for traditional tree search. This overhead arises from Metal command buffer creation, encoding, and synchronization---inherent costs that cannot be amortized without batching thousands of positions per dispatch. We present detailed measurements including percentile distributions and discuss the architectural implications for GPU-accelerated game engines. The implementation achieves 1.43 million nodes per second using CPU evaluation. Our findings suggest that GPU acceleration for chess engines is most effective when combined with batch-oriented search algorithms such as MCTS, rather than traditional alpha-beta search.
...
\begin{enumerate}
\item An empirical characterization of Metal GPU dispatch overhead showing 703~$\mu$s median latency versus 84~ns for CPU evaluation---an 8,400$\times$ slowdown for single positions.
...
\begin{table}[t]
\caption{Single-position evaluation latency. CPU times in nanoseconds; GPU times in microseconds. Percentiles capture timing variability from system scheduling, cache effects, and GPU driver behavior.}
...
The GPU evaluation is approximately 8,400$\times$ slower (median-to-median) for single positions. The high CPU max (55~$\mu$s) reflects occasional cache misses or preemption; the median and P99 values (84--125~ns) better represent typical performance.
...
\addlegendentry{CPU (84~ns/pos)}
\addplot[red, thick, domain=1:20000] {703};
\addlegendentry{GPU dispatch (703~$\mu$s)}
\addplot[only marks, mark=*, green!60!black] coordinates {(8369, 703)};
\addlegendentry{Crossover ($\approx$8,400 pos)}
\end{axis}
\end{tikzpicture}
\caption{CPU vs GPU evaluation time under a synchronous per-dispatch model. GPU dispatch overhead (703~$\mu$s) is approximately constant regardless of positions evaluated, as kernel compute time is small relative to dispatch overhead in our implementation. Crossover occurs at approximately 8,400 positions per dispatch.}
\label{fig:crossover}
\end{figure}
...
For alpha-beta search, attempts to leverage GPU parallelism have focused on parallel subtree evaluation and speculative execution. Our work provides concrete measurements showing why single-position GPU evaluation is counterproductive for sequential search, complementing prior theoretical analyses of GPU-search integration challenges.
...
\begin{enumerate}
\item GPU dispatch overhead (703~$\mu$s median) exceeds CPU evaluation time (84~ns median) by 8,400$\times$ for single positions
