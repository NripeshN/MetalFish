...
AlphaZero~\cite{silver2018general} demonstrated that MCTS guided by deep neural networks could achieve superhuman chess play without domain-specific knowledge beyond the rules. Lc0~\cite{lc0} is the open-source continuation of this approach, using transformer networks~\cite{vaswani2017attention} for policy and value prediction.
...
\begin{tikzpicture}[
    ...
    block/.style={rectangle, draw, rounded corners, minimum width=2.6cm, minimum height=0.8cm, align=center, font=\small},
    ...
    % Position classifier
    \node[block, below=0.8cm of uci] (classifier) {Position Classifier};
    
    % Three engines
    \node[cpu, below left=1.0cm and 2.2cm of classifier] (ab) {Alpha-Beta\\Engine};
    \node[gpu, below=1.0cm of classifier] (mcts) {MCTS\\Engine};
    \node[hybrid, below right=1.0cm and 2.2cm of classifier] (hybride) {Hybrid\\Coordinator};
    
    % Shared state
    \node[shared, below=1.0cm of mcts] (state) {Lock-Free\\Shared State};
    
    % Backend components side by side below shared state
    \node[cpu, below left=1.2cm and 1.5cm of state] (nnue) {NNUE\\Evaluation (CPU)};
    \node[gpu, below right=1.2cm and 1.5cm of state] (transformer) {Transformer\\(Metal GPU)};
    
    % Unified memory bar at very bottom
    \node[block, fill=gray!15, below=2.8cm of state, minimum width=9cm] (memory) {Apple Silicon Unified Memory};
...
On 8~threads, the engine reaches 7.6M nodes per second on Apple M2~Max and 9.8M NPS on M1~Ultra, demonstrating near-linear scaling (7.24$\times$ on 8~threads). The engine is the strongest of our three modes, achieving an estimated 4048 Elo with zero losses in 110 tournament games.
...
The hybrid engine coordinates alpha-beta and MCTS running simultaneously:
...
The MCTS engine required several Apple Silicon-specific optimizations to achieve competitive performance. Table~\ref{tab:optimizations} summarizes the impact of each optimization.
...
The most impactful optimization replaced \texttt{std::mutex} (64~bytes on Darwin/ARM64) with Apple's \texttt{os\_unfair\_lock} (4~bytes) for per-node score updates during backpropagation. This single change:
...
The \texttt{os\_unfair\_lock} is a 4-byte futex-like primitive with priority inheritance, designed for Apple Silicon's memory model. It provides userspace fast-path acquisition without syscall overhead on uncontended locks.
...
\textbf{Engines.} We evaluate three configurations of our engine: MetalFish-AB (alpha-beta with NNUE), MetalFish-MCTS (pure transformer MCTS), and MetalFish-Hybrid (parallel AB+MCTS). Reference engines: Stockfish dev-20260106 at skill levels 5, 8, 10, 12, 14, 16, and full strength ($\sim$3800 Elo); Berserk 20250622 ($\sim$3722); Patricia ($\sim$3415); and Lc0 with BT4-1024$\times$15$\times$32h network via Metal backend.
...
We conducted a comprehensive tournament of 322 games across 33 matches at 300s + 0.1s time control on M1~Ultra instances. Each of our three engines played against all 10 reference opponents plus each other. Results are shown in Table~\ref{tab:tournament} and the estimated Elo ratings in Fig.~\ref{fig:elo}.

\begin{table}[!htbp]
\centering
\caption{Tournament results: MetalFish engines vs.\ reference opponents (322 games, 300+0.1s TC, M1 Ultra, 10 threads). W/D/L from MetalFish's perspective.}
\label{tab:tournament}
\small
\begin{tabular}{llcccr}
\toprule
\textbf{Engine} & \textbf{Opponent} & \textbf{W} & \textbf{D} & \textbf{L} & \textbf{Score} \\
\midrule
\multirow{9}{*}{\rotatebox{90}{\textbf{MF-AB}}}
& Stockfish      & --- & --- & 0 & undefeated \\
& SF-L16 ($\sim$3584) & 5 & 0 & 0 & 5.0/5 \\
& SF-L14 ($\sim$3505) & 3 & 2 & 0 & 4.0/5 \\
& SF-L12 ($\sim$3412) & 5 & 0 & 0 & 5.0/5 \\
& SF-L10 ($\sim$3304) & 5 & 0 & 0 & 5.0/5 \\
& SF-L8 ($\sim$3156)  & 5 & 0 & 0 & 5.0/5 \\
& Berserk ($\sim$3722) & 5 & 0 & 0 & 5.0/5 \\
& Lc0 ($\sim$3700)     & 2 & 3 & 0 & 3.5/5 \\
& Patricia ($\sim$3415) & 5 & 0 & 0 & 5.0/5 \\
\midrule
\multirow{7}{*}{\rotatebox{90}{\textbf{MF-Hybrid}}}
& Stockfish      & 0 & 4 & 2 & 2.0/6 \\
& SF-L16         & 2 & 3 & 0 & 3.5/5 \\
& SF-L14         & 4 & 1 & 0 & 4.5/5 \\
& SF-L12         & 3 & 2 & 0 & 4.0/5 \\
& SF-L10         & 4 & 1 & 0 & 4.5/5 \\
& Berserk        & 5 & 0 & 0 & 5.0/5 \\
& Lc0            & 0 & 4 & 1 & 2.0/5 \\
& Patricia       & 0 & 2 & 3 & 1.0/5 \\
\midrule
\multirow{7}{*}{\rotatebox{90}{\textbf{MF-MCTS}}}
& Lc0            & 0 & 1 & 5 & 0.5/6 \\
& SF-L16         & 0 & 3 & 2 & 1.5/5 \\
& SF-L14         & 0 & 2 & 3 & 1.0/5 \\
& SF-L10         & 3 & 0 & 2 & 3.0/5 \\
& SF-L8          & 4 & 0 & 1 & 4.0/5 \\
& SF-L5 ($\sim$2800) & 5 & 0 & 0 & 5.0/5 \\
& Berserk        & 5 & 0 & 0 & 5.0/5 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    bar width=0.4cm,
    xmin=2700, xmax=4200,
    xlabel={Estimated Elo Rating},
    ytick=data,
    yticklabels={SF-L5, SF-L8, SF-L10, \textbf{MF-MCTS}, SF-L12, Patricia, SF-L14, SF-L16, Lc0, Berserk, \textbf{MF-Hybrid}, Stockfish, \textbf{MF-AB}},
    nodes near coords,
...
However, the current hybrid is limited by coordination overhead: the AB component reports nodes through the hybrid coordinator rather than directly, introducing latency. Additionally, the MCTS component's 200 NPS throughput means it contributes relatively few evaluations during the alpha-beta engine's thinking time.
...
Future work includes improving the MCTS engine through cache key optimization for transposition hits, implementing the remaining Lc0 search heuristics, and exploring learned arbitration for the hybrid decision function. The complete source code is available as open-source software.
...
\bibitem{uci}
Huber, S.: Universal Chess Interface protocol specification. \url{http://wbec-ridderkerk.nl/html/UCIProtocol.html} (2000)
