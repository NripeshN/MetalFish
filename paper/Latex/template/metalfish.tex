...
\begin{tikzpicture}[
    ...
    % Three engines
    \node[cpu, below left=1cm and 2cm of classifier] (ab) {Alpha-Beta\\Engine};
    \node[gpu, below=1cm of classifier] (mcts) {MCTS\\Engine};
    \node[hybrid, below right=1cm and 2cm of classifier] (hybrid) {Hybrid\\Coordinator};
    
    % Shared state
    \node[shared, below=1.2cm of mcts] (state) {Lock-Free\\Shared State};
    
    % Backend components
    \node[cpu, below left=1cm and 0.5cm of state] (nnue) {NNUE\\Evaluation};
    \node[gpu, below right=1cm and 0.5cm of state] (transformer) {Transformer\\(MPSGraph)};
    
    % Unified memory
    \node[block, fill=gray!15, below=0.8cm of state, minimum width=7cm] (memory) {Apple Silicon Unified Memory};
    
    % Arrows
    \draw[arrow] (uci) -- (classifier);
    \draw[arrow] (classifier) -- (ab);
    \draw[arrow] (classifier) -- (mcts);
    \draw[arrow] (classifier) -- (hybrid);
    \draw[arrow] (ab) -- (state);
    \draw[arrow] (mcts) -- (state);
    \draw[arrow] (hybrid) -- (state);
    \draw[arrow] (state) -- (nnue);
    \draw[arrow] (state) -- (transformer);
    \draw[arrow, dashed] (nnue) -- (memory);
    \draw[arrow, dashed] (transformer) -- (memory);
