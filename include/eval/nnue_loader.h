...
struct NNUEWeights {
    ...
    std::vector<FCWeight> fc0_weights;  // [HalfDims*2 x FC0_OUTPUTS x LAYER_STACKS]
    std::vector<FCBias> fc0_biases;     // [FC0_OUTPUTS x LAYER_STACKS]
    std::vector<FCWeight> fc1_weights;  // [FC1_INPUTS x FC1_OUTPUTS x LAYER_STACKS]
...
template<typename T>
inline T read_le(std::istream& stream) {
    T value;
...
private:
    ...
    bool read_header(std::istream& stream, uint32_t& version, uint32_t& hash, std::string& arch);
