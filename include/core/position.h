...
// StateInfo stores information needed to restore a Position to its previous state
struct StateInfo {
...
public:
    ...
    int count(Color c) const { return pieceCount[make_piece(c, Pt)]; }
    ...
    bool castling_impeded(CastlingRights cr) const { return pieces() & castlingPath[cr]; }
