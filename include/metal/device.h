...
struct DeviceStream {
    DeviceStream(MTL::CommandQueue* q) : queue(q), buffer(nullptr), encoder(nullptr) {}
    ~DeviceStream() { if (buffer) buffer->release(); }
    
    // Move only
    DeviceStream(DeviceStream&& other) noexcept 
        : queue(other.queue), buffer(other.buffer), encoder(std::move(other.encoder)) {
        other.queue = nullptr;
...
public:
    ...
    MTL::ComputePipelineState* get_kernel(const std::string& name, MTL::Library* mtl_lib = nullptr);
