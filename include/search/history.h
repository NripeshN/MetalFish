...
struct CorrectionBundle {
  ...
  void update_pawn(int bonus) {
    ...
    int clampedBonus =
        std::clamp(bonus, -CORRECTION_HISTORY_LIMIT, CORRECTION_HISTORY_LIMIT);
    int16_t newVal = val + clampedBonus -
                     val * std::abs(clampedBonus) / CORRECTION_HISTORY_LIMIT;
...
inline Key non_pawn_key(const Position &pos, Color c) {
  return pos.pieces(c, KNIGHT) ^ pos.pieces(c, BISHOP) ^ pos.pieces(c, ROOK) ^
         pos.pieces(c, QUEEN);
}
...
inline int compute_full_correction_value(
    ...
    const ContinuationCorrectionHistory *contCorrHist4, const Position &pos,
    Move prevMove) {
  ...
  int wnpcv = corrHist.at(nonPawnWhiteIdx, us)
                  .nonPawnWhite.load(std::memory_order_relaxed);
  int bnpcv = corrHist.at(nonPawnBlackIdx, us)
                  .nonPawnBlack.load(std::memory_order_relaxed);
...
inline Value to_corrected_static_eval(Value v, int correctionValue) {
  return std::clamp(v + correctionValue / 131072, VALUE_TB_LOSS_IN_MAX_PLY + 1,
                    VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// Update full correction history (matching Stockfish)
inline void
update_full_correction_history(UnifiedCorrectionHistory &corrHist,
                               ContinuationCorrectionHistory *contCorrHist2,
                               ContinuationCorrectionHistory *contCorrHist4,
                               const Position &pos, Move prevMove, int bonus) {
  ...
  corrHist.at(nonPawnWhiteIdx, us)
      .update_nonpawn_white(bonus * nonPawnWeight / 128);
  corrHist.at(nonPawnBlackIdx, us)
      .update_nonpawn_black(bonus * nonPawnWeight / 128);
  ...
  if (prevMove.is_ok() && contCorrHist2 && contCorrHist4) {
    ...
    if (pc != NO_PIECE) {
      Square to = prevMove.to_sq();
      history_update((*contCorrHist2)[pc][to], bonus * 127 / 128,
                     CORRECTION_HISTORY_LIMIT);
      history_update((*contCorrHist4)[pc][to], bonus * 59 / 128,
                     CORRECTION_HISTORY_LIMIT);
    }
  }
}
